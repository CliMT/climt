# Cork Longwave: Ozone (O₃) Absorption — Design

**Date:** 2026-06-01
**Status:** Design — awaiting review before implementation plan
**Author:** scoping session (climt `feature/cork-radiation`)

## Goal

Give `CorkLongwaveRadiation` a longwave ozone absorber so it can reproduce a
realistic stratosphere. Experiment #10 of the CO₂ band-refinement plan
(`docs/superpowers/plans/2026-05-16-cork-co2-band-refinement.md`) showed that
...
column. CORK currently has **no ozone absorber at all**, so every prior
CORK-vs-RRTMG head-to-head was RRTMG-with-O₃ vs CORK-without. The dominant LW
feature is the O₃ **9.6 µm band (~980–1080 cm⁻¹)**, which lies inside CORK's
...
**not** an O₃-self-VMR axis (so it does not correspond to option 3). CORK's
plain additive overlap is a coarser treatment of that overlap, consistent with
CORK's existing coarse-band philosophy and adequate in a window band where CO₂ is
weak and O₃ is optically thin.

## Architecture fit

The CORK correlated-k stack already has every primitive this needs; the work is
wiring them together in a combination that has **not been exercised before**.

### What already exists (no change)

- **`interpolate_k`** (`climt/_components/cork/optics/correlated_k.py:96`)
...
- **The netcdf writer** (`scripts/cork_table_builder/netcdf_writer.py`) already
...
1. **Table builder** (`scripts/cork_table_builder/kappa_sampling.py` +
   `scripts/generate_cork_tables_linepyline.py`). Today non-H₂O absorbers are
...
2. **Component** (`climt/_components/cork/lw/component.py`). The
...
3. **New shipped table.** Regenerate the Earth LW table with the O₃ gas, e.g.
   `climt/_data/cork/correlated_k/earth_low_res_lw_co2refined_o3.nc`
...
Extend `scripts/experiments/radiative_equilibrium_cork_convergence.py` to run
**CORK-with-O₃ vs RRTMG-with-O₃**, both using climt's `init_ozone` profile (same
field). Acceptance: CORK recovers most of the ~30 K stratospheric warming O₃
