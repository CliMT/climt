# Cork Correlated-k Table Optimizer — Design
...
Building a cork (CORK) correlated-k LW table that matches line-by-line
(LBL) radiative transfer is currently a manual, expert-driven loop: a human
picks band edges, runs the generator, runs Cork on a fixed profile,
...
`docs/superpowers/plans/2026-05-16-cork-co2-band-refinement.md` distilled a set of
...
that places bands, decides how many are needed, builds the table, compares CORK
...
4. **Splitting fixes dry, not moist** (exp #23). **→ band splitting is chosen
   ...
   continuum lever.**
5. **Target accuracy** is RRTMG's own agreement with LBL: |LBL − CORK| total OLR
...
| Cork RT solve | band edges + table | low (njit) | every iteration |
...
loop in the **climt** env (it needs Cork). It shells out to
`conda run -n linepyline python -m cork_optimizer.<module>` only to (re)build the
...
- **`kappa_cache.py`** (linepyline env) — sample κ(ν,T,p,X) on the scenario grid
  via `cork_table_builder.kappa_sampling`, including the decoupled-continuum
...
- **`table_build.py`** (numpy only) — given a cached κ-cube + candidate
  ...
  `.nc` table. Reuses `cork_table_builder.k_distribution`,
  `planck_fraction`, `netcdf_writer`. No linepyline.
- **`evaluate.py`** (climt env) — run `CorkLongwaveRadiation` on the validation
  columns, return residuals vs cached truth: total OLR error (moist & dry),
  heating-rate RMSE/level, and per-band re-binned LBL−CORK for attribution.
...
- **`scripts/optimize_cork_table.py`** — CLI driver (scenario, kind, seed bands,
...
2. **Otherwise split the worst dry band.** Choose the band with the largest dry
   per-band `|LBL − CORK|` OLR residual (splitting fixes dry — exp #23). Place the
...
no linepyline, no CORK run needed to choose the split.
...
resulting OLR/heating-rate residuals, accept/reject) to the active cork
...
- **`table_build.py`** — round-trips a known κ-cube to a table and checks
  k-coefficients/shape against `cork_table_builder` directly.
- **`evaluate.py`** — runs CORK on a tiny column, checks residual wiring against a
  cached toy truth.
