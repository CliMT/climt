...
1. **The discovery story (faithful).** The shipped 4-band Earth-LW table ran far too warm vs RRTMG. A single-column + RCE investigation root-caused the warm bias to the **correlated-k longwave scheme**, not the line data:
   - Line-by-line truth (linepyline at PF's D=1.66) matches RRTMG; PF over-traps by ~17 (dry) to ~36 (moist) W/m².
...
| single-column LBL−PF | — | +11.5 moist / +11.0 dry (= candD; the corr-k floor) |
...
| throughput, 1000 cols | — | **PF 37.9 µs/col vs RRTMG 45.2** (~540× over the start) |

**Best teaching subtlety:** single-column fixed-profile LBL−PF reads +11 W/m² for *both* the candD prototype and the shipped table, yet full RCE converges to ~2 K. The fixed-profile metric **over-states** the error a self-adjusting column actually incurs — a lesson about which diagnostic to trust.
...
- `scripts/experiments/bench_pf_vs_rrtmg.py` — the fast-vs-faithful benchmark (PF vs RRTMG, N columns).
- `scripts/experiments/eval_band_structure.py` — single-column LBL−PF per band (`co2=` override).
- `scripts/experiments/eval_co2_interp_accuracy.py` — the A5 probe (Part 1 leave-one-out; Part 2 off-node vs LBL).
- `scripts/experiments/rce_{moist,dry}_pf_vs_rrtmg.py` — RCE (`--table`, `--co2` flags) for the headline T_sfc contrast.
...
- A spec: `docs/superpowers/specs/2026-06-03-pf-co2-adjustable-hifi-earth-lw-design.md`
- A plan: `docs/superpowers/plans/2026-06-04-pf-co2-adjustable-hifi-earth-lw.md`
- Investigation log (all results, the full arc): `docs/superpowers/plans/2026-05-16-pf-co2-band-refinement.md`
