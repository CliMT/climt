# Cork LW: CO2 15 μm Band Refinement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Refine the Earth LW cork band partition inside the 500–800 cm⁻¹ CO2 15 μm region so the line-core opacity is no longer diluted by band-averaging with the wings, and validate against RRTMG cool-to-space and radiative-equilibrium diagnostics.

**Architecture:** The CORK component already reads `num_bands` from the table's `k_coefficients` shape (`climt/_components/cork/lw/component.py:42`), so no component code needs to change. The fix is entirely in table generation + a new shipped `.nc` + diagnostic validation. Two-stretch quadrature (already wired into `scripts/generate_cork_tables.py`) stays on and composes with the new band partition. All changes are additive — new files alongside existing ones — so rollback is delete-only.

**Tech Stack:** numpy, exo_k (in `radiation` conda env), climt (in `climt` conda env), matplotlib.

**Background (read before starting):**
- Prior diagnostic work in this session established: at isothermal 250 K with q=0, 376 ppm CO2, the shipped CORK tables (`earth_low_res_lw_8gpt.nc` with band edges `10,500,800,1800,3250` and Gauss-Legendre quadrature) reproduce OLR correctly but collapse cool-to-space at p < ~10 hPa. Two-stretch quadrature with g_split sweep (0.90 → 0.99) only partially closes the gap, with the same kink shape persisting at all g_split values. Conclusion: the 500–800 cm⁻¹ band is too wide — one k-distribution can't represent both the 667 cm⁻¹ line cores and the wings without diluting the cores.
- Existing diagnostic scripts: `examples/cooling_to_space_T_sweep.py`, `examples/cooling_to_space_gsplit_sweep.py`, `examples/radiative_equilibrium_cork_convergence.py`.
...
- Conda envs: use `climt` for everything except table generation; use `radiation` (has `exo_k`) for `generate_cork_tables.py`. See memory `feedback_conda_env.md`.
...
**Files:**
- Modify: none yet — we use existing CLI flags on `scripts/generate_cork_tables.py`.
- Create: `climt/_data/cork/correlated_k/earth_low_res_lw_co2refined.nc`
...
```bash
conda run -n radiation python scripts/generate_cork_tables.py \
  --input /Users/joymonteiro/github/chaverot/data/correlated-k_tables/Earth_376ppmCO2_R500_10-30k.ktable.SI.h5 \
  --output climt/_data/cork/correlated_k/earth_low_res_lw_co2refined.nc \
  --kind lw \
...
nc = netcdf_file('climt/_data/cork/correlated_k/earth_low_res_lw_co2refined.nc', 'r')
...
- [x] **Step 3: Smoke-test that `CorkLongwaveRadiation` loads the new table**

Run:
```bash
conda run -n climt python -c "
from climt._components.cork import CorkLongwaveRadiation
cork = CorkLongwaveRadiation(optics='correlated_k', table='earth_low_res_lw_co2refined')
print('num_bands:', cork._num_bands)
print('num_gpts:', cork._num_gpts)
"
```
...
git add climt/_data/cork/correlated_k/earth_low_res_lw_co2refined.nc
git commit -m "feat(cork): add CO2-refined Earth LW k-table (6 bands, 2-stretch ngpt=8)"
```
...
LABELS = [
    ...
    "CORK 4-band 2s g=0.97",
    "CORK 6-band 2s g=0.97 (refined)",
]
TABLES = {
    "CORK 4-band GL (shipped)":      "earth_low_res_lw_8gpt",
    "CORK 4-band 2s g=0.97":         "earth_low_res_lw_8gpt_2s097",
    "CORK 6-band 2s g=0.97 (refined)": "earth_low_res_lw_co2refined",
}
...
```python
ax.set_title(
    f"Cool-to-space: 4-band vs CO2-refined 6-band CORK — T_iso = {T_ISO:.0f} K"
)
```
...
Expected stdout: 4 rows of OLR (~221.4 W/m²) and max|HR| values; OLR for all 3 CORK variants should match within ~0.1 W/m² (table dimension change doesn't break total opacity).
...
git commit -m "test(cork): switch cool-to-space diag to compare 4-band vs CO2-refined 6-band"
```
...
**Files:**
- Modify: `examples/radiative_equilibrium_cork_convergence.py` — replace the g-point-convergence comparison with a band-structure comparison.

- [x] **Step 1: Edit the script header constants**

In `examples/radiative_equilibrium_cork_convergence.py`, replace the `LABELS`, `COLORS`, `STYLES`, `HR_KEY` block with:

```python
LABELS = ["RRTMG (16 bands)", "CORK 4-band GL", "CORK 4-band 2s", "CORK 6-band 2s (refined)"]
COLORS = ["black", "#999999", "#ff7f0e", "#d62728"]
STYLES = ["-", "--", "--", "-"]
HR_KEY = {
    "RRTMG (16 bands)":          "air_temperature_tendency_from_longwave",
    "CORK 4-band GL":              "longwave_heating_rate",
    "CORK 4-band 2s":              "longwave_heating_rate",
    "CORK 6-band 2s (refined)":    "longwave_heating_rate",
}
```

- [x] **Step 2: Edit the component instantiation block**

Find the three `cork_2gpt = ...`, `cork_4gpt = ...`, `cork_8gpt = ...` lines (around `examples/radiative_equilibrium_cork_convergence.py:67-69`) and replace with:

```python
cork_gl   = CorkLongwaveRadiation(optics="correlated_k", table="earth_low_res_lw_8gpt")
cork_2s   = CorkLongwaveRadiation(optics="correlated_k", table="earth_low_res_lw_8gpt_2s097")
cork_ref  = CorkLongwaveRadiation(optics="correlated_k", table="earth_low_res_lw_co2refined")
```

Replace the `surface_cork_2/4/8` and `cork_state_2/4/8` blocks with three corresponding renames (`surface_cork_gl`, `surface_cork_2s`, `surface_cork_ref` and `cork_state_gl`, `cork_state_2s`, `cork_state_ref`).
...
Also inspect `radiative_equilibrium_cork_convergence.png`: stratospheric T for refined-band should sit between RRTMG (~180 K at 1 hPa) and shipped 4-band (~120 K). If refined-band's strat T is now within ~10–20 K of RRTMG, that's success.

- [x] **Step 5: Commit**

```bash
git add examples/radiative_equilibrium_cork_convergence.py
git commit -m "test(cork): RCE comparison — 4-band vs CO2-refined 6-band Earth LW"
```
...
**Files:**
- Create: `climt/_data/cork/correlated_k/earth_low_res_lw_co2refined7b.nc`
...
conda run -n radiation python scripts/generate_cork_tables.py \
  --input /Users/joymonteiro/github/chaverot/data/correlated-k_tables/Earth_376ppmCO2_R500_10-30k.ktable.SI.h5 \
  --output climt/_data/cork/correlated_k/earth_low_res_lw_co2refined7b.nc \
  --kind lw \
...
```python
LABELS.append("CORK 7-band 2s g=0.97 (deeper)")
TABLES["CORK 7-band 2s g=0.97 (deeper)"] = "earth_low_res_lw_co2refined7b"
...
```bash
ls -la /Users/joymonteiro/github/climt/climt/_data/cork/correlated_k/earth_low_res_lw_8gpt_2s*.nc /Users/joymonteiro/github/climt/climt/_data/cork/correlated_k/earth_low_res_lw_8gpt_twostretch.nc
```
...
- The plan does not attempt to modify `CorkLongwaveRadiation` or any component code, because the component already reads band count from the table shape (confirmed at `climt/_components/cork/lw/component.py:42`). If that assumption breaks during Task 1 Step 3, the plan instructs the worker to STOP and report, not to fix it inline.
- Quadrature change from prior session is preserved (already merged into `scripts/generate_cork_tables.py`); this plan composes with it and does not re-touch it.
...
**Hypothesis.** RRTMG settles to T_strat ≈ 190 K while all CORK variants sit at 130–160 K in pure-LW radiative equilibrium (no SW heating). Initial guess: the cork tables have insufficient LW opacity in the upper atmosphere.

**Method.**
- Ran `examples/radiative_equilibrium_cork_convergence.py` (modified to N_STEPS = 8000 ≈ 1000 days @ 3 h timestep, slab surface, SW = 240 W m⁻², CO₂ = 376 ppm, dry).
- Models: RRTMG (16 bands), CORK 4-band shipped, CORK 6-band 2s g=0.97 (CO₂-refined), CORK 6-band GL (CO₂-refined), CORK 10-band GL.
- Added `upwelling_longwave_flux_in_air_per_band` capture and a per-band brightness-temperature diagnostic at p ≈ 100 hPa using band-integrated Planck inversion. New plot: `examples/radiative_equilibrium_cork_per_band.png`.

**Result (per-band BT at 105 hPa vs simulated T_local):**

| Table | T_local | CO₂ core (band, BT) | Wing bands BT |
|---|---|---|---|
| CORK 4-band shipped | 154 K | 500–800 cm⁻¹ lumped, BT = 233 K | n/a (lumped) |
| CORK 6-band 2s g=0.97 | 157 K | 630–700, BT = 164 K | 500–630: 252 K, 700–800: 242 K |
| CORK 6-band GL | 155 K | 630–700, BT = 164 K | 500–630: 251 K, 700–800: 243 K |
| CORK 10-band GL | 149 K | 630–700, BT = 158 K | 500–630: 249 K, 700–820: 246 K |

All non-CO₂-core bands at 100 hPa carry brightness temperatures ≥ 230 K → essentially window-like (κ ≈ 0 aloft).

**What we learned.**
1. Cork physics is internally consistent: T_local ≈ CO₂-core BT for all 6/10-band tables, exactly as the κ-weighted Planck-balance argument predicts. Cold stratosphere is *not* a bug in the integrator, time-stepping, or convergence — it's the equilibrium the spectral distribution wants.
2. The CO₂ core (630–700 cm⁻¹) is the *only* band constraining T_strat in the CORK tables. Everything else is window-like at 100 hPa.
3. RRTMG's warmer stratosphere (~190 K) must come from the **CO₂ wings (500–630 and 700–820 cm⁻¹) carrying nontrivial opacity** in the stratosphere, raising their BT and pulling T_eq up via the κ-weighted balance. CORK tables apparently have these wings as windows — opacity concentrated too tightly in the 630–700 core.
...
**What we rejected.**
- "CORK runs haven't converged yet." Rejected — Top-N-level T panel is flat, OLR plateaus at 240 W m⁻², TOA imbalance ≈ 0.
...
**Files touched.**
- `examples/radiative_equilibrium_cork_convergence.py` — N_STEPS to 8000, added per-band capture + diagnostic figure + brightness-T inversion helper.
- New diagnostic figure: `examples/radiative_equilibrium_cork_per_band.png`.
...
**Method.** `examples/experiment_co2_wing_boost.py` copies the baseline table, multiplies `k_coefficients` in bands 1 and 3 by 1× / 3× / 10× / 30×, writes `*_wingboost{N}x.nc` (NETCDF3_CLASSIC, required by CORK loader), and runs 1000 days of pure-LW RE for each.
...
4. **Look beyond CO₂.** RRTMG includes O₃ 9.6 μm and H₂O continuum LW; even at zero H₂O specific humidity, the O₃ band could be contributing stratospheric warming. The CORK table at 376 ppm CO₂ + 0 ppm O₃ may simply lack a critical absorber. Worth checking what gases the Chaverot table has baked into the "premixed" background.

**Files touched.**
- `examples/experiment_co2_wing_boost.py` (new).
- New diagnostic figure: `examples/experiment_co2_wing_boost.png`.
- Throwaway `.nc` files in `climt/_data/cork/correlated_k/`: `earth_low_res_lw_co2refined_gl_wingboost{3,10,30}x.nc` — do not commit; delete after experiment series concludes.

### 2026-05-18 — Direct opacity inspection (quantitative confirmation)

**Hypothesis (clarified).** The CORK k-tables over-concentrate CO₂ opacity into the 630–700 cm⁻¹ core (τ_total ≫ 1000) while leaving the wings (500–630, 700–820 cm⁻¹) with too little total opacity to saturate anywhere except the lower troposphere.

**Method.** `examples/diagnose_cork_opacity_structure.py` equilibrates the column and then makes one forward call to capture `longwave_optical_depth_per_band` directly from the CORK component diagnostic (no offline interp — guaranteed self-consistent with the production calc). Computes cumulative τ(TOA → p) per band and finds p(τ=1).
...
**Reframed hypothesis.**
The discrepancy with RRTMG is not "CORK tables have wrong total CO₂ opacity" — they likely have *too much* in the core and *too little* in the wings. The fix is structural: regenerate the tables with either (a) finer g-point distribution that captures moderate-strength wing lines, or (b) different band edges that don't lump the strong core into a band where it dominates everything.

**Candidate next experiments.**
1. **Compare to LBL**: take the existing `examples/compare_lbl_cork_band_k.py` diagnostic and quantify how badly the Chaverot-rebinned τ underestimates wing opacity vs an LBL benchmark at, say, 100 hPa / 250 K.
...
**Files touched.**
- `examples/diagnose_cork_opacity_structure.py` (new; first version was buggy offline interp, rewrote to use the component's own per-band τ diagnostic).
- `examples/diagnose_cork_opacity_structure.png` — opacity & cumulative-τ plots for both 6-band and 10-band tables.
...
1. **Generate three new tables** (requires `radiation` conda env per memory `feedback_conda_env.md`; source HDF5 per memory `reference_chaverot_ktables.md`):
   ...
   conda run -n radiation python scripts/generate_cork_tables.py \
     --input /Users/joymonteiro/github/chaverot/data/correlated-k_tables/Earth_376ppmCO2_R500_10-30k.ktable.SI.h5 \
     --output climt/_data/cork/correlated_k/earth_low_res_lw_4band_16gpt.nc \
     --kind lw \
   ...
   conda run -n radiation python scripts/generate_cork_tables.py \
     --input /Users/joymonteiro/github/chaverot/data/correlated-k_tables/Earth_376ppmCO2_R500_10-30k.ktable.SI.h5 \
     --output climt/_data/cork/correlated_k/earth_low_res_lw_4band_32gpt.nc \
     --kind lw \
     --bands 10,500,820,1800,3250 \
     --ngpt 32 \
     --quadrature two-stretch \
   ...
   conda run -n radiation python scripts/generate_cork_tables.py \
     --input /Users/joymonteiro/github/chaverot/data/correlated-k_tables/Earth_376ppmCO2_R500_10-30k.ktable.SI.h5 \
     --output climt/_data/cork/correlated_k/earth_low_res_lw_6band_16gpt.nc \
     --kind lw \
     --bands 10,500,630,700,800,1800,3250 \
     --ngpt 16 \
...
2. **Add the three new tables to the diagnostic and the convergence script.**
   - `examples/diagnose_cork_opacity_structure.py` → add the three new tables to `TABLES`. Compare per-band τ(sfc) and p(τ=1).
   - `examples/radiative_equilibrium_cork_convergence.py` → add the three new tables as labels with distinct colors. Run for 1000 days.
...
- New: `climt/_data/cork/correlated_k/earth_low_res_lw_{4band_16gpt,4band_32gpt,6band_16gpt}.nc`
- Modify: `examples/diagnose_cork_opacity_structure.py`, `examples/radiative_equilibrium_cork_convergence.py`

### 2026-05-18 — Result: ngpt boost REJECTED

**Method (as proposed).** Generated three new LW tables from the Chaverot HDF5 source using `scripts/generate_cork_tables.py`:
...
Ran the per-band brightness-T diagnostic from `examples/radiative_equilibrium_cork_convergence.py` (1000 days, pure LW, SW=240 W/m², 376 ppm CO₂, q=0). Compared baselines (4-band 8 gpt shipped, 6-band 8 gpt GL, 10-band 8 gpt GL) against the three new high-ngpt variants.

**Result (per-band BT at p ≈ 105 hPa, T_local in K).**

| Table | ngpt | T_local | CO₂-core band BT | Wing bands BT |
|---|---|---|---|---|
| CORK 4-band GL (shipped) | 8 | 153.9 | 500–800 lumped, 233.4 | n/a |
| CORK 4-band 2s g=0.97 | **16** | 156.3 | 500–820 lumped, 236.0 | n/a |
| CORK 4-band 2s g=0.97 | **32** | 155.8 | 500–820 lumped, 236.3 | n/a |
| CORK 6-band GL | 8 | 155.0 | 630–700, **163.8** | 500–630: 250.9, 700–800: 242.6 |
| CORK 6-band GL | **16** | 156.4 | 630–700, **164.6** | 500–630: 250.9, 700–800: 242.5 |
| CORK 10-band GL | 8 | 148.9 | 630–700, **157.7** | 500–630: 249.1, 700–820: 245.6 |
...
**Result (opacity structure, from `examples/diagnose_cork_opacity_structure.py`).**

| Table | ngpt | Key CO₂ band | τ(sfc) | p(τ=1) hPa | T_strat (col min) |
|---|---|---|---|---|---|
| CORK 6-band GL | 8 | b2 630–700 (core) | 1011 | 4.38 | 117.1 |
| CORK 6-band GL | **16** | b2 630–700 (core) | **1309** | **2.40** | 113.0 |
| CORK 10-band GL | 8 | b3 630–700 (core) | 988 | 4.38 | 112.4 |
| CORK 4-band 2s | 8 (shipped uses GL) | b1 500–800 (lumped) | — | — | (153.9 in RCE) |
| CORK 4-band 2s g=0.97 | **16** | b1 500–820 (lumped) | 253 | 12.0 | 111.6 |
| CORK 4-band 2s g=0.97 | **32** | b1 500–820 (lumped) | 261 | 11.9 | 111.2 |
| CORK 6-band 2s g=0.97 | 8 (co2refined) | (not in this run) | — | — | — |
...
**Diagnosis (now load-bearing).** The cold-stratosphere bias is **upstream of the cork integrator**. The Chaverot R=500 source k-distribution itself lacks the moderate-strength CO₂-wing line opacity at low pressure that warms RRTMG's stratosphere. No band edge choice or g-point count can recover absorbers that aren't in the input CDF. The fix path lies outside this plan — either (a) regenerate Chaverot's source k-tables with explicit line-by-line wing treatment, or (b) source a different LW k-table (e.g. RRTMG's own, or RFMIP-style LBL) for the Earth CORK configuration.

**Files touched.**
- New (kept; useful as reference for future investigators verifying the ngpt-null result): `climt/_data/cork/correlated_k/earth_low_res_lw_4band_16gpt.nc`, `_4band_32gpt.nc`, `_6band_16gpt.nc`.
- Modified: `examples/diagnose_cork_opacity_structure.py` (added 3 new tables to `TABLES`), `examples/radiative_equilibrium_cork_convergence.py` (added 3 new labels + states + steppers).
...
- Evaluate alternative source k-tables (RRTMG's, or an LBL-derived set) for the cork Earth LW path.
- Document this conclusion in `climt/_components/cork/lw/component.py` or a `docs/` note so future contributors don't re-walk this path.
...
**What we rejected.**
- "Wing opacity is being eaten by re-binning inside generate_cork_tables.py." **Rejected** — the raw Chaverot CDF, before any climt-side processing, already shows the wing/core deficit at the same magnitude as the shipped tables suggest.
...
1. **Try a different source k-table for the same Earth LW config.** RRTMG ships its own k-tables that we know warm the stratosphere correctly. Extracting them into a form `generate_cork_tables.py` can ingest tests the diagnosis end-to-end and, if it works, gives a shipping path.
...
3. **Document the conclusion in `climt/_components/cork/lw/component.py`** so future contributors don't re-walk this diagnostic path.
...
3. **The radiative-equilibrium target may genuinely be this cold.** Observed stratospheric T is set by radiation + dynamics; CORK/RRTMG-pure-radiative answers could both be cold and only RRTMG's tuning + bigger gas set masks it.
...
**What I got wrong.** Last turn I speculated that the cold stratosphere might not be a real bias — that with O₃=0 the pure-radiative answer just *is* cold. That speculation was untethered from the actual data. `scripts/experiments/radiative_equilibrium_cork_convergence.py` already runs RRTMG alongside the CORK variants under identical conditions (CO₂=376 ppm, q=0, no O₃, SW=240 W/m²) for 1000 days. The script's per-band diagnostic block excludes RRTMG (gated on `CORK_TABLE[label] is not None`), so the printed `T_local` numbers in the experiment-#1 log were CORK-only and I didn't pull RRTMG's T_strat from the history.

**What the head-to-head actually shows** (read from `radiative_equilibrium_cork_convergence.png`, same run that produced the experiment-#1 numbers):

| level | RRTMG | CORK (any variant) | gap |
|---|---:|---:|---:|
| ~100 hPa | ~210–220 K | ~155–170 K | 40–60 K |
| 0.1–1 hPa | ~190–200 K | ~120–140 K | 60–70 K |

The LW heating rate panel shows the mechanism: RRTMG's stratospheric LW heating rate is ≈0 (radiative equilibrium / Schwarzschild balance), while every CORK variant has substantial net cooling extending well into the upper atmosphere.
...
- The "cold-stratosphere bias is upstream of the integrator" claim from the 2026-05-18 ngpt-rejection entry was **wrong**. The bias is real and head-to-head with RRTMG under identical inputs, so it is *downstream* of the source CDF — in the band partition, the integrator, or in physics RRTMG includes that CORK does not.

**Open hypotheses (revised).**
1. **CORK integrator / two-stream divergence from RRTMG's source-function treatment.** RRTMG fits the in-band Planck source with a specific scheme (multiple source-function moments per layer). CORK may use a single-Planck-fraction-per-band approximation that overestimates cooling-to-space when the band is very optically thick (the CO₂ core).
2. **CO₂ hot-band or 4.3 µm contribution dilution.** CORK's band containing 2326 cm⁻¹ averages it with surrounding window, possibly weakening absorption that RRTMG resolves.
...
1. Add a print of `T_local` and column-min T for RRTMG to `radiative_equilibrium_cork_convergence.py`'s diagnostic block, plus a per-level RRTMG–CORK temperature *and* heating-rate difference printout, to put exact numbers on the gap (no re-run needed if a `.npz` was saved; otherwise one re-run).
...
### 2026-05-18 — Follow-up experiment #3: Quantify the RRTMG–CORK gap per level

**Why.** The prior convergence run set up RRTMG and CORK head-to-head under identical conditions but the diagnostic block printed `T_local` only for CORK (gated on `CORK_TABLE[label] is not None`). The previous plot makes the gap visible at eyeball precision (RRTMG ~210 K at 100 hPa, CORK ~155–170 K) but we don't have exact numbers per level, per variant, and we don't have heating-rate diffs that would tell us *where in the column* the gap is generated.

**Method.** Modify `scripts/experiments/radiative_equilibrium_cork_convergence.py`:
...
- Add a new diagnostic block AFTER the TOA balance print: for each CORK label, print a per-level table of `(p, T_RRTMG, T_CORK, ΔT, HR_RRTMG, HR_CORK, ΔHR)` so we can localize where in the column CORK cools more than RRTMG and by how much.

Then re-run at ~300 days (a compromise: the previous plot's top-3-level T(t) panel showed RRTMG levels off by day 200–300, while CORK was still drifting in the upper levels through day 400–600 — so 300 days catches the bulk of the stratospheric pattern without the full 1000-day cost).

**Expected outputs.**
- Column-aligned table of ΔT(p) and ΔHR(p) per CORK variant.
...
### 2026-05-18 — Follow-up experiment #4: CORK k vs LBL on dry-adiabat profile

**Why.** Experiment #2 spot-checked k-table fidelity at one (T, p) point (T=250 K, p=100 hPa, three bands) and found ~factor-of-2 agreement. Experiment #3 quantified the RRTMG-vs-CORK bias as a column-internal redistribution: CORK +4 to +7 K below 400 hPa, −25 to −27 K at 23–41 hPa, OLR matching at 241–243 W/m². To distinguish "k-table is fine, integrator is the bug" from "k-table itself diverges across the column," need a per-layer, per-band comparison along a realistic T(p) profile.

**Method.** New script `scripts/experiments/compare_cork_k_vs_lbl_profile.py`:
- LBL: `lbl_k_adiabat.nc` (dry adiabat T_surf=288 K, 60 levels, dnu=0.1, CO₂-only at 376 ppm in air, pseudovoigt).
- CORK: load `k_coefficients(1, nband, ngpt, nT, nP, nX)` from each candidate CORK table, g-integrate to band-mean k(b, T, P, X), bilinear-interp in (log p, T) at X_H2O=1e-6 (table minimum) to each LBL layer's (p, T).
- LBL band-mean κ over each CORK band's wavenumber window (uniform-dnu mean).
- Compare CO₂-dominated bands only (LBL is CO₂-only — non-CO₂ bands like 10–500, 800–1800 give spurious 1e6+ ratios because CORK has H₂O baseline and LBL doesn't; expected, not real).
- Restrict ratio summary to T ≥ 100 K to avoid double-sided extrapolation artifacts (CORK T-grid bottoms at 50 K; LBL `T_FLOOR=80 K`).
- Spot-print at p ∈ {1000, 500, 200, 100, 50, 20} hPa with T from the adiabat.

**Result (CORK/LBL ratio, `earth_low_res_lw_co2refined_gl` table).**
...
**What we learned.**
1. **Two systematic k-table biases align with the experiment-#3 RRTMG-vs-CORK temperature pattern:**
   - At warm T (surface): CORK is ~2× too opaque across all CO₂ bands. More absorption near surface → traps energy in the boundary layer → CORK surface is warmer (matches +4 to +7 K tropospheric warm bias).
   - At cold T (strat): 500–630 cm⁻¹ wing CORK/LBL grows from 1.15 at 230 K to **19.7 at 94 K**. More wing opacity at cold T → cool-to-space radiates from higher/colder layers → cold strat (matches −25 K bias at 23–41 hPa).
...
3. **1800–3250 cm⁻¹ (CO₂ v3) is uniformly under-opaque in CORK (ratio 0.10–0.93)**, but Planck emission there is small at strat T, so radiative impact is limited.
...
1. `bin_down` distortion. Averaging native R=500 sub-bands into CORK bands may bias wing-vs-core differently at low T.
2. GL rebin from 20 native g-points onto 8 CORK g-points. GL nodes cluster toward (0, 1); at cold T where the strong-line tail dominates band-mean k, sparse sampling of the weak end may inflate the band-mean estimate.
...
- If raw Chaverot agrees with LBL at the cold strat layer → bias is in our `bin_down` + rebin + T-interp pipeline (CORK-side fix; mechanism candidates 1–3).
...
**Files touched.** New: `scripts/experiments/compare_cork_k_vs_lbl_profile.py`, `debug_data/cork_vs_lbl_k_profile__earth_low_res_lw_co2refined_gl.png`, `..._10band_gl.png`, `..._8gpt_twostretch.png`. Modified: `scripts/experiments/radiative_equilibrium_cork_convergence.py` (added `--days` argparse, per-level RRTMG-vs-CORK diagnostic block, `.npz` snapshot save). No code in `climt/` changed.
...
3. **But interpolation curvature alone (1.5×) does not explain experiment #4's 4.15× CORK-table/LBL wing ratio at 147 K.** The remaining ~2.7× is *not* interpolation — it is intrinsic to the CORK table at the nodes (bin_down + GL rebin, or the Chaverot source CDF at cold T). So "low resolution" is *one* contributor, not the whole story.
...
**T2 (next):** isolate node-fidelity from interp error — compare the *shipped CORK table* band-mean k at the 110/170 K nodes against LBL at those same nodes, to confirm the ~2.7× residual is node/pipeline error rather than interpolation. Then decide regen vs loader-fix.

**T2 RESULTS (2026-05-29) — the bias is NOT the T-grid; it is baked into the cold table node.** Script: `scripts/experiments/cork_node_fidelity_vs_lbl.py` (radiation env). Compared shipped `earth_low_res_lw_co2refined_gl` band-mean k *directly at its T-nodes* 110/170 K, at the exact p-node 1e4 Pa (no p-interp, no T-interp), vs LBL. p=1e4 Pa and T=110/170 K are all exact Chaverot nodes; the table bands match the diagnostic windows exactly.

| band (cm⁻¹) | CORK@110 | LBL@110 | **node110** | CORK@170 | LBL@170 | node170 | PFlin(147) | LBL(147) | r147 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
**RECOMMENDED FIX (for user decision): build the Earth LW table from linepyline, not Chaverot.** We already have the full linepyline table pipeline (`scripts/generate_cork_tables_linepyline.py` + `cork_table_builder/`) used for Titan/TRAPPIST-1e, and linepyline is the LBL reference that exonerates the core and convicts the source. Adding an `"earth"` CO₂=376 ppm LW scenario to that registry and regenerating `earth_low_res_lw` would replace the defective far-wing with the correct sub-Lorentzian opacity. This is the clean path; a Chaverot-side χ-factor patch is not available to us.
...
Generated `earth_low_res_lw_co2refined_linepyline.nc` (6 LW bands, grids matching the shipped table) via `scripts/generate_cork_tables_linepyline.py --scenario earth --kind lw`, then ran the node-fidelity sanity check (`cork_node_fidelity_vs_lbl.py <table>`) vs LBL.
...
**Code fix applied.** `scripts/generate_cork_tables_linepyline.py`: defaults changed to `dnu=0.1`, `line_shape="pseudovoigt"`, `binning=False`; added `--binning` opt-in flag (documented as low-accuracy preview only). No change to `cork_table_builder/`.
...
**Files added (exp #7).** `scripts/experiments/probe_lpl_kappa_apis.py`. Code edited: `scripts/generate_cork_tables_linepyline.py`.
...
**Deliverable:** `climt/_data/cork/correlated_k/earth_low_res_lw_co2refined_linepyline.nc` — validated drop-in for the defective Chaverot `earth_low_res_lw_co2refined_gl.nc`, identical grids/layout (ngpt=8), correct sub-Lorentzian far-wing opacity.
...
The deferred follow-up (a) — the definitive end-to-end RCE confirmation of the linepyline table. Ran `scripts/experiments/radiative_equilibrium_cork_convergence.py` trimmed to a 4-way head-to-head: RRTMG (16 bands) vs CORK 4-band shipped vs CORK 6-band Chaverot (`co2refined_gl`, defective) vs CORK 6-band linepyline (`co2refined_linepyline`, the exp #7 fix). Dry, CO₂=376 ppm, slab surface, SW=240 W/m², 1000 days.
...
**Finding 2 — the "cold-strat bias" was largely a missing-O₃ comparison artifact.** RRTMG's `get_default_state` populates a **climatological ozone profile (max ~3.9 ppm in the stratosphere)**; the script set CO₂ and zeroed `specific_humidity` but **never zeroed O₃**, while the CORK CO₂(+H₂O) tables contain none. So every prior "head-to-head" compared **RRTMG-with-O₃ vs CORK-without**. The 2026-05-18 "Correction" entry's assertion that the comparison ran with "no O₃" was never enforced in code and is **wrong**. Zeroing `mole_fraction_of_ozone_in_air` in the RRTMG state cooled RRTMG's middle atmosphere by **~30 K at 40–90 hPa** (90 hPa: 178.5→148.7 K; 40 hPa: 170.5→141.4 K; 10 hPa: 153.4→136.9 K; 2.4 hPa: 143.6→135.0 K). CORK curves unchanged. The −26 K CORK gap at 90 hPa collapsed to **+3 K**.

**Residual biases under matched inputs (dry, CO₂-only, no O₃).** Two separate, smaller effects remain:
- **Troposphere → ~50 hPa: CORK is WARMER than RRTMG by +5 to +16 K** (peak ~+16 K at 600–700 hPa). Shared by all CORK tables (linepyline did not change it). Consistent with exp #4's warm-T ~2× CO₂ over-opacity — never validated or fixed at warm nodes (230–290 K).
- **Above ~40 hPa: CORK still COLDER by −7 K (23 hPa) → −16 K (model top, 2.4 hPa).** Candidate causes: CORK integrator / two-stream source-function treatment vs RRTMG, or core-saturation behavior.

**Rejected.**
- "500–630 wing over-opacity causes the cold strat" (exp #6) — **falsified end-to-end.**
- "CORK has a 20–70 K cold-strat bias vs RRTMG" — **mostly an O₃ artifact**; under matched no-O₃ inputs the 40–90 hPa strat gap is within ±3 K. The headline number that motivated exps #1–#9 was a comparison-setup error.
...
**Files touched.** `scripts/experiments/radiative_equilibrium_cork_convergence.py` — trimmed to the 4-way comparison, added the linepyline table, and zeroed RRTMG O₃. No `climt/` code changed.
...
Followed up exp #10's residual tropospheric warm bias (CORK +5 to +16 K vs RRTMG, surface → ~50 hPa). exp #4 blamed a warm-T CO₂ over-opacity, but exps #7/#8 only validated k-fidelity at the COLD nodes (110/170 K). Script `scripts/experiments/cork_warm_node_fidelity.py` compares CORK table band-mean k (Chaverot & linepyline, GL-8 g-integrated, read at exact T/p nodes, dry) vs LBL (exp #9 envelope, grid-converged at p=1e4/1e5 Pa) at the WARM nodes 230/290 K.

**CORK/LBL core (630–700) band-mean:**
...
3. **Therefore the +15 K warm bias is NOT a CO₂ k-fidelity problem.** It is shared by all CORK tables and is a CORK-vs-RRTMG difference in the optically-thin bands / two-stream integrator / absorber set (RRTMG carries O₂=0.21 and possible CO₂ line-mixing/continuum the Chaverot/linepyline tables omit).
4. **Context — it is a PURE-radiative-equilibrium artifact.** `radiative_equilibrium_cork_convergence.py` has NO convective adjustment, so its troposphere is superadiabatic and unphysically warm for *both* models. The project goal is radiative-*convective* equilibrium with a reasonable tropopause; with convective adjustment the troposphere is reset to the adiabat and the pure-radiative warm overshoot should largely collapse.

**Rejected.** "The tropospheric warm bias is caused by CORK CO₂ k-table over-opacity" — the over-opacity is real in Chaverot but (a) fixed in linepyline and (b) radiatively inert because the core is saturated near the surface. Not the lever.
...
**Next.** Add convective adjustment (`DryConvectiveAdjustment`) to the RCE script and re-check the tropopause for CORK vs RRTMG — the direct test of the original goal. The CO₂ k-table warm bias is not worth chasing further (saturated/inert).

**Separately flagged (feature gap, not this plan).** CORK has no ozone absorber. exp #10 showed O₃ accounts for ~30 K of RRTMG's stratospheric warmth; for CORK to match a real atmosphere it needs an O₃ 9.6 µm LW band/absorber. Tracked as a future enhancement.

**Files added.** `scripts/experiments/cork_warm_node_fidelity.py`.

---

### Experiment #12 — Ascend the hierarchy: dry & moist RCE scripts (CORK-LW vs RRTMG-LW tropopause) (2026-06-01)

Pure radiative equilibrium (exps #10–#11) was the simplest rung — used to characterise CORK-LW in isolation. Per user direction, moving up the hierarchy to radiative-*convective* equilibrium, the configuration whose tropopause was the original target. Two separate single-column scripts were built so the two convection treatments can be compared independently. Both reuse the established CORK-vs-RRTMG setup: prescribed fixed SW (240 W/m², identical on all columns), O₃ zeroed everywhere (apples-to-apples LW; CORK has no O₃ absorber yet — see the spawned task), columns = RRTMG-LW / CORK-LW linepyline / CORK-LW shipped 4-band, slab surface.

- **Dry:** `scripts/experiments/rce_dry_cork_vs_rrtmg.py` — `SimplePhysics(boundary_layer=False)` + `DryConvectiveAdjustment`, dry column (q=0). Tendency stepper = `AdamsBashforth([lw, slab])`; SimplePhysics and dry adjustment applied by operator-splitting.
- **Moist:** `scripts/experiments/rce_moist_cork_vs_rrtmg.py` — `SimplePhysics(boundary_layer=True)` + `EmanuelConvection` (moisture spins up from surface evaporation). Tendency stepper = `AdamsBashforth([convection, lw, slab])`; `convection.current_time_step` set each step.

Both mirror the canonical `examples/column_code_with_slab.py` / `gmd_radiative_convective.py` coupling. Both **smoke-tested (14 steps) and run end-to-end without error**; the coupling, CORK band-count ordering (RRTMG built first), and diagnostics are validated. Full equilibration runs (dry ~400 d, moist ~200 d) are the user's to run. Each writes a T(p)/heating(/moisture) figure to `debug_data/` with a cold-point tropopause marker, plus an `.npz`. One plotting bug fixed: the Emanuel `air_temperature_tendency_from_convection` diagnostic has dims `('*','mid_levels')` (reversed vs the radiation profiles) — flattened for the single column.

**Next.** Run both to equilibrium; compare the CORK-LW vs RRTMG-LW tropopause pressure/temperature under dry vs moist convection. Expectation from exp #11: the pure-radiative +15 K tropospheric warm bias should largely collapse once convection sets the lapse rate, leaving the tropopause as the discriminating diagnostic.

---

### Caveat (2026-06-01) — CORK moisture handling is mechanically correct but UNVALIDATED

Traced via graphify (`component.py` → `optics/correlated_k.py`) and confirmed numerically. For the Earth `premixed_bg` tables, CorkLongwaveRadiation handles water vapour correctly *mechanically*:
...
**Implication:** the DRY RCE script rests on validated (CO₂-only, dry) physics; the MOIST script's CORK-vs-RRTMG differences — especially in the window region — are provisional until the H₂O axis is validated.

---

### Experiment #13 — H₂O-axis fidelity vs LBL (lines + MT_CKD): CORK moisture is validated (2026-06-01)

Script `scripts/experiments/cork_h2o_axis_fidelity.py` (linepyline env). LBL reference = `cork_table_builder.kappa_sampling.sample_kappa_grid` with the validated recipe (binning=False, dnu=0.1, pseudovoigt), sampled **with the MT_CKD continuum ON and OFF**, at tropospheric nodes (T=230/290 K, p=1e4/1e5 Pa) across all 7 H₂O VMR nodes. Compared to the CORK table GL-8 band-mean k. Reads via `np.exp(pressure_grid_log)` (ln-Pa). Output log: `debug_data/cork_h2o_axis_fidelity.log`.

**linepyline table, T=290 K / 1000 hPa, CORK/LBL across X=1e-6→1e-1 (physical range) | continuum× at X=0.1:**

| band | CORK/LBL (dry→X=0.1) | cont× @X=0.1 |
|---|---|---|
...
**Findings.**
1. **The H₂O axis is faithful to LBL within the known GL-8 quadrature under-read.** Across the physical humidity range (X ≤ 0.1) the radiatively important bands sit at CORK/LBL ≈ 0.76–1.0 — the *same* flat Gauss-Legendre ngpt=8 truncation signature (exp #8) that affects the already-validated CO₂ dry node (core 0.81–0.88 here vs 0.87 dry in exp #7). No H₂O-specific defect.
...
3. **MT_CKD continuum is confirmed present AND correctly matched.** `cont×` (LBL_on/LBL_off) > 1 exactly where expected — window and CO₂ wings under humidity — and CORK k tracks the continuum-**ON** reference, not OFF. This upgrades caveat #2 from "confirmed by code path" to **confirmed against LBL**. CORK is NOT missing window absorption.
...
**Conclusion.** CORK handles moisture **properly** — mechanically (correct q→VMR, trilinear interp, τ=k·air-mass) *and* in fidelity (H₂O lines + continuum match LBL to the same ~0.85 GL-8 factor as CO₂). The MOIST RCE script therefore rests on validated physics, with the caveat that the whole table carries a uniform ~0.85 ngpt=8 band-mean under-read (radiatively benign per exp #8: the under-sampled tail is saturated line cores that contribute little flux). Caveats #1 and #2 RESOLVED; #3 (interpolation *between* X nodes) untested but low-risk given the flat node ratios.
...
**Files added.** `scripts/experiments/cork_h2o_axis_fidelity.py`, `debug_data/cork_h2o_axis_fidelity.log`.

---

### Experiment #14 — Dry RCE bug: the "dry" run was secretly moist (2026-06-02)

First dry-RCE run (400 d, 5 m slab) came out **way too warm and unconverged**: RRTMG OLR=239.6/T_sfc=281 K (≈balanced), but CORK-linepyline OLR=213 (imbalance +27 → still warming) at T_sfc=306 K, CORK-shipped 228.6/301 K. CORK surface 25 K above RRTMG *and climbing* — impossible for dry convective adjustment, which can only *cool* the surface below the pure-radiative value (pure-RE CORK equilibrium was ~265–272 K in exp #10).
...
**Status.** Dry script fixed and smoke-tested (q=0 confirmed); awaiting a full converged run to read the CORK-vs-RRTMG dry tropopause/surface comparison.

**Why did RRTMG NOT run away under the same accidental moistening? (verified)** A side-by-side moist trajectory (both columns, identical SimplePhysics evaporation, 1 m slab, from 260 K) shows: **both moisten essentially identically** (q_max ≈ 1 g/kg for both), so moisture asymmetry is NOT the cause. But CORK's OLR is *lower* than RRTMG's at every step *despite CORK being warmer* (day 40: CORK 252 W/m² @ 284 K vs RRTMG 262 W/m² @ 276 K) → CORK has a stronger total greenhouse. The T_sfc gap widens monotonically (+2 K day 5 → +6 K day 20 → +9 K day 40 → +25 K day 400). So the divergence is driven by **CORK's intrinsic over-trapping (the exp #11 dry warm bias: thin bands / integrator, NOT CO₂/H₂O k which are LBL-faithful)**, with the Clausius-Clapeyron water-vapour feedback *amplifying* it as CORK warms. RRTMG, lacking the bias, finds a moist balance; CORK (seeded ~+15 K warm) climbs past where the feedback converges. **Implication: the MOIST (Emanuel) run will likely show the same CORK over-warming — it is the warm bias under water-vapour feedback, an important CORK finding, not a setup artifact.** Trajectory reproduced in conversation (not saved as a script).

---

### Experiment #15 — Moist RCE (Emanuel) confirms the warm-bias prediction (2026-06-02)

`rce_moist_cork_vs_rrtmg.py`, 200 d, 5 m slab, SW=240, O₃=0:

| column | T_sfc | q_sfc g/kg | OLR | imbalance (240−OLR) |
|---|---:|---:|---:|---:|
| RRTMG-LW | 283.5 | 2.02 | 254.1 | −14 (cooling) |
| CORK-LW linepyline | 296.9 | 6.77 | 233.3 | +6.7 (warming) |
| CORK-LW shipped 4band | 295.8 | 6.60 | 240.5 | −0.5 (≈balanced) |

**Findings.**
1. **Prediction confirmed.** CORK runs ~13 K warmer than RRTMG with ~3× the surface water vapour — the exp #14 warm-bias-under-water-vapour-feedback mechanism, now in the moist (Emanuel) configuration. Not converged (RRTMG cooling, CORK-lpl warming), so the converged gap will *exceed* 13 K.
2. **Table-independent.** CORK-linepyline and CORK-shipped both ≈296 K → the bias does not depend on the CO₂ table (consistent with exp #11/#13: CO₂ and H₂O k are LBL-faithful). The cause is the radiative scheme itself (thin bands / two-stream integrator).
...
**Conclusion / pivot.** Both dry and moist RCE are dominated by CORK's intrinsic over-trapping. The CO₂ and H₂O k-tables are validated and NOT the cause. The "reasonable tropopause" goal cannot be met until the over-trapping is understood. **Highest-value next step: a fixed-profile forward comparison** — run RRTMG-LW and CORK-LW once on one identical (T,p,q) profile and compare per-band up/down fluxes and per-level heating rates, to localize which bands/levels CORK over-traps (isolates the radiative scheme difference from the convective/feedback dynamics). This finally targets the exp #11 open question (thin bands / two-stream / source-function treatment).

---

### Experiment #16 — Fixed-profile LW forward comparison: CORK over-traps by 51 W/m² (2026-06-02)

`scripts/experiments/lw_forward_cork_vs_rrtmg.py`. ONE shared profile (288 K moist troposphere → ~200 K isothermal strat, q≈15 g/kg surface decaying upward), each scheme called once, NZ=40, CO₂=376, O₃=0, emissivity=1. No time-stepping, convection, or feedback.

| quantity | RRTMG | CORK (linepyline) | CORK−RRTMG |
|---|---:|---:|---:|
...
**Findings.**
1. **CORK over-traps by ~51 W/m² OLR on an IDENTICAL profile** (and radiates +46 W/m² more back to the surface). Surface emission matches to 0.03 W/m² (identical T_surf + emissivity=1) → the comparison is clean and the divergence is purely atmospheric. This single number fully explains the RCE warm bias: to restore OLR=240, CORK must warm/moisten until it emits 51 W/m² more.
2. **Definitively a radiative-scheme problem, not k-tables or feedback.** No feedback here, and the k-tables are LBL-validated (exps #7/#13). CORK's band-mean k is actually ~0.85× LBL (slightly *less* opaque) yet it emits *less* to space — so the over-trapping is in the SOLVER, not the absorption coefficients.
...
4. **CORK per-band TOA OLR** (total 191.2): rotation 10–500 = 58.7; 500–630 = 30.8; CO₂ core 630–700 = 6.6; 700–800 = 20.6; **window 800–1800 = 69.8**; 1800–3250 = 4.8. (RRTMG per-band not exposed, so no direct spectral diff — but the wide 800–1800 "window" band lumping the transparent 8–12 µm window with H₂O-opaque regions into one 8-g-point k-distribution is a prime suspect, the same dilution problem that motivated the CO₂ band split.)

**Mechanism hypotheses (next).** Since it's the solver: (a) diffusivity factor (1.66?) wrong or mis-applied in the two-stream LW; (b) band-Planck-fraction weighting biasing emission; (c) the 800–1800 window band too wide (dilutes window transparency); (d) two-stream/Eddington closure or layer-transmittance formulation over-absorbing. **Next step: inspect the CORK LW solver (`climt/_components/cork/lw/kernels.py`) — diffusivity factor, layer transmittance, Planck source — to find the 51 W/m².**

**Files added.** `scripts/experiments/lw_forward_cork_vs_rrtmg.py`, `debug_data/lw_forward_cork_vs_rrtmg.png`.

**Dry-vs-moist split (`--dry` flag).** Same profile, q zeroed (CO₂-only):

| | OLR deficit (CORK−RRTMG) | back-radiation excess |
|---|---:|---:|
| Moist | −51.4 | +46.1 |
| **Dry (CO₂-only)** | **−13.3** | **+20.9** |

Removing H₂O cuts the OLR over-trapping 51 → 13 W/m². So **~38 W/m² (≈¾) is H₂O/window-related** (the wide 800–1800 window band and/or H₂O handling — suspect #2), while a **~13 W/m² residual persists dry in CO₂-only** (a genuine solver-level discrepancy independent of H₂O — suspect #1). Dry CORK per-band TOA OLR: rotation 100.0, 500–630 46.2, core 6.6, 700–800 26.0, **window 800–1800 = 141.5**, 1800–3250 7.1. Both components are real; the dry CO₂-only residual (−13 W/m² OLR, +21 back-radiation) is the **cleanest "CORK Python two-stream vs RRTMG Fortran" signal** (the CORK solver was originally built against RRTMG), so it is the place to start a kernel-level comparison. **Next: read `climt/_components/cork/lw/kernels.py` — diffusivity factor, layer transmittance, Planck source — against the standard/RRTMG two-stream.**

**Kernel inspection (done 2026-06-02).** `climt/_components/cork/lw/kernels.py` (`lw_transport` / `_lw_transport_single_gpt`) uses the standard Schwarzschild two-stream with **diffusivity factor D=1.66** (`DIFFUSIVITY_FACTOR = 1.66`, lines 34/78 — CORRECT, so a wrong/double-applied diffusivity is ruled out). Per g-point: `up[k+1]=up[k]·trans + B[k]·(1−trans)`, `down[k]=down[k+1]·trans + B[k]·(1−trans)`, `trans=exp(−1.66·tau[k])`. This is the **isothermal-layer** emissivity method (one Planck value `B(T_mid)` per layer, emitted equally up/down). It is correct in form but coarser than RRTMG's **linear-in-τ Planck source** (Clough/RRTMG source function, B varies within each layer) — the prime suspect for the ~13 W/m² dry residual, especially on thick layers.
...
**Build.** `scripts/generate_cork_tables_linepyline.py` `SCENARIOS["earth"]["lw_band_edges"]` → `[10,500,630,700,800,1250,1800,3250]`. Generated `earth_low_res_lw_co2refined_linepyline_winsplit.nc` (linepyline env, validated recipe: binning=False, dnu=0.1, pseudovoigt, ngpt=8). `k_coefficients.shape=(1, 7, 8, 12, 8, 7)` — 7 bands. Added a `--table` arg to `scripts/experiments/lw_forward_cork_vs_rrtmg.py` so tables can be swapped without overwriting figures.

**Result (fixed-profile forward, CORK−RRTMG OLR deficit, W/m²):**
...
CORK per-band TOA OLR (moist), total 211.0 (was 191.2): the new transparent **800–1250 band emits 82.5 W/m²** while 1250–1800 emits only 7.1 — vs the lumped 800–1800 band's 69.8 in exp #16. Surface up matched to 0.03 W/m² (clean comparison). Figures: `debug_data/lw_forward_cork_vs_rrtmg_{moist,dry}_earth_low_res_lw_co2refined_linepyline_winsplit.png`.
...
**Files touched.** `scripts/generate_cork_tables_linepyline.py` (earth lw_band_edges), `scripts/experiments/lw_forward_cork_vs_rrtmg.py` (`--table` arg + table-tagged figure name). New table: `earth_low_res_lw_co2refined_linepyline_winsplit.nc`. New figures in `debug_data/`.
...
| table | window edges | moist OLR deficit (CORK−RRTMG) | window-region (800–1800) CORK emission |
|---|---|---:|---:|
...
**Files touched.** `scripts/generate_cork_tables_linepyline.py` (edges set to winsplit2, then reverted to 1250). New table: `earth_low_res_lw_co2refined_linepyline_winsplit2.nc` (rejected). Figure: `debug_data/lw_forward_cork_vs_rrtmg_moist_..._winsplit2.png`.

---

### Experiment #19 — Task B: dry solver residual is RESOLUTION-INDEPENDENT → a two-stream formulation difference (2026-06-02)

Ran the Task B resolution test: `lw_forward_cork_vs_rrtmg.py --dry` (CO₂-only, the cleanest solver signal) at NZ=40/80/160 with the winsplit table. Added an `--nz` arg.

**Result — flat with resolution:**

| NZ | dry OLR deficit (CORK−RRTMG) | dry back-radiation excess |
|---:|---:|---:|
| 40 | −9.83 | +15.87 |
| 80 | −9.86 | +15.87 |
| 160 | −9.86 | +15.87 |
...
3. **The true solver bias is larger than −10.** CORK's band-mean k is ~0.85× LBL (less opaque, exp #8), which alone would *raise* CORK's OLR. CORK over-traps anyway (lower OLR + higher back-radiation, both directions) → the solver effect is partially masked by under-opacity and is genuinely the dominant remaining error.
...
**Files touched.** `scripts/experiments/lw_forward_cork_vs_rrtmg.py` (`--nz` arg). No `climt/` code changed.

---

### Experiment #20 — Task D: CORK solver vs rte-rrtmgp term-by-term; source function is NOT the over-trapping (2026-06-02)

Per user direction, compared the CORK LW kernel against the actual reference solver it was built from, `rte-rrtmgp` (`/Users/joymonteiro/github/other_libs/rte-rrtmgp`, `rte/kernels/mo_rte_solver_kernels.F90` + `rte/frontend/mo_rte_lw.F90`), BEFORE pursuing the heavyweight Task D sub-steps.

**Term-by-term comparison (`_lw_transport_single_gpt` vs `lw_solver_noscat`):**

| element | CORK | rte-rrtmgp | difference |
|---|---|---|---|
...
This reordered the planned Task D: D1 (diffusivity) predicted null, D2 (Planck-fraction) a non-issue (CORK already uses `planck_frac`), and D3 (linear-in-τ) identified as the sole formulation difference.

**D1 — confirmed null.** Setting `DIFFUSIVITY_FACTOR` 1.66→1.64 moved the dry deficit −9.83 → −9.65 (+0.18 W/m²). Diffusivity is not the lever. (rte-rrtmgp's secant 1.64 ≈ CORK's 1.66.)

**D3 — implemented and TESTED; radiatively inert.** Implemented the Clough linear-in-τ source in the CORK kernel (`kernels.py`): added interface-level Planck (`lev_src`) computed from log-p-interpolated interface temperatures (`_interface_temperatures` in `component.py`), and the edge-anchored source with the 3rd-order `fact` series for small τ. Backward-compatible (lev_source=None → exact isothermal fallback; all 20 CORK kernel/validation tests pass; parmentier path unchanged).

Forward result (winsplit table): dry −9.83 → **−9.86**, moist −31.65 → **−32.02**. Essentially unchanged (~0.4 W/m²).

Resolution sweep (dry, CORK OLR absolute) confirms it's inert even at GCM-coarse resolution:
...
**Reframe — what the over-trapping is NOT (now exhaustively ruled out):** k-table band-means (exps #7/#11/#13), CO₂/window band width beyond the 1250 split (exps #17/#18), diffusivity (D1), source function (D3), vertical resolution (exp #19 + this). **What remains:** the comparison is NOT "same k, different solver" — RRTMG runs its own 16-band/140-g-point gas optics while CORK runs a 7-band/8-g-point correlated-k table. The leading remaining hypothesis is the **correlated-k band STRUCTURE itself** (few wide bands + 8 g-points violate the spectral-correlation assumption and over-trap vs RRTMG's 16 bands) and/or genuine CORK-table vs RRTMG-gas-optics differences. The window split helping +16 W/m² is direct evidence band structure matters.

**Decisive next experiment (#21): per-band OLR localization vs a line-faithful reference.** Run rte-rrtmgp (or RRTMG per-band) on the IDENTICAL fixed profile and compare per-band TOA up-flux to CORK's per-band OLR (already emitted by `lw_forward_cork_vs_rrtmg.py`). This localizes WHICH bands CORK over-traps — H₂O rotation (10–500), H₂O ν₂ (1250–1800), CO₂, or broadband — and distinguishes "band structure" from "table difference." That, not more solver work, is the way forward.

**Files touched.** `climt/_components/cork/lw/kernels.py` (linear-in-τ source, `lev_planck_source` arg), `climt/_components/cork/lw/component.py` (`_interface_temperatures`, interface Planck `lev_src`, wiring). `scripts/experiments/lw_forward_cork_vs_rrtmg.py` (`--nz` arg). No table changes.

---

### Experiment #21 — LBL OLR tiebreak: CORK over-traps vs ground truth; it is the correlated-k BAND STRUCTURE (2026-06-02)

The user's decisive framing: the CORK−RRTMG OLR gap (exp #16) could mean CORK over-traps OR RRTMG under-traps. Add **linepyline (LBL) as a third reference** on the IDENTICAL fixed profile to settle which scheme the truth agrees with. Crucially, run the LBL two-stream at **D=1.66 (CORK's diffusivity)** so the ONLY difference between LBL and CORK is full spectral resolution vs CORK's 7-band/8-g-point correlated-k — and the LBL is built by the SAME linepyline that made CORK's k-table, so the k-data is identical too.

Scripts: `scripts/experiments/dump_forward_profile.py` (climt env — saves the exp #16 profile + CORK/RRTMG OLR via one-RRTMG-per-process subprocess calls to the forward script; note a second RRTMG instance in one process returns garbage) and `scripts/experiments/lbl_olr_tiebreak.py` (linepyline env — `rtm.radiative_transfer` over 10–3250 cm⁻¹, dnu=0.1, pseudovoigt, binning=False, MT_CKD on, D=1.66; OLR binned onto CORK band edges).

**Result — the LBL truth sits with RRTMG, not CORK:**

| case | RRTMG | CORK | **LBL** | LBL−CORK | LBL−RRTMG | LBL closer to |
|---|---:|---:|---:|---:|---:|---|
| moist | 242.62 | 210.97 | **246.76** | **+35.8** | +4.1 | **RRTMG** |
| dry | 340.62 | 330.79 | **347.89** | **+17.1** | +7.3 | **RRTMG** |

**CORK genuinely over-traps** (~36 W/m² moist, ~17 W/m² dry vs line-by-line). RRTMG is approximately correct (within ~4–7 of LBL). This rejects any lingering "maybe CORK is right and RRTMG/O₃ was the artifact" — confirmed against ground truth.

**Per-band localization (LBL−CORK OLR, W/m²; >0 = CORK over-traps that band):**
...
**CORK OLR is flat across ngpt 8→16→32 (≤0.3 W/m²).** The ~36 (moist)/~17 (dry) W/m² over-trapping vs LBL is unchanged.
...
**GOAL (user-set).** Design a CORK LW band structure whose single-column TOA OLR matches line-by-line truth as well as RRTMG does: target **|LBL − CORK| < 5 W/m²** for both moist and dry on the fixed exp#16 profile (RRTMG itself achieves 4.1 moist / 7.3 dry, exp #21). Downstream validation: the converged moist-RCE surface-T gap vs RRTMG should drop to within a few K. Rationale (user): poor single-column RCE → wrong tropical tropopause + amplified water-vapour feedback; "reasonable circulation despite bad radiation" is not assumed. Keep the band count as low as hits the target.

**Recent surface-T gap (answering "how far from RRTMG?").** Most recent RCE is exp #15 (moist, 200 d, 5 m slab, SW=240, O₃=0, the pre-winsplit 6-band linepyline table): RRTMG T_sfc = 283.5 K, CORK = **296.9 K → +13.4 K and widening** (RRTMG cooling, CORK warming, not converged). Table-independent (CORK-shipped also ≈296). The ~+13 K maps to the ~30–36 W/m² single-column over-trapping (sensitivity ≈ 2.3 W/m²/K). No converged RCE for the winsplit table yet.

**Tooling.** Added `--bands` and `--no-continuum` to `scripts/generate_cork_tables_linepyline.py`; new `scripts/experiments/eval_band_structure.py` (climt env) computes CORK per-band TOA OLR on the fixed profile and re-bins the saved LBL spectrum onto the candidate's band edges — no linepyline re-run per iteration (the LBL spectrum is profile-only).

**Candidates tested (ngpt=8, vs LBL):**

| table | bands | edges | moist LBL−CORK | dry LBL−CORK |
|---|---|---|---:|---:|
...
**Files.** `scripts/experiments/eval_band_structure.py` (new), `lbl_olr_tiebreak.py` (+`--no-continuum`), `generate_cork_tables_linepyline.py` (+`--bands`,`--no-continuum`). Experiment tables (negative/diagnostic, deletable): `earth_lw_candA.nc`, `earth_lw_candB.nc`, `earth_lw_candA_nocont.nc`, `..._winsplit_{16,32}gpt.nc`, `..._winsplit2.nc`. LBL spectra: `debug_data/lbl_olr_spec_{moist,dry}{,_nocont}.npz`.
...
- `scripts/generate_cork_tables_linepyline.py` `--decouple-continuum`: builds the k-distribution from LINE-ONLY kappa (continuum off), computes the MT_CKD continuum as `full − lines`, band-grey-averages it over ν → `continuum_kappa(nband,nT,nP,nX)`, written by `write_lw_table(continuum_kappa=…)` (`cork_table_builder/netcdf_writer.py`). Also added `--bands` and `--no-continuum`.
- `climt/_components/cork/optics/correlated_k.py`: loads `continuum_kappa`; new `interpolate_continuum` adds it grey-across-g to every g-point's τ in `_compute_ck_optical_depth_additive` with the air-column-mass scaling.
- 28 CORK tests pass; existing tables unaffected.

**Two-step result on candC (= candB's 12 bands + decoupled continuum), vs LBL:**

| variant | moist LBL−CORK | dry LBL−CORK |
|---|---:|---:|
...
| table (all decoupled+log-X continuum unless noted) | bands | ngpt | moist LBL−CORK | dry LBL−CORK |
|---|---|---|---:|---:|
...
Converged moist RCE (`rce_moist_cork_vs_rrtmg.py --days 200`, Emanuel convection, SW=240, O₃=0, 5 m slab), comparing RRTMG vs CORK-candD (decoupled+log-X continuum, 14-band) vs CORK-winsplit (7-band, folded continuum, current shipped).

| column | OLR | T_sfc | gap vs RRTMG | q_sfc (g/kg) |
|---|---:|---:|---:|---:|
| RRTMG-LW | 254.10 | 283.54 | — | 2.02 |
| **CORK candD (decoupled continuum)** | 254.81 | **285.58** | **+2.04 K** | 2.38 |
| CORK winsplit (folded continuum) | 249.81 | 290.52 | +6.98 K | 3.60 |

**The single-column continuum fix translates directly to RCE.** Surface-T gap vs RRTMG: **+13.4 K (original 6-band, exp #15) → +7.0 K (winsplit 7-band) → +2.0 K (candD)**. candD's OLR now matches RRTMG to 0.7 W/m² (winsplit still traps ~4.3 W/m² more → +7 K). The water-vapour feedback that amplified the old bias is tamed: candD q_sfc 2.38 vs RRTMG 2.02 (winsplit 3.60, ~1.8× RRTMG). This confirms exps #24–26 end-to-end: the decoupled+log-X continuum + 14-band partition brings CORK moist RCE to within ~2 K of RRTMG.
...
| **CORK candD** | 240.09 | 279.03 | **+2.74 K** | 121.9 | 156.3 | 1.33 |
| CORK winsplit | 239.87 | 285.92 | +9.63 K | 121.9 | 161.2 | 2.35 |
...
| **CORK shipped default (4-band)** | 238.31 | **296.38** | **+20.1 K** | 583* | 259.8* | 6.94 |
| CORK candD (improved) | 240.09 | 279.03 | +2.74 K | 122 | 156.3 | 1.33 |
...
Converged dry RCE (`rce_dry_cork_vs_rrtmg.py --days 400`, SimplePhysics no-BL held dry + DryConvectiveAdjustment, 1 m slab, SW=240, O₃=0), RRTMG vs CORK-candB (12-band; continuum inert when dry, so this isolates the line k-distribution / band structure).

| column | OLR | imbal | T_sfc | gap | p_cnvtop | T_tp | q_max |
|---|---:|---:|---:|---:|---:|---:|---:|
| RRTMG-LW | 242.61 | −2.61 | 266.51 | — | 379 hPa | 201.1 | 0.000 |
| CORK-LW candB (12-band) | 242.27 | −2.27 | 267.48 | **+0.97 K** | 330 hPa | 195.5 | 0.000 |
...
**Combined RCE picture (both fixes validated):** DRY candB +1.0 K surface; MOIST candD +2.0 K surface (exp #27) — both vs RRTMG, down from the original +13 K. The decoupled+log-X continuum (moist) + 14-band partition deliver a CORK Earth-LW scheme that matches RRTMG RCE to ~1–2 K. Remaining work for the tropopause is more bands (diminishing returns).
...
> - **Task D (exp #20):** term-by-term vs rte-rrtmgp. Only solver-formulation difference = isothermal-layer vs linear-in-τ source. D1 (diffusivity) null; rte-rrtmgp's secant is 1.64≈CORK's 1.66. **Linear-in-τ implemented (kernel + interface Planck) and TESTED — radiatively inert (~0.4 W/m²) at NZ=10/20/40.** Source function is NOT the cause (confirms exp #19). Decision pending: keep (correct physics, passes tests) vs revert (simpler, inert).
> - **Task E (exp #21) — DECISIVE, the root cause is found.** Added linepyline LBL as a 3rd reference on the identical profile at D=1.66 (CORK-matched). **LBL OLR agrees with RRTMG, not CORK** (moist: LBL 246.8 vs RRTMG 242.6 vs CORK 211.0; dry: LBL 347.9 vs RRTMG 340.6 vs CORK 330.8). CORK genuinely over-traps ~36 (moist)/~17 (dry) W/m² vs ground truth. Same k-source, same D, same profile → **the cause is purely CORK's correlated-k BAND STRUCTURE** (7 wide bands/8 g-pts). Localized: worst in spectrally-inhomogeneous bands mixing transparent+opaque — 800–1250 window (+16.4 moist), 10–500 H₂O rotation (+9–11), 1250–1800 ν₂ (+4); CO₂ core 630–700 near-perfect (+0.5).
> - **Task F(b) g-points — REJECTED (exp #22):** ngpt 8→16→32 leaves CORK OLR flat; over-trapping is band width, not g-resolution.
> - **Band-structure exploration (exp #23) + GOAL set:** target |LBL−CORK| < 5 W/m² (moist & dry) = RRTMG's own fidelity. Splitting (candA 10-band, candB 12-band) fixes DRY (17→12) but barely moves MOIST (36→30): the moist window over-traps +16.7 regardless of how finely split.
> - **Exp #24 DECISIVE — the moist excess is the H₂O MT_CKD CONTINUUM.** Continuum OFF: moist window +16.7→+2.5, moist total +30→+13 (≈dry). So moist over-trapping = ~17 W/m² continuum (folded into CORK's line-sorted k-distribution; RRTMG instead adds it as a SEPARATE per-g-point term `tauself+taufor` with explicit H₂O²·p/H₂O·p scaling — `rrtmg_lw_taumol.f90:331`, NOT grey) + ~12 W/m² band-structure-fixable.
...
> - **Recent surface-T gap:** exp #15 moist RCE CORK = 296.9 K vs RRTMG 283.5 K = **+13.4 K, widening** (pre-winsplit table; no converged winsplit RCE yet).
...
> - **Code changes (all backward-compatible):** `generate_cork_tables_linepyline.py` (+`--bands`,`--no-continuum`,`--decouple-continuum`), `cork_table_builder/netcdf_writer.py` (+`continuum_kappa`), `correlated_k.py` (+`continuum_kappa` load, `interpolate_continuum` log-space, additive τ-add), new `scripts/experiments/eval_band_structure.py`. 28 CORK tests pass. linear-in-τ from exp #20 reverted; diagnostic-rename refactor preserved.
...
The cork longwave scheme (`CorkLongwaveRadiation`) gives a radiative-convective equilibrium that is **far too warm** vs RRTMG (and SOCRATES): in moist RCE, CORK surface ≈ +13 K and climbing, water-vapour-feedback amplified. **Root cause is now definitively identified (exp #21): CORK's correlated-k BAND STRUCTURE over-traps.** On an identical (T,p,q) profile, line-by-line truth (linepyline at CORK's D=1.66) gives OLR = 246.8 (moist)/347.9 (dry); RRTMG matches it (242.6/340.6) but **CORK emits far too little (211.0/330.8) — over-trapping ~36/~17 W/m²**. Since the LBL uses the SAME k-source and diffusivity as CORK, the only difference is CORK's 7-wide-bands/8-g-points vs full spectral resolution. NOT the k-tables (LBL-validated, exps #7–13), NOT the solver source function (exp #20, linear-in-τ inert), NOT diffusivity (D1), NOT vertical resolution (exp #19), NOT convection/feedback. The fix is a band-structure design decision (Task F), not a bug. Worst bands: 800–1250 window (+16 moist), 10–500 H₂O rotation (+9–11), 1250–1800 ν₂ (+4); CO₂ core is near-perfect.
...
2. **H₂O axis (incl. MT_CKD continuum) is LBL-faithful** within the same ~0.85 ngpt=8 factor across X≤0.1 (exp #13). CORK reads `specific_humidity`, converts to VMR correctly, interpolates the tabulated mixture k.
3. **The original "cold stratosphere bias" was mostly a missing-O₃ comparison artifact** (exp #10): RRTMG ran with climatological O₃; CORK has none. Under matched O₃=0, the strat gap at 40–90 hPa is within ±3 K.
...
CORK over-traps by 51 W/m² OLR on an identical moist profile. The dry (`--dry`, CO₂-only) test splits it into TWO independent causes:
...
- **(a) more spectrally-homogeneous bands.** Target the offenders: isolate a CLEAN transparent window (the 8–12 µm region) separate from BOTH its edges and the H₂O ν₂; give the wide 10–500 H₂O-rotation band its own refinement. KEY lesson from exp #18: separate transparent-from-opaque regimes; do NOT subdivide within a single regime (that hurt). Re-run `lbl_olr_tiebreak.py` + `lw_forward_cork_vs_rrtmg.py` after each candidate partition; success = LBL−CORK per-band → 0.
- **(b) more g-points — REJECTED (exp #22).** ngpt 8→16→32 leaves CORK OLR flat (≤0.3 W/m²); the over-trapping is band width, not g-resolution. Do not revisit.
...
- The MOIST RCE script's tropopause diagnostic is still broken (cold-point returns model top, 2.40 hPa). Replace with a convective-heating-top or WMO lapse-rate criterion (the dry script already uses a convective-θ-top — see `tropopause()` in `rce_dry_cork_vs_rrtmg.py`; θ won't work for moist, use the convective heating tendency instead). Then re-run dry+moist RCE with the fixed solver/table and compare the CORK-vs-RRTMG tropopause — the original project goal.

### KEY FILES & ARTIFACTS
Experiment scripts (all in `scripts/experiments/`, run with the climt env unless noted):
- `lw_forward_cork_vs_rrtmg.py` — **the key diagnostic.** Fixed-profile single-call RRTMG vs CORK; `--dry`, `--table <name>` (exp #17), and `--nz <N>` (exp #19) flags. Outputs `debug_data/lw_forward_cork_vs_rrtmg_{moist,dry}_<table>.png`.
- `rce_dry_cork_vs_rrtmg.py` — dry RCE (SimplePhysics no-BL + DryConvectiveAdjustment). FIXED to be truly dry (`use_external_surface_specific_humidity=True`, surface q=0); 1 m slab; convective-θ-top tropopause; prints imbalance + q_max.
- `rce_moist_cork_vs_rrtmg.py` — moist RCE (SimplePhysics BL + EmanuelConvection). Works; tropopause diagnostic still broken (Task C).
- `cork_warm_node_fidelity.py`, `cork_h2o_axis_fidelity.py` (linepyline env), `cork_node_fidelity_vs_lbl.py`, `raw_chaverot_vs_lbl_coldnode.py`, etc. — k-fidelity validation (all PASSED; see exps #11–#13).
- Note `pressure_grid_log` in the .nc tables is **natural-log of Pa** (use `np.exp`, not `10**`).

Tables (`climt/_data/cork/correlated_k/`):
...
### LOOSE ENDS
- **Spawned background task:** "Add O₃ 9.6 µm LW absorber to Cork" (CORK has no ozone; needed for realistic stratosphere — exp #10). Independent of the warm-bias work.
- The graph needs re-indexing for the scripts added after exp #13 (see above).
- Convergence: RCE runs need long spin-up; check `imbal = SW−OLR` (|·|≲1 W/m²) and `q_max` before trusting RCE numbers. With the +51 W/m² solver bias unfixed, CORK RCE will not converge to a sensible state — fix Task A/B first.
...
| table | moist CORK | moist LBL−CORK | dry CORK | dry LBL−CORK |
|---|---|---|---|---|
| candD (validated ref) | 235.29 | **+11.47** | 336.90 | **+10.99** |
| earth_hifi_lw | 235.26 | **+11.50** | 336.90 | **+10.99** |

→ earth_hifi_lw is fidelity-**identical** to candD (Δ ≤ 0.03 W/m²). CO₂ interp at off-node 376 ppm (between the 215/464 nodes) is essentially exact. The single-column LBL−CORK ≈ +11 is the *fixed-profile* number for **both** tables (the docstring's <5 W/m² target was never met by candD either); candD is nonetheless the table that converged to the design's +2.7 K moist-RCE result, i.e. the RCE metric — not fixed-profile OLR — is what gates the ship.

**Per-band LBL−CORK (earth_hifi_lw, W/m²; +ve = CORK over-traps):** the residual is the **correlated-k spectral-correlation floor on the line side** (continuum already decoupled), concentrated in the far-IR H₂O rotation band + CO₂/window region — exactly the deferred "line-side log-X" non-goal.
...
**New/changed validation tooling (committed):** `--co2`/`--table` flags on `rce_moist/dry_cork_vs_rrtmg.py`; `co2=` override on `eval_band_structure.py`; new `eval_co2_interp_accuracy.py`.

**RCE VALIDATION (Task 10c) — PASSED.** Single-column RCE, SW=240 W/m², O₃=0, CO₂ 280/500 ppm (1120 too high for RRTMG-LW). The robust quantity is the **CORK−RRTMG bias** (both columns share the same near-equilibrium state). Dry `imbal ≈ −2.5 W/m²` → marginally unconverged but equally for both columns, so the relative bias holds. CO₂ labels were not in the run paste, so the two moist / two dry runs are reported as the 280–500 ppm pair without asserting order.

| case | RRTMG T_sfc | CORK `earth_hifi_lw` T_sfc | **ΔT_sfc** | q_sfc CORK/RRTMG [g/kg] | p_tp CORK/RRTMG [hPa] |
|---|---|---|---|---|---|
...
**PEDAGOGICAL HEADLINE (sub-project B, "fast vs faithful"):** the 4-band lumped-window table is +11.9 K too warm with 3.2× runaway humidity and no tropopause; the 14-band decoupled-continuum + runtime-CO₂ table tracks RRTMG to ~1–2 K with a co-located tropopause and tamed water-vapour feedback. Single-column fixed-profile LBL−CORK is +11 W/m² for *both* candD and earth_hifi, yet full RCE converges to ~2 K — the lesson that the fixed-profile OLR metric over-states the error a self-adjusting column actually incurs.

**SHIPPED (Task 11 — default flipped).** `earth_low_res_lw.nc` is now a byte-copy of `earth_hifi_lw.nc` (sha `3192c45f…`); old 4-band/ngpt2 default preserved as `earth_low_res_lw_4band_ngpt2_before.nc` (sha `94570a87…`). Shipped-table integration test parametrized over both names; MANIFEST updated; graph re-indexed. The default CORK Earth-LW scheme is now 14-band, decoupled+log-X continuum, runtime CO₂ 10–10000 ppm.

### PERFORMANCE — CORK-LW njit pass (2026-06-04)

Benchmark `scripts/experiments/bench_cork_vs_rrtmg.py` (warm JIT, NZ=30). The 14-band×8-gpt table is 14× the old 4×2 band×gpt work, and the CO₂ axis doubles the inner interpolation — exposing two **pure-Python assembly loops** that had always been cheap at 4×2:

| stage (100 cols) | CORK-LW | vs RRTMG | fix |
|---|---|---|---|
...
**~540× total speedup, zero physics change** — both kernels are bit-for-bit vs frozen oracles (rtol 1e-6, `tests/test_cork_ck_njit.py`, `tests/test_planck_sources_kernel.py`); single-column OLR byte-identical (235.26 moist / 336.90 dry); 48 CORK tests green. Throughput scales linearly and CORK beats RRTMG at every size (per-column, 1000 cols: **CORK 37.9 µs/col vs RRTMG 45.2**; ratio 0.83–0.90× across 10/100/1000 cols).

Key finding: the `@njit(parallel=True)` **transport** kernel (Task 8, `lw_transport`) was *never* the bottleneck — sub-ms throughout. The cost was the optical-depth and Planck-source **assembly** loops, only visible once bands×gpts grew. Both now compiled. *Pedagogical hook for sub-project B: the faithful scheme is also the fast one — CORK matches RRTMG's accuracy AND beats its throughput once the hot loops are JIT-compiled.*
