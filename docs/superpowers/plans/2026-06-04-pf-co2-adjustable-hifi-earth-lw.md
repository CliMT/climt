# CO₂-adjustable, RRTMG-fidelity Earth-LW picket-fence table — Implementation Plan
...
The Earth LW table is a **premixed-background** correlated-k table: `gas_names == ["effective"]`, k is in m²/kg-of-air, and H₂O is the only *live* (runtime) absorber axis. The component branch that handles this is `_premixed_bg` in `climt/_components/picket_fence/lw/component.py`. This plan adds CO₂ as a **second** live axis to that same path. CO₂ today is baked into the table at 376 ppm and not read from state; after this work it is read from `mole_fraction_of_carbon_dioxide_in_air` (already in the default state at 330 ppm, see `climt/_core/initialization.py:730`).
...
|------|----------------|--------|
| `scripts/pf_table_builder/netcdf_writer.py` | netCDF schema writer | add `co2_vmr_grid` var + CO₂ dim on `k_coefficients` (Task 1) |
| `climt/_components/picket_fence/optics/correlated_k.py` | loader, `interpolate_k`, optical-depth assembly | read `co2_vmr_grid`; quadrilinear interp; thread `co2_vmr` (Tasks 2–4) |
| `climt/_components/picket_fence/lw/component.py` | LW component | declare CO₂ input, pass VMR into optics (Task 5) |
| `scripts/pf_table_builder/kappa_sampling.py` | line-by-line κ sampler | add CO₂ sweep axis (Task 6) |
| `scripts/generate_pf_tables_linepyline.py` | table generation driver | `earth` 14-band + CO₂-grid config; 4-D moveaxis (Task 7) |
| `climt/_components/picket_fence/lw/kernels.py` | LW transport | consolidate into single `@njit(parallel=True)` (Task 8) |
| `scripts/experiments/eval_band_structure.py` | LBL-vs-PF evaluator | CO₂×H₂O grid (Task 10) |
| `tests/`, `tests/pf_table_builder/` | unit tests | added throughout |
...
**Files:**
- Modify: `scripts/pf_table_builder/netcdf_writer.py:14-104`
...
Run: `conda run -n climt python -m pytest tests/pf_table_builder/test_netcdf_writer.py::test_write_lw_table_with_co2_axis_roundtrip -v`
...
In `scripts/pf_table_builder/netcdf_writer.py`, change the signature and body of `write_lw_table`. Replace the signature line `h2o_vmr_grid=None,` to add `co2_vmr_grid=None,` right after it, and replace the shape-unpacking + dimension/variable block. The new function reads:
...
Run: `conda run -n climt python -m pytest tests/pf_table_builder/test_netcdf_writer.py -v`
...
**Files:**
- Modify: `climt/_components/picket_fence/optics/correlated_k.py:10-21`
- Test: covered by Task 1's roundtrip test.

- [ ] **Step 1: Add `co2_vmr_grid` to the loaded variables**

In `climt/_components/picket_fence/optics/correlated_k.py`, edit the `_NETCDF_VARS` tuple to add `"co2_vmr_grid"`:
...
git commit -m "feat(pf): add optional CO2 axis to LW k-table format
...
**Files:**
- Modify: `climt/_components/picket_fence/optics/correlated_k.py:97-195`
- Test: `tests/test_picket_fence_co2_interp.py` (create)

- [ ] **Step 1: Write the failing test**

Create `tests/test_picket_fence_co2_interp.py`:
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_co2_interp.py -v`
Expected: FAIL — `interpolate_k() got an unexpected keyword argument 'co2_vmr'`.

- [ ] **Step 3: Implement the CO₂ axis in `interpolate_k`**

In `climt/_components/picket_fence/optics/correlated_k.py`, add a module-level constant just below `_NETCDF_VARS`:
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_lw.py -v`
...
git add climt/_components/picket_fence/optics/correlated_k.py \
        tests/test_picket_fence_co2_interp.py
git commit -m "feat(pf): quadrilinear CO2 axis in interpolate_k (geometric log-k)
...
**Files:**
- Modify: `climt/_components/picket_fence/optics/correlated_k.py:230-257, 323-388`
- Test: `tests/test_picket_fence_co2_interp.py` (add)

- [ ] **Step 1: Write the failing test**

Append to `tests/test_picket_fence_co2_interp.py`:
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_co2_interp.py::test_compute_ck_optical_depth_threads_co2 -v`
...
In `climt/_components/picket_fence/optics/correlated_k.py`:
...
git add climt/_components/picket_fence/optics/correlated_k.py \
        tests/test_picket_fence_co2_interp.py
git commit -m "feat(pf): thread co2_vmr through CK optical-depth assembly
...
**Files:**
- Modify: `climt/_components/picket_fence/lw/component.py:38-51, 100-108, 228-262, 404-411`
- Test: `tests/test_picket_fence_lw.py` (add)
...
Run: `conda run -n climt python -m pytest "tests/test_picket_fence_lw.py::test_picket_fence_lw_co2_axis_changes_olr" -v`
Expected: FAIL — CO₂ axis is ignored (OLR identical, assertion fails), or a `co2_vmr` KeyError.

- [ ] **Step 3: Detect the CO₂ axis at init**

In `climt/_components/picket_fence/lw/component.py`, in the `elif optics == "correlated_k":` block, after the `_has_h2o_axis` line, add CO₂ detection:
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_lw.py tests/test_picket_fence_co2_interp.py -v`
...
git commit -m "feat(pf): read CO2 from state as runtime axis in LW component
...
**Files:**
- Modify: `scripts/pf_table_builder/kappa_sampling.py:14-105`
- Test: `tests/pf_table_builder/test_kappa_sampling.py` (add; skipped unless linepyline present)

- [ ] **Step 1: Write the failing test**

Append to `tests/pf_table_builder/test_kappa_sampling.py`:
...
Run: `conda run -n linepyline python -m pytest tests/pf_table_builder/test_kappa_sampling.py::test_sample_kappa_grid_with_co2_axis -v`
...
In `scripts/pf_table_builder/kappa_sampling.py`, add a `co2_vmr_grid` keyword and a sweep that wraps the existing base sampling. Edit the signature to add `co2_vmr_grid: np.ndarray | None = None,` after `h2o_vmr_grid`, extend the docstring's Returns to mention the 5-D shape, and insert the CO₂ branch near the top of the body — right after `rtm = linepyline.rtm(...)` and the `nu_min/nu_max/dnu/nT/nP/nNu` setup, **before** the `non_h2o = {...}` line:
...
Run: `conda run -n linepyline python -m pytest tests/pf_table_builder/test_kappa_sampling.py -v`
Expected: PASS (or SKIP only if linepyline truly unavailable — run this in the `linepyline` env).

- [ ] **Step 5: Commit**

```bash
git add scripts/pf_table_builder/kappa_sampling.py \
        tests/pf_table_builder/test_kappa_sampling.py
git commit -m "feat(pf-gen): CO2 sweep axis in line-by-line kappa sampler
...
**Files:**
- Modify: `scripts/generate_pf_tables_linepyline.py:36-101, 123-244`
- Test: manual dry-run (full run is Task 9).

- [ ] **Step 1: Add the `earth_hifi` scenario**

In `scripts/generate_pf_tables_linepyline.py`, add to the `SCENARIOS` dict (after the existing `"earth"` entry):
...
git commit -m "feat(pf-gen): earth_hifi 14-band CO2-grid scenario + build wiring
...
Rewrite `climt/_components/picket_fence/lw/kernels.py` as:
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_lw.py tests/test_picket_fence_sw.py -v`
Expected: PASS (the per-band-sum and isothermal tests rely on the kernel; they must still pass).

- [ ] **Step 6: Commit**

```bash
git add climt/_components/picket_fence/lw/kernels.py \
        tests/test_lw_kernel_consolidation.py
git commit -m "perf(pf): consolidate LW transport into single njit(parallel) kernel
...
**Files:**
- Create: `climt/_data/picket_fence/correlated_k/earth_low_res_lw.nc` (regenerated in place)
...
cp climt/_data/picket_fence/correlated_k/earth_low_res_lw.nc \
...
conda run -n linepyline python scripts/generate_pf_tables_linepyline.py \
    ...
    --output climt/_data/picket_fence/correlated_k/earth_hifi_lw.nc \
...
from climt._components.picket_fence.optics.correlated_k import load_k_table
t = load_k_table('climt/_data/picket_fence/correlated_k/earth_hifi_lw.nc')
...
```bash
git add climt/_data/picket_fence/correlated_k/earth_hifi_lw.nc \
        climt/_data/picket_fence/correlated_k/earth_low_res_lw_4band_ngpt2_before.nc
git commit -m "data(pf): generated CO2-adjustable 14-band Earth-LW table (earth_hifi_lw)
...
Extend the single-column evaluator to sweep CO₂ and confirm `|LBL − PF|` holds away from 376 ppm, and verify off-node CO₂ interpolation fidelity (the A5 check that confirms log-k vs linear-k).
...
In `scripts/experiments/eval_band_structure.py`, make `pf_olr` already accepts `co2`; add an optional CLI 3rd-positional CO₂-override and print it. Change `main()` to read an optional `--co2` (parse from argv) and pass it into `pf_olr` for both moist/dry cases instead of the profile's baked CO₂:
...
Expected: per-band table prints; total `LBL-PF` within a few W/m² (target < 5, matching RRTMG's own LBL agreement of 4.1 moist / 7.3 dry). Repeat with `co2=0.00112` (1120 ppm) and confirm the residual stays bounded.
...
# Reuse forward_profile.npz + eval_band_structure.pf_olr for the PF side.
# For the LBL side, call scripts/pf_table_builder/kappa_sampling.sample_kappa_grid
# at the single (profile) column with the exact CO2, k-distribute on the table's
# band edges, run lw_transport, and compare OLR.
# Print: CO2(ppm) | PF_interp_OLR | LBL_OLR | diff, for log-k and linear-k
# (toggle correlated_k._CO2_INTERP_LOGK) to confirm log-k is the better choice.
...
```

Implement it concretely by mirroring `eval_band_structure.pf_olr` for the PF side and `pf_table_builder` for the LBL side; run:
...
Use the existing RCE drivers `scripts/experiments/rce_moist_pf_vs_rrtmg.py` and `rce_dry_pf_vs_rrtmg.py`, pointing them at `earth_hifi_lw` and setting `mole_fraction_of_carbon_dioxide_in_air` to 280 ppm and 1120 ppm. Confirm converged surface-temperature bias vs RRTMG is a few K (design target: moist +2.7 K, dry +1.0 K at the reference CO₂). Also run the **true baseline** with the actual current default (`earth_low_res_lw`, ngpt=2) for the honest "before" number.

```bash
conda run -n climt python scripts/experiments/rce_moist_pf_vs_rrtmg.py --table earth_hifi_lw --co2 280e-6
conda run -n climt python scripts/experiments/rce_moist_pf_vs_rrtmg.py --table earth_hifi_lw --co2 1120e-6
conda run -n climt python scripts/experiments/rce_dry_pf_vs_rrtmg.py   --table earth_hifi_lw --co2 280e-6
# True baseline:
conda run -n climt python scripts/experiments/rce_moist_pf_vs_rrtmg.py --table earth_low_res_lw
```

(If those drivers don't yet accept `--table`/`--co2`, add the argparse flags — small, mechanical — and pass through to `PicketFenceLongwave(table=...)` and the state's CO₂ field.)

- [ ] **Step 4: Log results to the investigation plan**

Append a dated results block (CO₂×H₂O LBL-PF table, CO₂-interp errors, RCE biases, true baseline number) to `docs/superpowers/plans/2026-05-16-pf-co2-band-refinement.md` (the active investigation log, per project convention). Include the final log-k vs linear-k decision.
...
git add scripts/experiments/eval_band_structure.py \
        ...
        scripts/experiments/rce_dry_pf_vs_rrtmg.py \
        docs/superpowers/plans/2026-05-16-pf-co2-band-refinement.md
git commit -m "test(pf): CO2xH2O LBL fidelity + CO2-interp accuracy + RCE validation
...
**Files:**
- Modify: `climt/_data/picket_fence/correlated_k/earth_low_res_lw.nc` (replace in place)
- Modify: `climt/_data/picket_fence/correlated_k/MANIFEST.md`
- Test: `tests/test_picket_fence_lw.py` (add shipped-table test)

- [ ] **Step 1: Replace the shipped default in place**

```bash
cp climt/_data/picket_fence/correlated_k/earth_hifi_lw.nc \
...
def test_shipped_earth_lw_is_co2_adjustable():
    ...
    sympl.set_backend(sympl.DataArrayBackend())
    lw = PicketFenceLongwave(optics="correlated_k", table="earth_low_res_lw")
...
- [ ] **Step 3: Run the integration test + full PF suite**

Run: `conda run -n climt python -m pytest tests/test_picket_fence_lw.py tests/test_picket_fence_co2_interp.py tests/test_lw_kernel_consolidation.py tests/pf_table_builder/ -v`
...
In `climt/_data/picket_fence/correlated_k/MANIFEST.md`, update the `earth_low_res_lw` entry: 14 bands, ngpt=8, decoupled+log-X H₂O continuum, runtime CO₂ axis 10–10000 ppm (10 nodes), source `linepyline:earth_hifi`. Note `earth_low_res_lw_4band_ngpt2_before.nc` and `earth_low_res_lw_8gpt.nc` as preserved "before" references.
...
```bash
git add climt/_data/picket_fence/correlated_k/earth_low_res_lw.nc \
        climt/_data/picket_fence/correlated_k/MANIFEST.md \
        tests/test_picket_fence_lw.py graphify-out/
git commit -m "feat(pf): make CO2-adjustable 14-band table the default earth_low_res_lw
