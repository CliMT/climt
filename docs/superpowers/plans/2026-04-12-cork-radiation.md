# Cork Radiation Scheme Implementation Plan
...
climt/
  _components/
    cork/
      __init__.py              # Exports CorkLongwaveRadiation, CorkShortwaveRadiation
      common.py                # Shared: heating rate, flux reduction, column amount
      lw/
        __init__.py            # (empty, for package)
        component.py           # CorkLongwaveRadiation(TendencyComponent)
        kernels.py             # _lw_transport numba kernel
      sw/
        __init__.py            # (empty, for package)
        component.py           # CorkShortwaveRadiation(TendencyComponent)
...
tests/
  test_cork_lw.py      # LW component tests
  test_cork_sw.py      # SW component tests
  test_cork_kernels.py # Kernel-level unit tests
  test_cork_optics.py  # Gas optics function tests
```
...
**Files:**
- Create: `climt/_components/cork/__init__.py`
- Create: `climt/_components/cork/common.py`
- Create: `climt/_components/cork/lw/__init__.py`
- Create: `climt/_components/cork/sw/__init__.py`
- Create: `climt/_components/cork/optics/__init__.py`

- [x] **Step 1: Create the package structure**

```python
# climt/_components/cork/__init__.py
from .lw.component import CorkLongwaveRadiation
from .sw.component import CorkShortwaveRadiation

__all__ = ["CorkLongwaveRadiation", "CorkShortwaveRadiation"]
```

```python
# climt/_components/cork/lw/__init__.py
# (empty)
```

```python
# climt/_components/cork/sw/__init__.py
# (empty)
```

```python
# climt/_components/cork/optics/__init__.py
# (empty)
```
...
Run: `cd /Users/joymonteiro/github/climt && python -c "from climt._components.cork.common import compute_heating_rate, compute_column_amount; print('OK')"`
Expected: `OK`
...
```bash
git add climt/_components/cork/
git commit -m "feat(cork): add package skeleton and shared utilities"
```
...
**Files:**
- Create: `climt/_components/cork/lw/kernels.py`
- Create: `tests/test_cork_kernels.py`

- [x] **Step 1: Write the failing test for the LW kernel**

```python
# tests/test_cork_kernels.py
import numpy as np
import pytest
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_kernels.py -v`
...
```bash
git add climt/_components/cork/lw/kernels.py tests/test_cork_kernels.py
git commit -m "feat(cork): add LW transport kernel with tests"
```
...
**Files:**
- Create: `climt/_components/cork/optics/parmentier.py`
- Create: `climt/_data/cork/parmentier/solar_composition.npz`
- Create: `tests/test_cork_optics.py`

- [x] **Step 1: Write the failing test**

```python
# tests/test_cork_optics.py
import numpy as np
import pytest
...
def test_parmentier_coefficients_grey_limit():
    ...
    from climt._components.cork.optics.parmentier import compute_thermal_opacities
...
def test_parmentier_lookup_coefficients():
    ...
    from climt._components.cork.optics.parmentier import load_parmentier_coefficients, lookup_ratio_coefficients
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_optics.py -v`
...
data_dir = os.path.join(os.path.dirname(__file__), "climt", "_data", "cork", "parmentier")
...
```python
# climt/_components/cork/optics/parmentier.py
import os
import numpy as np
...
def load_parmentier_coefficients(name_or_path):
    ...
    data_path = importlib_resources.files("climt._data.cork.parmentier").joinpath(
        f"{name_or_path}.npz"
    )
...
```bash
mkdir -p climt/_data/cork/parmentier
mkdir -p climt/_data/cork/correlated_k
mkdir -p climt/_data/cork/stellar_spectra
touch climt/_data/cork/__init__.py
touch climt/_data/cork/parmentier/__init__.py
touch climt/_data/cork/correlated_k/__init__.py
touch climt/_data/cork/stellar_spectra/__init__.py
```
...
```bash
git add climt/_components/cork/optics/parmentier.py \
       climt/_data/cork/ \
       tests/test_cork_optics.py
git commit -m "feat(cork): add Parmentier gas optics with coefficient lookup"
```

---

### Task 4: CorkLongwaveRadiation component (Parmentier mode)

**Files:**
- Create: `climt/_components/cork/lw/component.py`
- Modify: `climt/_components/__init__.py`
- Create: `tests/test_cork_lw.py`

- [x] **Step 1: Write the failing test**

```python
# tests/test_cork_lw.py
import numpy as np
import pytest
import sympl


def test_cork_lw_parmentier_runs():
    """CorkLongwaveRadiation in Parmentier mode produces non-zero fluxes."""
...
def test_cork_lw_isothermal_equilibrium():
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_lw.py -v`
Expected: FAIL with `ImportError`

- [x] **Step 3: Implement CorkLongwaveRadiation**

```python
# climt/_components/cork/lw/component.py
import numpy as np
...
class CorkLongwaveRadiation(TendencyComponent):
    ...
    def __init__(self, optics="parmentier", table=None, coefficients="solar_composition",
        ...
        super(CorkLongwaveRadiation, self).__init__(**kwargs)
...
Note: `CorkShortwaveRadiation` doesn't exist yet, so temporarily import only `CorkLongwaveRadiation` or create a stub. To avoid breaking imports, update `climt/_components/cork/__init__.py` to handle the missing SW gracefully:

```python
# climt/_components/cork/__init__.py
from .lw.component import CorkLongwaveRadiation

__all__ = ["CorkLongwaveRadiation"]
```

Then in `climt/_components/__init__.py`:

```python
from .cork import CorkLongwaveRadiation
```

And add `CorkLongwaveRadiation` to `__all__`.
...
```bash
git add climt/_components/cork/lw/component.py \
       climt/_components/cork/__init__.py \
       climt/_components/__init__.py \
       tests/test_cork_lw.py
git commit -m "feat(cork): add CorkLongwaveRadiation component with Parmentier mode"
```
...
**Files:**
- Create: `climt/_components/cork/optics/correlated_k.py`
- Create: `climt/_data/cork/correlated_k/test_2band_lw.npz`
- Modify: `tests/test_cork_optics.py`

- [x] **Step 1: Write the failing test**

Append to `tests/test_cork_optics.py`:
...
def test_correlated_k_optical_depth():
    ...
    from climt._components.cork.optics.correlated_k import load_k_table, compute_ck_optical_depth
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_optics.py::test_correlated_k_load_table -v`
Expected: FAIL with `ImportError`
...
def load_k_table(name_or_path):
    ...
    data_path = importlib_resources.files("climt._data.cork.correlated_k").joinpath(
        f"{name_or_path}.npz"
    )
    with importlib_resources.as_file(data_path) as f:
...
git add climt/_components/cork/optics/correlated_k.py \
       climt/_data/cork/correlated_k/test_2band_lw.npz \
       tests/test_cork_optics.py
git commit -m "feat(cork): add correlated-k gas optics with table loading and interpolation"
```

---

### Task 6: CorkLongwaveRadiation correlated-k mode

**Files:**
- Modify: `climt/_components/cork/lw/component.py`
- Modify: `tests/test_cork_lw.py`
...
```python
def test_cork_lw_correlated_k_runs():
    """CorkLongwaveRadiation in correlated-k mode produces non-zero fluxes."""
    from climt import get_grid, get_default_state
    from climt._components.cork import CorkLongwaveRadiation

    sympl.set_backend(sympl.DataArrayBackend())

    lw = CorkLongwaveRadiation(optics="correlated_k", table="test_2band_lw")
...
def test_cork_lw_correlated_k_per_band_sum():
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_lw.py::test_cork_lw_correlated_k_runs -v`
Expected: FAIL with `NotImplementedError`

- [x] **Step 3: Add correlated-k mode to CorkLongwaveRadiation**
...
git commit -m "feat(cork): add correlated-k mode to CorkLongwaveRadiation"
```
...
**Files:**
- Create: `climt/_components/cork/sw/kernels.py`
- Modify: `tests/test_cork_kernels.py`

- [x] **Step 1: Write the failing test**

Append to `tests/test_cork_kernels.py`:
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_kernels.py::test_sw_two_stream_no_atmosphere -v`
Expected: FAIL with `ImportError`
...
git commit -m "feat(cork): add SW direct-beam kernel with tests"
```

---

### Task 8: CorkShortwaveRadiation component

**Files:**
- Create: `climt/_components/cork/sw/component.py`
- Modify: `climt/_components/cork/__init__.py`
- Modify: `climt/_components/__init__.py`
- Create: `tests/test_cork_sw.py`

- [x] **Step 1: Write the failing test**

```python
# tests/test_cork_sw.py
import numpy as np
import pytest
import sympl


def test_cork_sw_parmentier_runs():
    """CorkShortwaveRadiation in Parmentier mode produces non-zero fluxes."""
    from climt import get_grid, get_default_state
...
def test_cork_sw_nighttime_zero():
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_sw.py -v`
Expected: FAIL with `ImportError`

- [x] **Step 3: Implement CorkShortwaveRadiation**

The structure mirrors `CorkLongwaveRadiation` but uses `sw_two_stream` kernel and handles solar flux, zenith angle, and albedo. The Parmentier SW optics uses 3 visible bands with opacities gamma_vi * kappa_R.
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_sw.py tests/test_cork_lw.py tests/test_cork_kernels.py tests/test_cork_optics.py -v`
Expected: All tests PASS

- [x] **Step 6: Commit**

```bash
git add climt/_components/cork/sw/component.py \
       climt/_components/cork/__init__.py \
       climt/_components/__init__.py \
       tests/test_cork_sw.py
git commit -m "feat(cork): add CorkShortwaveRadiation component with Parmentier mode"
```
...
git commit -m "feat(cork): add irradiation/internal temperature to default_values"
```
...
**Files:**
- Create: `tests/test_cork_integration.py`
...
Run: `cd /Users/joymonteiro/github/climt && python -m pytest tests/test_cork_integration.py -v`
Expected: PASS
...
git commit -m "test(cork): add integration tests for combined LW+SW and cloud hook"
```
...
| 4 | CorkLongwaveRadiation (Parmentier) | 3 component tests |
| 5 | Correlated-k gas optics | 3 optics tests |
| 6 | CorkLongwaveRadiation (correlated-k) | 2 component tests |
| 7 | SW two-stream kernel | 2 kernel tests |
| 8 | CorkShortwaveRadiation (Parmentier) | 3 component tests |
