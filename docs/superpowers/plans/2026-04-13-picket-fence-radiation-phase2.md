# Picket-Fence Radiation Phase 2: Two-Stream, ESFT, and Correlated-k SW

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Complete the picket-fence radiation scheme by adding the full Meador & Weaver two-stream SW solver, ESFT gas overlap for correlated-k mode, correlated-k SW mode, and stellar spectrum loading from data files.
...
tests/
  test_picket_fence_kernels.py # MODIFY: add two-stream diffuse tests
  test_picket_fence_optics.py  # MODIFY: add ESFT overlap tests
  test_picket_fence_sw.py      # MODIFY: add correlated-k SW tests
  test_picket_fence_stellar.py # CREATE: stellar spectrum loading tests
```
...
Add these tests to `tests/test_picket_fence_kernels.py`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_picket_fence_kernels.py::test_sw_two_stream_conservative_scattering tests/test_picket_fence_kernels.py::test_sw_two_stream_forward_scattering tests/test_picket_fence_kernels.py::test_sw_two_stream_albedo_reflection -v`
...
Replace the contents of `climt/_components/picket_fence/sw/kernels.py` with:
...
- [x] **Step 5: Run full picket-fence test suite to check for regressions**

Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_picket_fence_sw.py tests/test_picket_fence_integration.py -v`
Expected: All PASS
...
git commit -m "feat(picket-fence): replace direct-beam SW with Meador & Weaver two-stream solver"
```
...
**Files:**
- Modify: `climt/_components/picket_fence/optics/correlated_k.py`
...
Add these tests to `tests/test_picket_fence_optics.py`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_picket_fence_optics.py::test_esft_single_gas_matches_additive tests/test_picket_fence_optics.py::test_esft_two_gas_weight_sum tests/test_picket_fence_optics.py::test_esft_two_gas_optical_depth -v`
...
In `climt/_components/picket_fence/lw/component.py`, the correlated-k path calls `compute_ck_optical_depth` and uses `self._table["gpoint_weights"]`. For ESFT, the function now returns a tuple. Update the correlated-k branch in `array_call`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_picket_fence_optics.py tests/test_picket_fence_lw.py -v`
Expected: All PASS

- [x] **Step 6: Commit**

```bash
git add climt/_components/picket_fence/optics/correlated_k.py climt/_components/picket_fence/lw/component.py tests/test_picket_fence_optics.py
git commit -m "feat(picket-fence): add ESFT gas overlap for correlated-k mode"
```
...
**Files:**
- Create: `climt/_components/picket_fence/optics/stellar.py`
- Create: `climt/_data/picket_fence/stellar_spectra/sun.npz`
- Create: `tests/test_picket_fence_stellar.py`
...
```python
# Script to generate climt/_data/picket_fence/stellar_spectra/sun.npz
...
np.savez(
    "climt/_data/picket_fence/stellar_spectra/sun.npz",
...
```python
# tests/test_picket_fence_stellar.py
import numpy as np
import pytest
...
```python
# climt/_components/picket_fence/optics/stellar.py
import os
...
```bash
git add climt/_components/picket_fence/optics/stellar.py climt/_data/picket_fence/stellar_spectra/sun.npz tests/test_picket_fence_stellar.py
git commit -m "feat(picket-fence): add stellar spectrum loading and band integration"
```
...
**Files:**
- Create: `climt/_data/picket_fence/correlated_k/test_2band_sw.npz`
- Modify: `tests/test_picket_fence_optics.py`
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_picket_fence_optics.py::test_correlated_k_sw_table_loads -v`
...
np.savez(
    "climt/_data/picket_fence/correlated_k/test_2band_sw.npz",
    k_coefficients=k_data,
...
```bash
git add climt/_data/picket_fence/correlated_k/test_2band_sw.npz tests/test_picket_fence_optics.py
git commit -m "test(picket-fence): add SW correlated-k test table"
```

---

### Task 5: PicketFenceShortwave correlated-k mode
...
git commit -m "feat(picket-fence): add correlated-k mode to PicketFenceShortwave"
```
...
```bash
git add climt/_components/picket_fence/sw/component.py tests/test_picket_fence_sw.py
git commit -m "feat(picket-fence): wire stellar spectrum loading into SW component"
```
...
git commit -m "test(picket-fence): add phase 2 integration tests for two-stream and correlated-k SW"
```
...
| 5 | PicketFenceShortwave correlated-k mode | 3 component tests |
