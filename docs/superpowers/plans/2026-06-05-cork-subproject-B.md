# Sub-project B Implementation Plan — Cork LW Discovery Post (Quarto)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship the cork longwave "fast vs faithful" discovery story as the first Quarto Experiments post (`docs/experiments/2026-06-05-cork-co2-bands/`), with a runnable companion notebook, a robust tropopause helper, and the minimal forward-compatible Quarto/artifact scaffolding to render and regenerate it.
...
│   └── rce_moist_cork_vs_rrtmg.py          # MODIFY: add --save <npz>
│   └── rce_dry_cork_vs_rrtmg.py            # MODIFY: add --save <npz>
│   └── bench_cork_vs_rrtmg.py              # MODIFY: add --save <npz>
...
git commit -m "feat(cork): robust theta-curvature tropopause locator + tests"
```
...
```bibtex
% references.bib -- only entries cited by the cork post. Each carries a
...
**Files:**
- Modify: `scripts/experiments/rce_moist_cork_vs_rrtmg.py`
- Modify: `scripts/experiments/rce_dry_cork_vs_rrtmg.py`
- Modify: `scripts/experiments/bench_cork_vs_rrtmg.py`
...
Apply the identical `--save` argparse line and the identical save block to `rce_dry_cork_vs_rrtmg.py` (it has the same `COLUMNS`/`history`/state-dict shape). Repeat the code rather than importing across scripts — they are standalone experiment scripts.

- [ ] **Step 3: `bench` — add `--save` writing the two throughput scalars**

In `bench_cork_vs_rrtmg.py`'s `main()`, after the two timing prints (`mean_r`, `mean_p` known), add an argparse `--save` and:
...
Run: `conda run --no-capture-output -n climt python scripts/experiments/rce_moist_cork_vs_rrtmg.py --help`
Expected: `--save` appears in the help. Repeat `--help` for `rce_dry`, `bench`, and confirm `--out` for `plot_kg_curves`.

- [ ] **Step 6: Commit**

```bash
git add scripts/experiments/rce_moist_cork_vs_rrtmg.py scripts/experiments/rce_dry_cork_vs_rrtmg.py scripts/experiments/bench_cork_vs_rrtmg.py scripts/experiments/plot_kg_curves.py
git commit -m "feat(cork): add --save/--out hooks to RCE/bench/kg scripts for figure pipeline"
```
...
git commit -m "feat(cork): single figure generator for the discovery post"
```
...
**Files:**
- Create: `docs/experiments/2026-06-05-cork-co2-bands/sources.yml`
- Generated: `docs/experiments/2026-06-05-cork-co2-bands/_artifacts/*` (figures, captured stdout, `.hashes.json`)
...
artifacts:
  ...
  - out: _artifacts/rce_moist_before.npz
    cmd: conda run --no-capture-output -n climt python scripts/experiments/rce_moist_cork_vs_rrtmg.py --table earth_low_res_lw_4band_ngpt2_before.nc --days 200 --save docs/experiments/2026-06-05-cork-co2-bands/_artifacts/rce_moist_before.npz
    deps:
  ...
  - out: _artifacts/rce_moist_after.npz
    cmd: conda run --no-capture-output -n climt python scripts/experiments/rce_moist_cork_vs_rrtmg.py --table earth_low_res_lw.nc --days 200 --save docs/experiments/2026-06-05-cork-co2-bands/_artifacts/rce_moist_after.npz
    deps:
      - scripts/experiments/rce_moist_cork_vs_rrtmg.py
      - climt/_components/cork/**/*.py
  ...
  - out: _artifacts/kg_window_band.png
    cmd: conda run --no-capture-output -n climt python scripts/experiments/make_subproject_B_figures.py --figure kg --out docs/experiments/2026-06-05-cork-co2-bands/_artifacts/kg_window_band.png
    deps:
      - scripts/experiments/make_subproject_B_figures.py
...
`ls docs/experiments/2026-06-05-cork-co2-bands/_artifacts/` shows `kg_window_band.png lbl_overlay.png rce_before_after.png throughput.png throughput.npz throughput.txt rce_moist_before.npz rce_moist_after.npz .hashes.json`.
...
cp debug_data/lbl_olr_spec_moist_co2_400ppm.npz debug_data/lbl_olr_spec_dry_co2_400ppm.npz \
   docs/experiments/2026-06-05-cork-co2-bands/_artifacts/
```
...
```bash
git add docs/experiments/2026-06-05-cork-co2-bands/sources.yml docs/experiments/2026-06-05-cork-co2-bands/_artifacts/
git commit -m "feat(cork): experiment sources.yml + regenerated hero artifacts"
```
...
**Files:**
- Create: `docs/experiments/2026-06-05-cork-co2-bands/cork_co2_bands.ipynb`
...
2. **py — setup.** Imports; `sys.path.insert` for `tropopause`; define `ARTIF = "_artifacts"`; construct `CorkLongwaveRadiation(table="earth_low_res_lw")` and the "before" `CorkLongwaveRadiation(table="earth_low_res_lw_4band_ngpt2_before.nc")`; `RRTMG_AVAILABLE` try/except around `from climt import RRTMGLongwave`.
3. **md** — "The symptom."
...
8. **py — LBL overlay.** Load `_artifacts/lbl_olr_spec_moist_co2_400ppm.npz`; overlay CORK vs RRTMG scalar OLR on the LBL spectrum; print the three OLR numbers.
9. **md** — "The dead-ends."
...
14. **py — subtlety.** Print fixed-profile single-column LBL−CORK (+11 for both) next to the converged RCE ΔT (~2 K).
15. **md** — "Two more things: fast & the free knob."
16. **py — coda.** Load `_artifacts/throughput.npz` (CORK vs RRTMG µs/col); a CO₂-sweep cell calling CORK at a few CO₂ values (pure-Python, Pyodide-live).
17. **md** — "Pyodide note": which cells above are pure-Python-live (6, 16 CO₂ sweep, CORK construction) vs which need pre-baked `.npz` (4, 8, 12, 14). This is the cell-boundary map the live-cells follow-up consumes.

- [ ] **Step 2: Execute the notebook end-to-end in the climt env**

Run:
```bash
cd docs/experiments/2026-06-05-cork-co2-bands
conda run --no-capture-output -n climt jupyter nbconvert --to notebook --execute --inplace cork_co2_bands.ipynb
```
...
```bash
git add docs/experiments/2026-06-05-cork-co2-bands/cork_co2_bands.ipynb
git commit -m "feat(cork): companion notebook (reproduces every post figure)"
```
...
**Files:**
- Create: `docs/experiments/2026-06-05-cork-co2-bands/index.qmd`
...
description: "A cork correlated-k warm bias, root-caused and fixed."
date: 2026-06-05
author: "Joy Monteiro"
...
3. **Isolating the culprit** — linepyline matches RRTMG; CORK over-traps ~17 (dry)/~36 (moist) W/m². Embed `_artifacts/lbl_overlay.png`.
...
7. **The payoff + diagnostic-trust lesson** — before→after table (T_sfc +11.9→+1.9–2.1 K, dry +0.8 K, q 2.2 vs 2.0, tropopause 583→196 hPa). Embed `_artifacts/rce_before_after.png` (full). `callout-note`: fixed-profile LBL−CORK reads +11 for both prototype and shipped table yet RCE converges to ~2 K — the fixed-profile metric overstates a self-adjusting column's error.
...
```bash
git add docs/experiments/index.qmd docs/experiments/2026-06-05-cork-co2-bands/index.qmd references.bib
git commit -m "docs(cork): the cork discovery post (first Experiments post)"
```
...
```markdown
- The cork Experiments post (`docs/experiments/2026-06-05-cork-co2-bands/`)
...
git commit -m "docs(cork): cross-link pyodide spec + refresh knowledge graph"
```
