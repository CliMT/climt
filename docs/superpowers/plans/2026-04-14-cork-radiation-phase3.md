# Cork Radiation Phase 3: Pedagogical Features, Clouds, Multi-Planet, and Validation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Complete the cork radiation scheme by adding pedagogical introspection (diagnostics levels, component-level per-band diagnostics), SW cloud inputs, multi-planet atmospheric properties (TOML profiles), and validation against analytical solutions and RRTMG.
...
tests/
  test_cork_kernels.py # MODIFY: add diagnostics_level tests
  test_cork_sw.py      # MODIFY: add SW cloud tests
  test_atmospheric_properties.py # CREATE: TOML loading tests
  test_cork_validation.py # CREATE: validation against analytical solutions
```
...
Add to `tests/test_cork_kernels.py`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_kernels.py::test_sw_two_stream_diagnostics_level0_unchanged tests/test_cork_kernels.py::test_sw_two_stream_diagnostics_level1 tests/test_cork_kernels.py::test_sw_two_stream_diagnostics_level2 -v`
...
In `climt/_components/cork/sw/kernels.py`, rename the existing `sw_two_stream` function to `_sw_two_stream_core` (keep the `@njit` decorator and all existing code unchanged). Then add a new `sw_two_stream` wrapper:
...
git commit -m "feat(cork): add diagnostics_level to SW two-stream kernel"
```
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_kernels.py::test_lw_transport_diagnostics_level0_unchanged tests/test_cork_kernels.py::test_lw_transport_diagnostics_level1 -v`
Expected: FAIL

- [x] **Step 3: Implement diagnostics_level in lw_transport**

Same pattern as SW: keep `_lw_transport_single_gpt` jitted, make `lw_transport` a plain-Python function that optionally collects intermediate arrays. In `climt/_components/cork/lw/kernels.py`:
...
git commit -m "feat(cork): add diagnostics_level to LW transport kernel"
```
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_lw.py::test_lw_per_band_optical_depth_diagnostics tests/test_cork_sw.py::test_sw_per_band_optical_depth_diagnostics -v`
Expected: FAIL (diagnostic keys not present)

- [x] **Step 4: Implement LW per-band diagnostics**

In `climt/_components/cork/lw/component.py`:
...
Same pattern in `climt/_components/cork/sw/component.py`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_lw.py tests/test_cork_sw.py -v`
Expected: ALL PASS

- [x] **Step 7: Commit**

```bash
git add climt/_components/cork/lw/component.py climt/_components/cork/sw/component.py tests/test_cork_lw.py tests/test_cork_sw.py
git commit -m "feat(cork): add per-band optical depth, transmittance, and heating rate diagnostics"
```
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_sw.py::test_sw_parmentier_cloud_increases_reflection -v`
...
In `climt/_components/cork/sw/component.py`, add to the `input_properties` property (after the existing properties):
...
- [x] **Step 5: Run full cork test suite for regressions**

Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_*.py -v`
Expected: ALL PASS
...
git commit -m "feat(cork): add SW cloud optical property inputs (tau, ssa, g)"
```
...
**Files:**
- Create: `tests/test_cork_validation.py`

These tests verify physics accuracy rather than code correctness. They compare cork results against known analytical solutions and (where RRTMG is available) against the full correlated-k scheme.
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_validation.py -v`
Expected: ALL PASS (analytical tests should pass; RRTMG tests skipped if not available)

- [x] **Step 3: Commit**

```bash
git add tests/test_cork_validation.py
git commit -m "test(cork): add validation tests against analytical LW/SW solutions"
```
...
def test_sw_component_diagnostics_level(get_default_state_sw):
    ...
    sw = CorkShortwaveRadiation(optics="parmentier", diagnostics_level=1)
    state = get_default_state_sw(sw)
    state["zenith_angle"].values[:] = np.pi / 4
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_sw.py::test_sw_component_diagnostics_level -v`
Expected: FAIL (CorkShortwaveRadiation does not accept diagnostics_level)

- [x] **Step 3: Wire diagnostics_level through SW component**

In `CorkShortwaveRadiation.__init__`, add:
...
Apply the same pattern to `CorkLongwaveRadiation`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_sw.py tests/test_cork_lw.py -v`
Expected: ALL PASS

- [x] **Step 6: Commit**

```bash
git add climt/_components/cork/sw/component.py climt/_components/cork/lw/component.py tests/test_cork_sw.py
git commit -m "feat(cork): wire diagnostics_level through LW and SW components"
```
...
- [x] **Step 1: Run all cork tests**

Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_*.py tests/test_atmospheric_properties.py -v`
Expected: ALL PASS
...
Expected: ALL PASS (or only pre-existing failures unrelated to cork)
