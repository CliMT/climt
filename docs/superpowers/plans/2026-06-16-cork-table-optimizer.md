...
- All `pytest` and Cork steps run in the **climt** env (per project convention).
...
New package `scripts/cork_optimizer/`:
...
- `table_build.py` — (numpy) cached cubes + band_edges + decouple flag → temp `.nc` table. Reuses `cork_table_builder`.
...
- `evaluate.py` — (climt) run CorkLongwaveRadiation on validation columns → residuals vs cached truth.
...
Driver: `scripts/optimize_cork_table.py` (CLI).

Reused (refactored in Task 1): `scripts/generate_cork_tables_linepyline.py`,
`scripts/cork_table_builder/{kappa_sampling,k_distribution,planck_fraction,netcdf_writer}.py`.

Tests: `tests/cork_optimizer/test_*.py`.
...
`generate_cork_tables_linepyline.build_table`. Extract that numpy-only tail into a
standalone function so both the CLI and the optimizer share it.

**Files:**
- Modify: `scripts/generate_cork_tables_linepyline.py`
- Create: `tests/cork_optimizer/__init__.py`
- Test: `tests/cork_optimizer/test_assemble_lw_table.py`

- [ ] **Step 1: Write the failing test**

```python
# tests/cork_optimizer/test_assemble_lw_table.py
import os, sys
import numpy as np
import xarray as xr
...
from generate_cork_tables_linepyline import assemble_lw_table
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_assemble_lw_table.py -v`
Expected: FAIL — `ImportError: cannot import name 'assemble_lw_table'`.

- [ ] **Step 3: Extract `assemble_lw_table` from `build_table`**

In `scripts/generate_cork_tables_linepyline.py`, add this function (it lifts the
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_table_builder -q`
Expected: PASS (existing builder tests unchanged).

- [ ] **Step 6: Commit**

```bash
git add scripts/generate_cork_tables_linepyline.py tests/cork_optimizer/__init__.py tests/cork_optimizer/test_assemble_lw_table.py
git commit -m "refactor(cork): extract assemble_lw_table for optimizer reuse
...
**Files:**
- Create: `scripts/cork_optimizer/__init__.py` (empty)
- Create: `scripts/cork_optimizer/config.py`
- Test: `tests/cork_optimizer/test_config.py`

- [ ] **Step 1: Write the failing test**

```python
# tests/cork_optimizer/test_config.py
import os, sys
import numpy as np
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_REPO, "scripts"))
from cork_optimizer.config import OptimizerConfig, EARTH_LW
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_config.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'cork_optimizer'`.

- [ ] **Step 3: Implement config**

```python
# scripts/cork_optimizer/config.py
...
```bash
git add scripts/cork_optimizer/__init__.py scripts/cork_optimizer/config.py tests/cork_optimizer/test_config.py
git commit -m "feat(cork-optimizer): add OptimizerConfig + Earth-LW default
...
**Files:**
- Create: `scripts/cork_optimizer/table_build.py`
- Test: `tests/cork_optimizer/test_table_build.py`

- [ ] **Step 1: Write the failing test**

```python
# tests/cork_optimizer/test_table_build.py
import os, sys
import numpy as np
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_REPO, "scripts"))
from cork_optimizer.table_build import build_table_from_cache
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_table_build.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'cork_optimizer.table_build'`.

- [ ] **Step 3: Implement**

```python
# scripts/cork_optimizer/table_build.py
from __future__ import annotations
import os, sys
import numpy as np
...
```bash
git add scripts/cork_optimizer/table_build.py tests/cork_optimizer/test_table_build.py
git commit -m "feat(cork-optimizer): build temp table from cached kappa cube
...
**Files:**
- Create: `scripts/cork_optimizer/diagnostics.py`
- Test: `tests/cork_optimizer/test_diagnostics.py`

- [ ] **Step 1: Write the failing tests**

```python
# tests/cork_optimizer/test_diagnostics.py
import os, sys
import numpy as np
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_REPO, "scripts"))
from cork_optimizer.diagnostics import (
...
def test_worst_dry_band_picks_largest_abs_residual():
    per_band = np.array([0.5, -7.0, 2.0])      # LBL - CORK per band (dry)
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_diagnostics.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'cork_optimizer.diagnostics'`.

- [ ] **Step 3: Implement**

```python
# scripts/cork_optimizer/diagnostics.py
from __future__ import annotations
import numpy as np
...
def worst_dry_band(per_band_residual, band_edges):
    """Index of the band with the largest |LBL - CORK| dry OLR residual."""
...
```bash
git add scripts/cork_optimizer/diagnostics.py tests/cork_optimizer/test_diagnostics.py
git commit -m "feat(cork-optimizer): split metric, window detection, residual attribution
...
**Files:**
- Create: `scripts/cork_optimizer/actions.py`
- Test: `tests/cork_optimizer/test_actions.py`

- [ ] **Step 1: Write the failing tests**

```python
# tests/cork_optimizer/test_actions.py
import os, sys
import numpy as np
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_REPO, "scripts"))
from cork_optimizer.actions import choose_action, Residual, State
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_actions.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'cork_optimizer.actions'`.

- [ ] **Step 3: Implement**

```python
# scripts/cork_optimizer/actions.py
from __future__ import annotations
...
@dataclass
class Residual:
    olr_err_moist: float          # LBL - CORK, total (W/m^2)
    olr_err_dry: float
    per_band_dry: np.ndarray      # LBL - CORK per band, dry (W/m^2)
...
```bash
git add scripts/cork_optimizer/actions.py tests/cork_optimizer/test_actions.py
git commit -m "feat(cork-optimizer): choose_action brain encoding band-refinement learnings
...
## Task 6: `evaluate.py` — Cork on validation columns → residuals

**Files:**
- Create: `scripts/cork_optimizer/evaluate.py`
- Test: `tests/cork_optimizer/test_evaluate.py`

`evaluate.py` runs Cork on each validation column and re-bins the cached
...
```python
# tests/cork_optimizer/test_evaluate.py
import os, sys
import numpy as np
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_REPO, "scripts"))
from cork_optimizer.evaluate import rebin_lbl_per_band, residual_from_totals
...
def test_residual_from_totals():
    r = residual_from_totals(lbl_moist=250.0, cork_moist=240.0,
                             lbl_dry=350.0, cork_dry=349.0,
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_evaluate.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'cork_optimizer.evaluate'`.

- [ ] **Step 3: Implement**

```python
# scripts/cork_optimizer/evaluate.py
from __future__ import annotations
import numpy as np

from cork_optimizer.actions import Residual
...
def residual_from_totals(*, lbl_moist, cork_moist, lbl_dry, cork_dry,
                         per_band_dry, kappa_per_band, air_mass):
    return Residual(
        olr_err_moist=float(lbl_moist - cork_moist),
        olr_err_dry=float(lbl_dry - cork_dry),
        per_band_dry=np.asarray(per_band_dry, dtype=float),
        kappa_per_band=kappa_per_band, air_mass=float(air_mass),
    )


def cork_olr_on_column(table_path, p_int, T, q, Ts, co2, nz):
    """Total + per-band CORK OLR for one column. climt env only.
    ...
    cork = CorkLongwaveRadiation(optics="correlated_k", table=table_path)
    grid = get_grid(nx=1, ny=1, nz=nz)
    state = get_default_state([cork], grid_state=grid)
...
```bash
git add scripts/cork_optimizer/evaluate.py tests/cork_optimizer/test_evaluate.py
git commit -m "feat(cork-optimizer): CORK column eval + LBL re-bin + residual builder
...
**Files:**
- Create: `scripts/cork_optimizer/kappa_cache.py`
- Create: `scripts/cork_optimizer/truth.py`
- Test: `tests/cork_optimizer/test_cache_contract.py`

- [ ] **Step 1: Write the failing test (I/O contract only)**

```python
# tests/cork_optimizer/test_cache_contract.py
import os, sys
import numpy as np
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_REPO, "scripts"))
from cork_optimizer.kappa_cache import cache_path_for, save_kappa_cache
from cork_optimizer.config import EARTH_LW
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_cache_contract.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'cork_optimizer.kappa_cache'`.

- [ ] **Step 3: Implement `kappa_cache.py`**

```python
# scripts/cork_optimizer/kappa_cache.py  (RUN IN linepyline ENV)
from __future__ import annotations
import os, sys
import numpy as np
...
if __name__ == "__main__":
    ...
    #   python -m cork_optimizer.kappa_cache <cache_dir>
    import argparse
    from cork_optimizer.config import EARTH_LW
...
```python
# scripts/cork_optimizer/truth.py  (RUN IN linepyline ENV)
from __future__ import annotations
import os
import numpy as np
...
```bash
git add scripts/cork_optimizer/kappa_cache.py scripts/cork_optimizer/truth.py tests/cork_optimizer/test_cache_contract.py
git commit -m "feat(cork-optimizer): grid-keyed kappa cache + LBL truth (linepyline env)
...
calls are `build_table_from_cache`, `cork_olr_on_column`, and `choose_action`. Uses
an injected `cork_eval_fn` so the loop is testable without climt.

**Files:**
- Create: `scripts/cork_optimizer/loop.py`
- Test: `tests/cork_optimizer/test_loop.py`

- [ ] **Step 1: Write the failing test (fake eval, no climt/linepyline)**

```python
# tests/cork_optimizer/test_loop.py
import os, sys
import numpy as np
_REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
sys.path.insert(0, os.path.join(_REPO, "scripts"))
from cork_optimizer.loop import optimize
from cork_optimizer.config import OptimizerConfig


def test_loop_converges_by_splitting(tmp_path, monkeypatch):
    # Fake CORK eval: dry OLR error shrinks each time a band is added; the loop
    ...
    result = optimize(cfg, cork_eval_fn=fake_eval, log_path=str(tmp_path / "log.md"))
...
def test_loop_stalls_and_reports_best(tmp_path):
    ...
    result = optimize(cfg, cork_eval_fn=stuck_eval, log_path=str(tmp_path / "log.md"))
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_loop.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'cork_optimizer.loop'`.

- [ ] **Step 3: Implement**

```python
# scripts/cork_optimizer/loop.py
from __future__ import annotations
import numpy as np
...
def optimize(cfg, *, cork_eval_fn, log_path):
    """Greedy residual-driven band optimizer.

    cork_eval_fn(band_edges, decoupled, cfg) -> dict with keys
    ...
    best = {"score": np.inf, "band_edges": band_edges.copy(),
            "decoupled": decoupled}
    _log(log_path, [f"# CORK table optimizer run", "",
                    f"- seed bands: {band_edges.tolist()}",
                    f"- OLR target: {cfg.olr_target} W/m^2", ""])

    for it in range(cfg.max_iters):
        ev = cork_eval_fn(band_edges, decoupled, cfg)
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer -q`
Expected: PASS (all tasks' tests green).

- [ ] **Step 6: Commit**

```bash
git add scripts/cork_optimizer/loop.py tests/cork_optimizer/test_loop.py
git commit -m "feat(cork-optimizer): greedy orchestrator loop with logging + stall handling
...
## Task 9: CLI driver `scripts/optimize_cork_table.py` + real eval wiring

Wires the real climt CORK eval into the loop and provides the CLI. The real
`cork_eval_fn` builds a temp table from the κ-cache, runs CORK on each validation
...
**Files:**
- Create: `scripts/optimize_cork_table.py`
- Test: `tests/cork_optimizer/test_cli_smoke.py`

- [ ] **Step 1: Write the failing test (CLI arg parsing + dry validation)**

```python
# tests/cork_optimizer/test_cli_smoke.py
...
def test_cli_help_runs():
    p = subprocess.run(
        [sys.executable, os.path.join(_REPO, "scripts", "optimize_cork_table.py"),
...
Run: `/Users/joymonteiro/miniconda3/envs/climt/bin/python -m pytest tests/cork_optimizer/test_cli_smoke.py -v`
Expected: FAIL — file does not exist / FileNotFoundError.

- [ ] **Step 3: Implement the CLI + real eval**

```python
# scripts/optimize_cork_table.py  (RUN IN climt ENV)
"""Optimize a cork LW correlated-k band structure against LBL.

Prereqs (linepyline env, once per grid):
    python -m cork_optimizer.kappa_cache <cache_dir>
    # plus LBL truth via cork_optimizer.truth.build_lbl_truth(...)

Then (climt env):
    python scripts/optimize_cork_table.py --cache-dir debug_data/cork_opt \\
        --kappa-cache debug_data/cork_opt/kappa_cache_<hash>.npz \\
        --profile debug_data/forward_profile.npz \\
        --out climt/_data/cork/correlated_k/earth_opt_lw.nc
"""
from __future__ import annotations
...
sys.path.insert(0, _SCRIPTS)
from cork_optimizer.config import EARTH_LW
from cork_optimizer.table_build import build_table_from_cache, load_cache
from cork_optimizer.evaluate import (
    rebin_lbl_per_band, residual_from_totals, cork_olr_on_column)
from cork_optimizer.truth import truth_path_for
from cork_optimizer.loop import optimize
...
def make_cork_eval(cfg, cache, truth, profile, out_dir):
    ...
    def cork_eval(band_edges, decoupled, _cfg):
        ...
        cork_moist, pb_moist = cork_olr_on_column(tbl, p_int, T_climt, q_climt, Ts, co2, nz)
        cork_dry, pb_dry = cork_olr_on_column(tbl, p_int, T_climt, np.zeros_like(q_climt), Ts, co2, nz)
        ...
        r = residual_from_totals(
            lbl_moist=float(truth["total_moist"]), cork_moist=cork_moist,
            lbl_dry=float(truth["total_dry"]), cork_dry=cork_dry,
...
def main():
    ...
    cork_eval = make_cork_eval(cfg, cache, truth, profile, a.cache_dir)
    result = optimize(cfg, cork_eval_fn=cork_eval, log_path=log_path)
...
```bash
git add scripts/optimize_cork_table.py tests/cork_optimizer/test_cli_smoke.py
git commit -m "feat(cork-optimizer): CLI driver wiring real climt CORK eval into the loop
...
Run:
```bash
/Users/joymonteiro/miniconda3/envs/linepyline/bin/python -m cork_optimizer.kappa_cache debug_data/cork_opt
```
(Run from repo root with `PYTHONPATH=scripts`.)
Expected: prints `debug_data/cork_opt/kappa_cache_<hash>.npz`; file exists.
...
`cork_optimizer.truth.build_lbl_truth(EARTH_LW, columns, "debug_data/cork_opt")`.
Expected: `debug_data/cork_opt/lbl_truth_<hash>_forward.npz` exists with keys
...
Run:
```bash
/Users/joymonteiro/miniconda3/envs/climt/bin/python scripts/optimize_cork_table.py \
  --cache-dir debug_data/cork_opt \
  --kappa-cache debug_data/cork_opt/kappa_cache_<hash>.npz \
  --profile debug_data/forward_profile.npz \
  --truth debug_data/cork_opt/lbl_truth_<hash>_forward.npz \
  --out climt/_data/cork/correlated_k/earth_opt_lw.nc
```
...
`debug_data/cork_opt/optimizer_log.md`.
...
```
Expected: moist & dry `|LBL-CORK|` are within a few W/m² and no worse than
...
`docs/superpowers/plans/2026-05-16-cork-co2-band-refinement.md` (the active CORK
...
```bash
git add docs/superpowers/plans/2026-05-16-cork-co2-band-refinement.md
git commit -m "docs(cork-optimizer): log first end-to-end Earth-LW optimizer run
