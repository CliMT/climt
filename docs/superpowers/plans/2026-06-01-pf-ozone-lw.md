# PicketFence Longwave Ozone (O₃) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add longwave ozone absorption to `PicketFenceLongwave` as a dedicated additive gas with a precomputed `k_O3(T,p)` cross-section and a runtime-variable O₃ profile, so PF can reproduce a realistic (warm) stratosphere.
...
**Design reference:** `docs/superpowers/specs/2026-06-01-pf-ozone-lw-design.md`
...
|---|---|---|
| `scripts/pf_table_builder/kappa_sampling.py` | line-by-line κ sampling | **add** `sample_secondary_gas_kappa()` — raw per-kg-species κ(T,p,ν) |
| `scripts/pf_table_builder/k_distribution.py` | κ → k-distribution | **add** `broadcast_and_stack_gases()` — pure helper, no linepyline |
| `scripts/pf_table_builder/netcdf_writer.py` | write `.nc` schema | **add** `background_is_premixed` attribute to `write_lw_table` |
| `scripts/generate_pf_tables_linepyline.py` | scenario registry + `build_table` | **add** `lw_secondary_gases` to `earth`; stack O₃ gas in `build_table` |
| `climt/_components/picket_fence/lw/component.py` | LW component | **modify** `input_properties` + `array_call` premixed-bg branch for secondary gases |
| `tests/pf_table_builder/test_secondary_gas.py` | builder unit tests | **create** |
| `tests/test_picket_fence_optics.py` | optics unit tests | **add** 6-D × ngas=2 additive regression test |
| `tests/test_picket_fence_lw.py` | component tests | **add** O₃ on/off behaviour tests |
| `climt/_data/picket_fence/correlated_k/earth_low_res_lw_co2refined_o3.nc` | shipped table | **create** (Task 8, linepyline env) |
| `scripts/experiments/radiative_equilibrium_pf_o3.py` | RRTMG validation | **create** (Task 9) |

---

## Task 1: Writer — `background_is_premixed` attribute

The component decides premixed-background mode from `gas_names == ["effective"]` **or** a `background_is_premixed="true"` netCDF attribute (`climt/_components/picket_fence/lw/component.py:47-51`). A 2-gas table (`["effective","o3"]`) fails the first test, so the writer must emit the attribute. The writer (`scripts/pf_table_builder/netcdf_writer.py:14`) does not currently write it.

**Files:**
- Modify: `scripts/pf_table_builder/netcdf_writer.py:14-84`
- Test: `tests/pf_table_builder/test_netcdf_writer.py`

- [ ] **Step 1: Write the failing test**

Add to `tests/pf_table_builder/test_netcdf_writer.py`:

```python
def test_write_lw_table_background_is_premixed_attr(tmp_path):
    import numpy as np
    from scipy.io import netcdf_file
    from pf_table_builder.netcdf_writer import write_lw_table
...
Run: `conda run -n climt python -m pytest tests/pf_table_builder/test_netcdf_writer.py::test_write_lw_table_background_is_premixed_attr -v`
...
In `scripts/pf_table_builder/netcdf_writer.py`, add `background_is_premixed=False,` to the `write_lw_table` signature (after `gas_names=("effective",),`). Then, immediately after the line `nc.gas_names = ",".join(gas_names)`:
...
```bash
git add scripts/pf_table_builder/netcdf_writer.py tests/pf_table_builder/test_netcdf_writer.py
git commit -m "feat(pf-tables): write background_is_premixed attr in LW table"
```
...
**Files:**
- Modify: `scripts/pf_table_builder/k_distribution.py` (append function)
- Test: `tests/pf_table_builder/test_secondary_gas.py` (create)

- [ ] **Step 1: Write the failing test**

Create `tests/pf_table_builder/test_secondary_gas.py`:

```python
import numpy as np
from pf_table_builder.k_distribution import broadcast_and_stack_gases
...
Run: `conda run -n climt python -m pytest tests/pf_table_builder/test_secondary_gas.py -v`
Expected: FAIL — `ImportError: cannot import name 'broadcast_and_stack_gases'`.

- [ ] **Step 3: Implement the helper**

Append to `scripts/pf_table_builder/k_distribution.py`:
...
```bash
git add scripts/pf_table_builder/k_distribution.py tests/pf_table_builder/test_secondary_gas.py
git commit -m "feat(pf-tables): add broadcast_and_stack_gases helper"
```
...
**Files:**
- Modify: `scripts/pf_table_builder/kappa_sampling.py` (append function)
- Test: `tests/pf_table_builder/test_secondary_gas.py`

- [ ] **Step 1: Write the failing test (linepyline-gated)**

Append to `tests/pf_table_builder/test_secondary_gas.py`:
...
def test_sample_secondary_gas_kappa_o3_shape_and_positive():
    ...
    from pf_table_builder.kappa_sampling import sample_secondary_gas_kappa
...
Run: `conda run -n climt python -m pytest tests/pf_table_builder/test_secondary_gas.py::test_sample_secondary_gas_kappa_o3_shape_and_positive -v`
...
Append to `scripts/pf_table_builder/kappa_sampling.py` (it already imports numpy as np at module top and imports `linepyline` lazily inside `sample_kappa_grid`):
...
```bash
git add scripts/pf_table_builder/kappa_sampling.py tests/pf_table_builder/test_secondary_gas.py
git commit -m "feat(pf-tables): add sample_secondary_gas_kappa (per-kg minor gas)"
```
...
**Files:**
- Modify: `scripts/generate_pf_tables_linepyline.py:85-100` (earth scenario) and `:123-194` (`build_table`)

- [ ] **Step 1: Add the config key to the `earth` scenario**

In `scripts/generate_pf_tables_linepyline.py`, inside the `"earth": dict(...)` block, add after the `cia_files=[],` line:
...
In `build_table`, the imports block at the top of the file already pulls from `pf_table_builder.k_distribution import kappa_to_k_coeffs` and `pf_table_builder.kappa_sampling import sample_kappa_grid`. Extend those imports to also bring in the new helpers:

```python
from pf_table_builder.kappa_sampling import sample_kappa_grid, sample_secondary_gas_kappa
from pf_table_builder.k_distribution import kappa_to_k_coeffs, broadcast_and_stack_gases
```
...
Run: `conda run -n climt python -c "import ast; ast.parse(open('scripts/generate_pf_tables_linepyline.py').read()); print('ok')"`
Expected: `ok`.
...
git commit -m "feat(pf-tables): build O3 as additive secondary LW gas in earth scenario"
```
...
**Files:**
- Test: `tests/test_picket_fence_optics.py` (add)

- [ ] **Step 1: Write the test**

Add to `tests/test_picket_fence_optics.py`:
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_optics.py::test_interpolate_k_6d_two_gas_additive -v`
...
```bash
git add tests/test_picket_fence_optics.py
git commit -m "test(pf): pin 6-D H2O-axis + ngas=2 additive overlap path"
```
...
When the table is premixed-background AND carries secondary gases (e.g. `o3`), the component must request their runtime profiles. The premixed-bg branch (`climt/_components/picket_fence/lw/component.py:101-107`) currently only adds `specific_humidity`.

**Files:**
- Modify: `climt/_components/picket_fence/lw/component.py:100-107`
- Test: `tests/test_picket_fence_lw.py`

- [ ] **Step 1: Write the failing test (with a synthetic 2-gas table fixture)**

Add to `tests/test_picket_fence_lw.py` (imports `numpy as np`, `os`, and the writer from `scripts`):
...
def test_pf_lw_o3_table_requests_ozone_input(tmp_path):
    from climt import PicketFenceLongwave
    tbl = str(tmp_path / "o3.nc")
    _write_o3_test_table(tbl)
    comp = PicketFenceLongwave(optics="correlated_k", table=tbl)
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_lw.py::test_pf_lw_o3_table_requests_ozone_input -v`
...
In `climt/_components/picket_fence/lw/component.py`, replace the premixed-bg block (the `if self._premixed_bg:` body that adds `specific_humidity`) with:
...
git commit -m "feat(pf-lw): request mole_fraction_of_ozone_in_air for O3 tables"
```
...
The premixed-bg branch in `array_call` (`climt/_components/picket_fence/lw/component.py:236-249`) sets only `gas_amounts[0]` (air mass) and `h2o_vmr`. Extend it to convert each secondary gas's mole fraction to column mass.

**Files:**
- Modify: `climt/_components/picket_fence/lw/component.py:236-249`
- Test: `tests/test_picket_fence_lw.py`

- [ ] **Step 1: Write the failing test (O₃ on/off behaviour)**

Add to `tests/test_picket_fence_lw.py` (reuses `_write_o3_test_table`):

```python
def _o3_state(ncol=1, nlev=10):
    """Minimal state dict for PicketFenceLongwave correlated_k call."""
    import numpy as np
...
def test_pf_lw_o3_increases_window_band_optical_depth(tmp_path):
    import numpy as np
    from climt import PicketFenceLongwave
    tbl = str(tmp_path / "o3.nc")
    _write_o3_test_table(tbl)
...
Run: `conda run -n climt python -m pytest tests/test_picket_fence_lw.py::test_pf_lw_o3_increases_window_band_optical_depth -v`
...
In `climt/_components/picket_fence/lw/component.py`, in the `elif self._premixed_bg:` block of `array_call`, after the existing line that computes `h2o_vmr = q_h2o / np.maximum(...)`, append:
...
- [ ] **Step 5: Run the full PF LW suite for regressions**

Run: `conda run -n climt python -m pytest tests/test_picket_fence_lw.py tests/test_picket_fence_optics.py -q`
...
```bash
git add climt/_components/picket_fence/lw/component.py tests/test_picket_fence_lw.py
git commit -m "feat(pf-lw): fill column amounts for secondary additive gases (O3)"
```
...
**Files:**
- Create: `climt/_data/picket_fence/correlated_k/earth_low_res_lw_co2refined_o3.nc`
...
```bash
conda run -n radiation python scripts/generate_pf_tables_linepyline.py \
  --scenario earth --kind lw \
  --output climt/_data/picket_fence/correlated_k/earth_low_res_lw_co2refined_o3.nc \
...
import numpy as np
from scipy.io import netcdf_file
nc = netcdf_file('climt/_data/picket_fence/correlated_k/earth_low_res_lw_co2refined_o3.nc','r',mmap=False)
...
conda run -n climt python -c "
from climt import PicketFenceLongwave
c = PicketFenceLongwave(optics='correlated_k',
...
```bash
git add climt/_data/picket_fence/correlated_k/earth_low_res_lw_co2refined_o3.nc
git commit -m "data(pf): add Earth LW table with O3 9.6um additive gas (6-band, GL-8)"
```
...
End-to-end check that PF-with-O₃ recovers the stratospheric warming O₃ contributes in RRTMG, using climt's `init_ozone` profile on BOTH sides (apples-to-apples). Output figures to `debug_data/` per memory `feedback_experiment_scripts_location.md`.

**Files:**
- Create: `scripts/experiments/radiative_equilibrium_pf_o3.py`

- [ ] **Step 1: Write the validation script**

Create `scripts/experiments/radiative_equilibrium_pf_o3.py`. Base it on the existing `scripts/experiments/radiative_equilibrium_pf_convergence.py` (read it first for the column setup, surface/SW configuration, and integration loop). Adapt it to a 3-way comparison at radiative equilibrium (dry, CO₂=376 ppm, SW=240 W/m², ~1000 days):

1. **RRTMG** with `init_ozone` ozone profile (default `get_default_state`).
2. **PF (6-band CO₂-only)** using `earth_low_res_lw_co2refined_linepyline` — no O₃.
3. **PF (6-band CO₂+O₃)** using `earth_low_res_lw_co2refined_o3` — feed the SAME `mole_fraction_of_ozone_in_air` profile RRTMG uses.

Plot equilibrium T(p) for all three and save to `debug_data/pf_o3_rce_comparison.png`. Print T at the exp #10 diagnostic nodes (90, 40, 10, 2.4 hPa).

- [ ] **Step 2: Run the validation**

Run: `conda run -n climt python scripts/experiments/radiative_equilibrium_pf_o3.py`
Expected: writes `debug_data/pf_o3_rce_comparison.png` and prints a node table.

- [ ] **Step 3: Check acceptance**

Acceptance criteria (record the actual numbers in the experiment log):
- PF-with-O₃ is **warmer than PF-without-O₃** through the stratosphere (the O₃ warming has the right sign and is order ~10–30 K at 40–90 hPa).
- Under matched O₃, the PF-vs-RRTMG stratospheric gap at 40–90 hPa is materially smaller than the ~26 K no-O₃ artifact gap reported in exp #10 (target: within roughly ±10 K, acknowledging PF's coarse bands and the residual upper-strat integrator bias exp #10 flagged separately).
...
Append an experiment entry (tries / findings / numbers) to `docs/superpowers/plans/2026-05-16-pf-co2-band-refinement.md` per memory `feedback_log_experiments.md`, cross-referencing this plan and the design spec.

- [ ] **Step 5: Commit**

```bash
git add scripts/experiments/radiative_equilibrium_pf_o3.py docs/superpowers/plans/2026-05-16-pf-co2-band-refinement.md
git commit -m "test(pf): RCE validation of O3 stratospheric warming vs RRTMG"
```
...
git commit -m "chore(graph): refresh after PF O3 absorber"
```
...
- **Quadrature note:** the linepyline `kappa_to_k_coeffs` is Gauss-Legendre; the spec's "two-stretch g_split=0.97" referred to the separate exo_k `generate_picket_fence_tables.py` path and does not apply to this linepyline-built table.
```
