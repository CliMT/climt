...
scripts/
  generate_cork_tables.py # CREATE: Chaverot Exo_k → climt netCDF converter
  generate_trappist1_spectrum.py  # CREATE: writes trappist1.npz
  benchmark_cork.py       # CREATE: performance vs design targets
...
tests/
  ...
  test_grey_limit.py              # CREATE: cork vs GrayLongwaveRadiation
...
docs/
  ...
  radiative_transfer/
    ...
    06_cork.rst           # CREATE: Parmentier formulation, Rosseland, gammas
...
- Chaverot tables are **pre-mixed in their non-H2O background** (CO₂, N₂, continua and CIA are baked into the k-coefficients at a fixed composition), but the **H2O volume-mixing-ratio axis is preserved end-to-end**. The netCDF stores `k_coefficients` with shape `(gas, band, gpoint, T, P, X_H2O)` plus an `h2o_vmr_grid` coordinate and a `background_is_premixed = "true"` attribute. At runtime the cork component reads `specific_humidity` from state, converts it to H₂O mole fraction, and does trilinear `(T, log P, log X_H2O)` interpolation. The "gas" in `gas_names = ("h2o",)` is the live one; optical depth is computed as `k(T, p, X_H2O) · column_mass_of_AIR`. Earlier drafts of this plan collapsed the H2O axis at build time to a single representative VMR (e.g. 0.01 for Earth); that stopped the table from responding to moisture at runtime and agreed with RRTMG only by accident when the chosen bands were CO₂-dominated. Keeping the axis brings OLR agreement from ~−6% to ~−2% and makes the table respond correctly to dry vs. moist profiles.
...
- Create: `climt/_data/cork/stellar_spectra/trappist1.npz` (generated)
- Modify: `tests/test_cork_stellar.py`

The existing `CorkShortwaveRadiation` already supports loading non-Sun spectra by name (Phase 2 Task 6). We need to ship the TRAPPIST-1 spectrum.

- [x] **Step 1: Add test referencing the new spectrum**

Append to `tests/test_cork_stellar.py`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_cork_stellar.py::test_trappist1_spectrum_loads -v`
Expected: FAIL.
...
Expected: `wrote climt/_data/cork/stellar_spectra/trappist1.npz ...`
...
```bash
git add scripts/generate_trappist1_spectrum.py climt/_data/cork/stellar_spectra/trappist1.npz tests/test_cork_stellar.py
git commit -m "feat(cork): ship TRAPPIST-1 blackbody stellar spectrum"
```
...
- Modify: `climt/_components/cork/optics/parmentier.py`
...
def _make_state_with_high_albedo(sw):
    from climt._components.cork.tests_utils import make_default_state
...
If `tests_utils` doesn't exist yet, replace the helper call with an inline state dict (copy the pattern from `tests/test_cork_sw.py`'s `get_default_state_sw` fixture).
...
Append to `climt/_components/cork/optics/parmentier.py`:
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python -m pytest tests/test_bond_albedo_feedback.py tests/test_cork_sw.py -v`
Expected: ALL PASS.

- [x] **Step 6: Commit**

```bash
git add climt/_components/cork/sw/component.py climt/_components/cork/optics/parmentier.py tests/test_bond_albedo_feedback.py
git commit -m "feat(cork): add optional Bond-albedo self-consistency for Parmentier mode"
```
...
**Files:**
- Create: `scripts/generate_cork_tables.py`
- Create: `docs/radiative_transfer/table_generation.rst` (stub pointing at script)

Converts a Chaverot correlated-k table (Exo_k format) into climt's netCDF format (design spec §5.2). Re-bins to a small number of bands suitable for cork.
...
```python
"""Convert a Chaverot (Exo_k-format) correlated-k table into climt's cork
netCDF format and re-bin to cork resolution.
...
``scripts/generate_cork_tables_linepyline.py`` that builds correlated-k tables
...
For Titan tables, also download the HITRAN CIA files (one-time)::
    ...
    # Place in climt/_data/cork/cia/
    wget -P climt/_data/cork/cia/ \
...
Each invocation produces one ``.nc`` file (LW or SW). Run them in sequence::
    ...
    # Titan — LW + SW (requires CIA files in climt/_data/cork/cia/)
    conda run -n linepyline python scripts/generate_cork_tables_linepyline.py \\
...
``climt`` environment::
    ...
    conda run -n climt python -m pytest \\
        tests/test_cork_titan_trappist.py -v

**Pipeline internals**

The script assembles five helpers from ``scripts/cork_table_builder/``:
...
```bash
git add scripts/generate_cork_tables.py docs/radiative_transfer/table_generation.rst
git commit -m "feat(cork): add Chaverot->climt correlated-k table converter script"
```
...
**Files:**
- Create: `climt/_data/cork/correlated_k/earth_low_res_lw.nc`
- Create: `climt/_data/cork/correlated_k/earth_low_res_sw.nc`
- Create: `climt/_data/cork/correlated_k/mars_lw.nc`
- Create: `climt/_data/cork/correlated_k/mars_sw.nc`
- Create: `climt/_data/cork/correlated_k/venus_lw.nc`
- Create: `climt/_data/cork/correlated_k/venus_sw.nc`
...
python scripts/generate_cork_tables.py \
    --input $CHAV/H2O_N2_CO2_376ppm.h5 \
    --output climt/_data/cork/correlated_k/earth_low_res_sw.nc \
    --kind sw --bands 3250,8000,14000,30000 --ngpt 2

# Mars: CO2+N2 (96% CO2)
python scripts/generate_cork_tables.py \
    --input $CHAV/CO2_N2_mars.h5 \
    --output climt/_data/cork/correlated_k/mars_lw.nc \
    --kind lw --bands 10,500,1250,2500,3250 --ngpt 2

python scripts/generate_cork_tables.py \
    --input $CHAV/CO2_N2_mars.h5 \
    --output climt/_data/cork/correlated_k/mars_sw.nc \
    --kind sw --bands 3250,8000,14000,30000 --ngpt 2

# Venus: H2O+CO2 (96% CO2)
python scripts/generate_cork_tables.py \
    --input $CHAV/H2O_CO2_venus.h5 \
    --output climt/_data/cork/correlated_k/venus_lw.nc \
    --kind lw --bands 10,500,1250,2500,3250 --ngpt 2

python scripts/generate_cork_tables.py \
    --input $CHAV/H2O_CO2_venus.h5 \
    --output climt/_data/cork/correlated_k/venus_sw.nc \
    --kind sw --bands 3250,8000,14000,30000 --ngpt 2
```
...
Create `climt/_data/cork/correlated_k/MANIFEST.md`:
...
```bash
git add climt/_data/cork/correlated_k/*.nc climt/_data/cork/correlated_k/MANIFEST.md tests/test_correlated_k_tables.py
git commit -m "feat(cork): ship low-resolution correlated-k tables (Chaverot CC-BY)"
```
...
def test_grey_limit_matches_gray_longwave():
    """One-band, constant-opacity cork ≈ climt's GrayLongwaveRadiation."""
    gray = GrayLongwaveRadiation()
    cork = CorkLongwaveRadiation(optics="correlated_k", table="single_band_unit")

    state = get_default_state([gray, cork])
    tend_gray, _ = gray(state)
    tend_cork, _ = cork(state)

    g = tend_gray["air_temperature"].values
    p = tend_cork["air_temperature"].values
    # Gray radiation uses a bulk grey optical depth; cork single-band
...
Run: `python scripts/generate_cork_tables.py --input /tmp/unit.h5 --output climt/_data/cork/correlated_k/single_band_unit_lw.nc --kind lw --bands 10,3250 --ngpt 1`
...
```bash
git add tests/test_grey_limit.py scripts/generate_single_band_unit_table.py climt/_data/cork/correlated_k/single_band_unit_lw.nc
git commit -m "test(cork): grey-limit consistency with GrayLongwaveRadiation"
```
...
def test_cork_lw_vs_rrtmg_earth_standard():
    """Broadband LW fluxes from CorkLongwaveRadiation (earth_low_res) agree with
    RRTMG to within 25% at all pressure levels."""
    from climt import RRTMGLongwave, get_default_state
    from climt._components.cork import CorkLongwaveRadiation

    rrtmg = RRTMGLongwave()
    cork = CorkLongwaveRadiation(optics="correlated_k", table="earth_low_res")

    state = get_default_state([rrtmg, cork])
    _, d_rr = rrtmg(state)
    _, d_cork = cork(state)
...
def test_cork_sw_vs_rrtmg_earth_standard():
    ...
    cork = CorkShortwaveRadiation(optics="correlated_k", table="earth_low_res")
...
git commit -m "test(cork): RRTMG LW/SW comparison on Earth standard atmosphere"
```
...
git commit -m "test(cork): reproduce Lee et al. 2021 HD 209458b T-p profile"
```
...
def test_rfmip_broadband_olr(scenario):
    """Broadband OLR from CorkLongwaveRadiation (earth_low_res) matches
    ...
    lw = CorkLongwaveRadiation(optics="correlated_k", table="earth_low_res")
    _, d = lw(state)
...
git commit -m "test(cork): RFMIP broadband OLR benchmark (3 scenarios)"
```
...
Pre-generate `earth_low_res_lw_4gpt.nc` and `earth_low_res_lw_8gpt.nc` using the script in Task 5 with `--ngpt 4` and `--ngpt 8`. Commit them under `climt/_data/cork/correlated_k/`.
...
def test_olr_converges_with_g_points():
    ...
    for tbl in ("earth_low_res", "earth_low_res_4gpt", "earth_low_res_8gpt"):
        lw = CorkLongwaveRadiation(optics="correlated_k", table=tbl)
...
git add tests/test_resolution_convergence.py climt/_data/cork/correlated_k/earth_low_res_lw_4gpt.nc climt/_data/cork/correlated_k/earth_low_res_lw_8gpt.nc
git commit -m "test(cork): g-point resolution convergence check"
```
...
**Files:**
- Create: `scripts/benchmark_cork.py`
...
```python
"""Measure cork solver iterations per second on a single column.
...
def main():
    ...
    configs = [
        ("parmentier_lw", CorkLongwaveRadiation(optics="parmentier"), 1000),
        ("earth_low_res_lw", CorkLongwaveRadiation(
            optics="correlated_k", table="earth_low_res"
        ), 400),
        ("earth_low_res_sw", CorkShortwaveRadiation(
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt python scripts/benchmark_cork.py`
Paste output into `docs/radiative_transfer/performance.rst` (created in Task 15).

- [ ] **Step 3: Commit**

```bash
git add scripts/benchmark_cork.py
git commit -m "perf(cork): add single-column performance benchmark script"
```
...
git commit -m "docs(cork): add k_distribution_demo notebook"
```
...
2. Code: load standard Earth profile, run `CorkLongwaveRadiation`. Use the always-on per-band diagnostics (`longwave_optical_depth_per_band`, `longwave_transmittance_per_band`, `longwave_heating_rate_per_band`).
...
git commit -m "docs(cork): add spectral_radiation_anatomy notebook"
```

---

### Task 15: `cork_vs_rrtmg.ipynb`

**Files:**
- Create: `examples/cork_vs_rrtmg.ipynb`
...
```bash
git add examples/cork_vs_rrtmg.ipynb
git commit -m "docs(cork): add cork_vs_rrtmg notebook"
```
...
thing to the Parmentier cork formulation climt ships with, and
...
git commit -m "docs(cork): scaffold radiative-transfer walkthrough and figure script"
```
...
The cork scheme you will implement in the remaining chapters is
...
from ``CorkLongwaveRadiation``'s per-band diagnostics.
...
``climt/_components/cork/optics/correlated_k.py`` makes it
explicit what the file contains:

.. literalinclude:: ../../climt/_components/cork/optics/correlated_k.py
...
band are 1–3 % versus LBL. For cork with 2 g-points, errors
...
``climt._components.cork.optics.correlated_k._esft_combine``;
...
### Task 22: Chapter 6 — "The cork model"

**Files:**
- Create: `docs/radiative_transfer/06_cork.rst`
...
```rst
Chapter 6 — The cork model
==================================

Everything so far has been a generic correlated-k story. The
cork model is the simplest possible parameterisation of that
...
.. literalinclude:: ../../climt/_components/cork/optics/parmentier.py
   :pyobject: compute_rosseland_mean_opacity
   :language: python

.. figure:: figures/06_cork_opacity.png
   :width: 55%
...
short slats are the atmospheric window. The cork is the
...
* ``examples/cork_vs_rrtmg.ipynb`` compares the Parmentier
...
```bash
cd /Users/joymonteiro/github/climt/docs && conda run -n climt sphinx-build -W -b html . _build/html
git add docs/radiative_transfer/06_cork.rst
git commit -m "docs(rt-walkthrough): chapter 6 — cork model"
```
...
.. literalinclude:: ../../climt/_components/cork/sw/kernels.py
...
See ``_adding`` in ``climt/_components/cork/sw/kernels.py``.
...
.. literalinclude:: ../../climt/_components/cork/lw/kernels.py
...
The two runs share the same ``CorkLongwaveRadiation`` constructor;
...
climt's cork scheme is fast enough for interactive use. Targets
...
Reproduce the benchmark::
    ...
    python scripts/benchmark_cork.py
...
Run: `cd /Users/joymonteiro/github/climt && conda run -n climt jupyter nbconvert --to notebook --execute examples/k_distribution_demo.ipynb examples/spectral_radiation_anatomy.ipynb examples/cork_vs_rrtmg.ipynb --inplace`
...
At this point the cork scheme is feature-complete per the
design spec. Tag the state:

```bash
git tag -a cork-v1.0 -m "Cork radiation: feature-complete per design spec"
```
