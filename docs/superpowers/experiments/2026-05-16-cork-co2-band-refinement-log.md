# Experiment Log: Cork LW CO2 Band Refinement

**Date:** 2026-05-16
**Branch:** `feature/cork-radiation`
**Related plan:** `docs/superpowers/plans/2026-05-16-cork-co2-band-refinement.md`

## Goal

Reduce the Earth-LW cork cooling-to-space bias around the 5–10 hPa
...
| Script | Env | Purpose |
|---|---|---|
| `examples/cooling_to_space_gsplit_sweep.py` | climt | Heating-rate vs pressure, isothermal 250 K column, multiple CORK tables vs RRTMG |
| `examples/lbl_band_mean_k.py --profile {iso,adiabat}` | linepyline | Run LBL on a column, save `kappa(p, nu)` to netCDF |
| `examples/compare_lbl_cork_band_k.py` | climt | Single-pressure tabular comparison: CORK vs LBL Planck-weighted band-mean k |
| `examples/compare_lbl_cork_heatmap.py` | climt | (band × pressure) heatmap of CORK/LBL ratio, two profiles × all CORK tables |

LBL was Planck-weighted band-averaged using each CORK table's own band edges, so
the comparison is apples-to-apples per table.

## Tables Generated

All from `scripts/generate_cork_tables.py` against the Chaverot Earth
...
**Result:** CORK/LBL ratio in the 630–700 line-core band = **0.450** (with 2s
...
**Result (CORK/LBL at p=10 hPa, iso 250 K):**
...
**Result:** CORK/LBL ratio in 660–720 = **0.446**. Essentially identical to the
...
CORK tables can't recover what's been discarded upstream.
...
confined to the upper stratosphere/mesosphere (where any cork
...
1. **15 μm line-core opacity remains ~75–80% under-represented at low p** in
   ...
   R=500 ceiling and persists across all CORK configurations. Residual cool-
...
2. **Cold-T (T<150 K) behavior is unreliable across all tables.** Both GL
   and 2s tables on the dry-adiabat row show extreme CORK over-opacity
...
1. **Re-run cool-to-space and RCE diagnostics with `co2refined_gl` as the
   primary CORK curve.** Test whether the more balanced full-column opacity
...
4. **(Long-term) Build a higher-R source k-table** from HITRAN directly,
   ...
   sets the absolute upper bound on what any cork representation
   can achieve.
