"""Grey-limit consistency between PicketFenceLongwave and GrayLongwaveRadiation.

A picket-fence scheme with a single band, a single g-point, constant mass-
...
1. Optical-depth profile: ``GrayLongwaveRadiation`` consumes a prescribed
   ``longwave_optical_depth_on_interface_levels``, while PicketFence computes
   tau from ``k * column_mass(gas)``. So we build PF's per-layer tau and feed
   its cumulative surface-to-TOA integral to Gray.

2. Diffusivity factor: PicketFence multiplies vertical tau by D=1.66 inside
...
"the grey limit of the picket-fence scheme reproduces gray radiation" means
rigorously.
"""
import os
...
DIFFUSIVITY_FACTOR = 1.66  # matches climt/_components/picket_fence/lw/kernels.py
...
def test_grey_limit_matches_gray_longwave_bit_exact():
    """Single-band, single-gpt, constant-k picket-fence == GrayLongwave
    when tau profile and diffusivity factor are matched."""
    _ensure_unit_table_exists()

    gray = GrayLongwaveRadiation()
    pf = PicketFenceLongwave(optics="correlated_k", table="single_band_unit_lw")

    state = get_default_state([gray, pf])

    # Call PF to compute its vertical optical depth per layer.
    tend_pf, diag_pf = pf(state)
    tau_layer = diag_pf["longwave_optical_depth_per_band"].values[:, 0, 0, 0]

    # Build a cumulative interface-level tau for Gray, pre-multiplied by D
    # to absorb PF's diffusivity-factor convention.
    nlev = tau_layer.size
    ...
    olr_pf = diag_pf["upwelling_longwave_flux_in_air"].values.flat[-1]
    np.testing.assert_allclose(olr_pf, olr_gray, rtol=1e-10)
