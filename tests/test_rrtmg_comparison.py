"""Compare PicketFence (correlated-k) against RRTMG on an Earth standard atmosphere.

Picket-fence ships low-resolution (4 LW / 3 SW bands) correlated-k tables
...
Note on state building: ``PicketFenceLongwave.__init__`` calls
``set_num_longwave_bands(n)`` as a side-effect to size ``num_longwave_bands``
dimensions (e.g. surface emissivity). RRTMG needs 16; PF needs 4. We run
RRTMG *before* instantiating PF, then build PF's state independently.
"""
import numpy as np
import pytest
...
@pytest.mark.skipif(not _rrtmg_available(), reason="RRTMG not available")
def test_picket_fence_lw_responds_to_humidity():
