...
def get_surface_grid(grid_state, interface=False, horizontal=False):
    return tuple(grid_state["latitude"].shape), tuple(grid_state["latitude"].dims)


def get_land_grid(grid_state, interface=False, horizontal=False):
    y, x = grid_state["latitude"].shape
    y_dim, x_dim = grid_state["latitude"].dims
    ...
    if horizontal:
        ...
        raise NotImplementedError("3D land grids are not yet supported")


def get_ocean_grid(grid_state, interface=False, horizontal=False):
    y, x = grid_state["latitude"].shape
    y_dim, x_dim = grid_state["latitude"].dims
    ...
    if horizontal:
        ...
        raise NotImplementedError("3D ocean grids are not yet supported")


def get_ice_grid(grid_state, interface=False, horizontal=False):
    y, x = grid_state["latitude"].shape
    y_dim, x_dim = grid_state["latitude"].dims
...
class ConstantDefaultValue(DiagnosticComponent):
    ...
    def __init__(
        self, output_name, output_value, output_units, dtype=None, domain=None, **kwargs
    ):
        if dtype is None:
    ...
    def store_domain_properties(self, domain):
        ...
        if len(domain_and_type) > 1:
            if domain_and_type[1] == "horizontal":
                ...
                NotImplementedError("Unknown domain descriptor {}".format(domain))
    ...
    def __call__(self, grid_state):
        ...
        quantity_np_array = np.broadcast_to(
            np.array(self._output_value, dtype=self._dtype), shape
        ).copy()

        quantity_array = get_backend().create_quantity(
            quantity_np_array,
            self._output_name,
            self._output_units,
            dims,
...
class PressureFunctionDiagnosticComponent(DiagnosticComponent):
    ...
    input_properties = {
        ...
        "surface_air_pressure": {"dims": ["*"], "units": "Pa", "alias": "ps"},
    }

    diagnostic_properties = {}

    def __init__(
        self, output_name, output_function, output_units, mid_or_interface_levels="mid"
    ):
        """
        ...
        self.diagnostic_properties = {
            output_name: {"dims": [vertical_dimension, "*"], "units": output_units}
        }
...
def get_grid(
    ...
    return_state["surface_air_pressure"] = get_backend().create_quantity(
        np.ones((ny, nx)) * p_surf_in_Pa,
    ...
    if nx is not None:
        ...
        two_dim_lons[:] = np.linspace(0.0, 360.0, nx * 2, endpoint=False)[:-1:2][
            np.newaxis, :
        ]
        return_state["longitude"] = get_backend().create_quantity(
            two_dim_lons,
            "longitude",
            "degrees_east",
            (y_name, x_name),
    ...
    if ny is not None:
        ...
        if latitude_grid.lower() == "regular":
            two_dim_lats[:] = np.linspace(-90.0, 90.0, ny * 2 + 1, endpoint=True)[
                1:-1:2
            ][:, np.newaxis]
            return_state["latitude"] = get_backend().create_quantity(
                two_dim_lats,
                "latitude",
                "degrees_north",
                (y_name, x_name),
    ...
    if n_ice_interface_levels is not None:
        return_state["height_on_ice_interface_levels"] = get_backend().create_quantity(
...
class HybridSigmaPressureDiagnosticComponent(DiagnosticComponent):
    ...
    def array_call(self, state):
        ...
        p_interface = state["a_coord"] + state["b_coord"] * (
...
def get_hybrid_sigma_pressure_levels(
    num_levels=28,
    ...
    sigma_interface = (pressure_levels - model_top_pressure) / (
        reference_pressure - model_top_pressure
    )
    ...
    for level in range(num_isobaric_levels, num_levels - num_sigma_levels):
        ...
        ak[level] = model_top_pressure + (sigma_value - bk[level]) * (
            reference_pressure - model_top_pressure
        )

    for level in range(num_levels - num_sigma_levels, num_levels):
        sigma_value = sigma_interface[level]
        bk[level] = (sigma_interface[level] - isobaric_sigma_level) / (
            1 - isobaric_sigma_level
        )
        ak[level] = model_top_pressure + (sigma_value - bk[level]) * (
            reference_pressure - model_top_pressure
        )
    ...
    return {
        "atmosphere_hybrid_sigma_pressure_a_coordinate_on_interface_levels": get_backend().create_quantity(
            ak,
            "atmosphere_hybrid_sigma_pressure_a_coordinate_on_interface_levels",
            "dimensionless",
            ("interface_levels",),
        ),
        "atmosphere_hybrid_sigma_pressure_b_coordinate_on_interface_levels": get_backend().create_quantity(
            bk,
            "atmosphere_hybrid_sigma_pressure_b_coordinate_on_interface_levels",
            "dimensionless",
            ("interface_levels",),
...
default_values = {
    "air_temperature": {"value": 290.0, "units": "degK", "domain": "atmosphere"},
    "northward_wind": {"value": 0.0, "units": "m/s", "domain": "atmosphere"},
    "eastward_wind": {"value": 0.0, "units": "m/s", "domain": "atmosphere"},
    "divergence_of_wind": {"value": 0.0, "units": "s^-1", "domain": "atmosphere"},
    "atmosphere_relative_vorticity": {
        "value": 0.0,
        "units": "s^-1",
        "domain": "atmosphere",
    },
    "specific_humidity": {"value": 0.0, "units": "kg/kg", "domain": "atmosphere"},
    ...
    "surface_specific_humidity": {"value": 0.0, "units": "kg/kg", "domain": "surface"},
    "surface_temperature": {"value": 300.0, "units": "degK", "domain": "surface"},
    "soil_surface_temperature": {"value": 300.0, "units": "degK", "domain": "surface"},
    "surface_geopotential": {"value": 0.0, "units": "m^2 s^-2", "domain": "surface"},
    "surface_thermal_capacity": {
        "value": 4.1813e3,
        "units": "J kg^-1 degK^-1",
        "domain": "surface",
    },
    "depth_of_slab_surface": {"value": 50.0, "units": "m", "domain": "surface"},
    ...
    "snow_and_ice_temperature": {
        "value": 270.0,
        "units": "degK",
        "domain": "ice_interface",
    },
    "heat_flux_into_sea_water_due_to_sea_ice": {
        "value": 0.0,
        "units": "W m^-2",
        "domain": "ice_horizontal",
    },
    "land_ice_thickness": {"value": 0.0, "units": "m", "domain": "ice_horizontal"},
    "sea_ice_thickness": {"value": 0.0, "units": "m", "domain": "ice_horizontal"},
    "surface_snow_thickness": {"value": 0.0, "units": "m", "domain": "ice_horizontal"},
    "solar_cycle_fraction": {"value": 0.0, "units": "dimensionless", "domain": None},
...
def get_init_diagnostic(name, grid_state):
    ...
    raise NotImplementedError(
        "No initialization method for quantity name {}".format(name)
    )
...
init_diagnostics = [
    PressureFunctionDiagnosticComponent(
        ...
        lambda p, ps: 1.0 * (1.0 - p / ps),
        "dimensionless",
        "interface",
    ),
    PressureFunctionDiagnosticComponent(
        "mole_fraction_of_ozone_in_air", init_ozone, "mole/mole", "mid"
    ),
    ...
    for diag2 in init_diagnostics:
        ...
        if diag2 is diag:
            ...
            raise AssertionError(
                ...
                "be removed.".format(
                    diag.__class__.__name__, diag2.__class__.__name__, output_overlap
                )
            )
