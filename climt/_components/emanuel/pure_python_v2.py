...
class EmanuelConvectionPythonV2(object):
    ...
    def array_call(self, state, timestep):
        ...
        results = _numpy_vectorized_convect(
            ...
            tra_vector,
            self._params,
            _convect_functional_numba,
            _tlift_functional_numba,
        )
...
def _tlift_functional(xp, P, T, Q, QS, GZ, ICB, NK, ND, NL, KK, TVP, TPK, CLW, params):
    ...
    if KK == 1:
        ...
        for i in range(NK, ICB):
            ...
            TVP = set_item(TVP, i, TPK[i] * (1.0 + Q[NK] * EPSI))
    ...
    for i in range(NSB, NST + 1):
        ...
        RG = QG / (1.0 - Q[NK])
        TVP = set_item(TVP, i, TPK[i] * (1.0 + RG * EPSI))

    return TVP, TPK, CLW


def _convect_functional(
    xp,
    tlift_func,
    T_in,
    Q_in,
    ...
    for i in range(1, NL + 1):
        ...
        CPN = set_item(CPN, i, params.CPD * (1.0 - Q[i]) + params.CPV * Q[i])
        ...
        TV = set_item(TV, i, T[i] * (1.0 + Q[i] * EPSI - Q[i]))
    ...
    TP = xp.zeros(ND)
    CLW = xp.zeros(ND)
    TVP, TP, CLW = tlift_func(
        xp, P, T, Q, QS, GZ, ICB, NK, ND, NL, 1, TVP, TP, CLW, params
    )
    ...
    TVP, TP, CLW = tlift_func(
        xp, P, T, Q, QS, GZ, ICB, NK, ND, NL, 2, TVP, TP, CLW, params
    )
    ...
    QP = set_item(QP, 0, Q[0])
    UP = set_item(UP, 0, U[0])
    VP = set_item(VP, 0, V[0])
    for i in range(NTRA):
        TRAP = set_item(TRAP, (0, i), TRA[0, i])
    for i in range(1, NL + 1):
        QP = set_item(QP, i, Q[i - 1])
        UP = set_item(UP, i, U[i - 1])
        VP = set_item(VP, i, V[i - 1])
        for j in range(NTRA):
            TRAP = set_item(TRAP, (i, j), TRA[i - 1, j])
    ...
    for i in range(ICB + 1, INB + 1):
        ...
        for j in range(ICB, INB + 1):
            ...
            if 0.0 < SIJ[i, j] < 0.9:
                ...
                MENT = set_item(MENT, (i, j), M[i] / (1.0 - SIJ[i, j]))
                NENT = set_item(NENT, i, NENT[i] + 1)
            SIJ = set_item(SIJ, (i, j), xp.minimum(xp.maximum(SIJ[i, j], 0.0), 1.0))
        if NENT[i] == 0:
            MENT = set_item(MENT, (i, i), M[i])
            QENT = set_item(QENT, (i, i), Q[NK] - EP[i] * CLW[i])
            UENT = set_item(UENT, (i, i), U[NK])
            VENT = set_item(VENT, (i, i), V[NK])
            for k in range(NTRA):
                TRAENT = set_item(TRAENT, (i, i, k), TRA[NK, k])
            ELIJ = set_item(ELIJ, (i, i), CLW[i])
            SIJ = set_item(SIJ, (i, i), 1.0)
                ...
                if 0.0 < SIJ[i, j] < 0.9:
                    if j > i:
                        ...
                        if SMID < SMIN and SIJ[i, j + 1] < SMID:
                            SMIN = SMID
                            SJMAX = xp.minimum(
                                xp.minimum(SIJ[i, j + 1], SIJ[i, j]), SCRIT
                            )
                            SJMIN = xp.minimum(
                                xp.maximum(SIJ[i, j - 1], SIJ[i, j]), SCRIT
                            )
                    else:
                        SJMAX = xp.maximum(SIJ[i, j + 1], SCRIT)
                        SMID = xp.maximum(SIJ[i, j], SCRIT)
                        SJMIN = xp.maximum(SIJ[i, j - 1] if j > 0 else 0.0, SCRIT)
                    ASIJ += (xp.abs(SJMAX - SMID) + xp.abs(SJMIN - SMID)) * (
                        PH[j] - PH[j + 1]
                    )
                    MENT = set_item(
                        MENT,
                        (i, j),
                        MENT[i, j]
                        * (xp.abs(SJMAX - SMID) + xp.abs(SJMIN - SMID))
                        * (PH[j] - PH[j + 1]),
                    )
            ASIJ = xp.maximum(1.0e-21, ASIJ)
            for j in range(ICB, INB + 1):
                MENT = set_item(MENT, (i, j), MENT[i, j] / ASIJ)
            if xp.sum(MENT[i, ICB : INB + 1]) < 1.0e-18:
                NENT = set_item(NENT, i, 0)
                MENT = set_item(MENT, (i, i), M[i])
                QENT = set_item(QENT, (i, i), Q[NK] - EP[i] * CLW[i])
                UENT = set_item(UENT, (i, i), U[NK])
                VENT = set_item(VENT, (i, i), V[NK])
        ...
        for i in range(INB, -1, -1):
            ...
            if i > 0:
                for j in range(i):
                    WDTRAIN += (
                        params.G
                        * xp.maximum(0.0, ELIJ[j, i] - (1.0 - EP[i]) * CLW[i])
                        * MENT[j, i]
                    )
            ...
            REVAP = 0.5 * (-B6 + xp.sqrt(B6 * B6 + 4.0 * C6))
            ...
            if i > 0:
                ...
                MP = set_item(MP, i, (FAC * MP[i + 1] + MP[i]) / (1.0 + FAC))
                if P[i] > (0.949 * P[0]):
                    JTT = max(JTT, i)
            ...
            if i != INB:
                ...
                if MP[i] > MP[i + 1]:
                    ...
                    UP = set_item(UP, i, UP[i + 1] * RAT + U[i] * (1.0 - RAT))
                    VP = set_item(VP, i, VP[i + 1] * RAT + V[i] * (1.0 - RAT))
                    for k in range(NTRA):
                        TRAP = set_item(
                            TRAP,
                            (i, k),
                            TRAP[i + 1, k] * RAT + TRAP[i, k] * (1.0 - RAT),
                        )
                elif MP[i + 1] > 0.0:
                    QP = set_item(
                        QP,
                        ...
                        (
                            ...
                            - GZ[i]
                            + QP[i + 1]
                            * (LV[i + 1] + T[i + 1] * (params.CL - params.CPD))
                            + params.CPD * (T[i + 1] - T[i])
                        )
                        / (LV[i] + T[i] * (params.CL - params.CPD)),
                    )
                    UP = set_item(UP, i, UP[i + 1])
                    VP = set_item(VP, i, VP[i + 1])
                    for k in range(NTRA):
                        TRAP = set_item(TRAP, (i, k), TRAP[i + 1, k])
    ...
    FU = set_item(
        FU, 0, FU[0] + params.G * DPINV * (MP[1] * (UP[1] - U[0]) + AM * (U[1] - U[0]))
    )
    FV = set_item(
        FV, 0, FV[0] + params.G * DPINV * (MP[1] * (VP[1] - V[0]) + AM * (V[1] - V[0]))
    )
    for j in range(NTRA):
        FTRA = set_item(
            FTRA,
    ...
    for j in range(1, INB + 1):
        FQ = set_item(
            FQ, 0, FQ[0] + params.G * DPINV * MENT[j, 0] * (QENT[j, 0] - Q[0])
        )
        FU = set_item(
            FU, 0, FU[0] + params.G * DPINV * MENT[j, 0] * (UENT[j, 0] - U[0])
        )
        FV = set_item(
            FV, 0, FV[0] + params.G * DPINV * MENT[j, 0] * (VENT[j, 0] - V[0])
        )
        for k in range(NTRA):
            FTRA = set_item(
                FTRA,
                (0, k),
                FTRA[0, k]
                + params.G * DPINV * MENT[j, 0] * (TRAENT[j, 0, k] - TRA[0, k]),
            )
    ...
    for i in range(1, INB + 1):
        ...
        for k in range(i + 1):
            AMP1 += xp.sum(MENT[k, i + 1 : INB + 2])
        if (2.0 * params.G * DPINV * AMP1) >= DELTI:
            IFLAG = 4
        AD = 0.0
        for k in range(i):
            AD += xp.sum(MENT[i : INB + 1, k])
        FT = set_item(
            FT,
            i,
            FT[i]
        ...
        FQ = set_item(
            ...
            i,
            FQ[i]
            + params.G * DPINV * (AMP1 * (Q[i + 1] - Q[i]) - AD * (Q[i] - Q[i - 1])),
        )
        FU = set_item(
            FU,
            i,
            FU[i]
            + params.G * DPINV * (AMP1 * (U[i + 1] - U[i]) - AD * (U[i] - U[i - 1])),
        )
        FV = set_item(
            FV,
            i,
            FV[i]
            + params.G * DPINV * (AMP1 * (V[i + 1] - V[i]) - AD * (V[i] - V[i - 1])),
        )
        for k in range(NTRA):
            FTRA = set_item(
                FTRA,
        ...
        for k in range(i):
            AWAT = xp.maximum(ELIJ[k, i] - (1.0 - EP[i]) * CLW[i], 0.0)
            FQ = set_item(
                FQ,
                i,
            ...
            FU = set_item(
                FU, i, FU[i] + params.G * DPINV * MENT[k, i] * (UENT[k, i] - U[i])
            )
            FV = set_item(
                FV, i, FV[i] + params.G * DPINV * MENT[k, i] * (VENT[k, i] - V[i])
            )
            for j in range(NTRA):
                FTRA = set_item(
                    FTRA,
        ...
        for k in range(i, INB + 1):
            FQ = set_item(
                FQ, i, FQ[i] + params.G * DPINV * MENT[k, i] * (QENT[k, i] - Q[i])
            )
            FU = set_item(
                FU, i, FU[i] + params.G * DPINV * MENT[k, i] * (UENT[k, i] - U[i])
            )
    ...
    FQOLD = FQ[INB]
    FQ = set_item(FQ, INB, FQ[INB] * (1.0 - FRAC))
    FQ = set_item(
        FQ,
        INB - 1,
        FQ[INB - 1]
    ...
    FTOLD = FT[INB]
    FT = set_item(FT, INB, FT[INB] * (1.0 - FRAC))
    FT = set_item(
        FT,
        INB - 1,
        FT[INB - 1]
    ...
    FUOLD = FU[INB]
    FU = set_item(FU, INB, FU[INB] * (1.0 - FRAC))
    FU = set_item(
        FU,
        INB - 1,
        FU[INB - 1]
    ...
    FVOLD = FV[INB]
    FV = set_item(FV, INB, FV[INB] * (1.0 - FRAC))
    FV = set_item(
        FV,
        INB - 1,
        FV[INB - 1]
    ...
    for k in range(NTRA):
        FTRAOLD = FTRA[INB, k]
        FTRA = set_item(FTRA, (INB, k), FTRA[INB, k] * (1.0 - FRAC))
        FTRA = set_item(
            FTRA,
            (INB - 1, k),
            FTRA[INB - 1, k]
    ...
    UAV = xp.sum(FU[: INB + 1] * (PH[: INB + 1] - PH[1 : INB + 2])) / (
        PH[0] - PH[INB + 1]
    )
    VAV = xp.sum(FV[: INB + 1] * (PH[: INB + 1] - PH[1 : INB + 2])) / (
        PH[0] - PH[INB + 1]
    )
    for i in range(INB + 1):
        FT = set_item(FT, i, FT[i] - ENTS / CPN[i])
        FU = set_item(FU, i, (1.0 - params.CU) * (FU[i] - UAV))
        FV = set_item(FV, i, (1.0 - params.CU) * (FV[i] - VAV))
    for k in range(NTRA):
        TRAAV = xp.sum(FTRA[: INB + 1, k] * (PH[: INB + 1] - PH[1 : INB + 2])) / (
            PH[0] - PH[INB + 1]
        )
