...
def _convect_functional_np(T_in, Q_in, QS_in, U_in, V_in, P_in, PH_in, ND, NL, NTRA, DELT, CBMF_in, TRA_in, params):
    ...
    PRECIP = 0.0; WD = 0.0; TPRIME = 0.0; QPRIME = 0.0; IFLAG = 0; OUTCAPE = 0.0
    TH = np.zeros(NL + 1)
    for i in range(NL + 1):
    ...
    GZ[0] = 0.0; CPN[0] = params.CPD * (1. - Q[0]) + Q[0] * params.CPV; H[0] = T[0] * CPN[0]
    LV[0] = params.LV0 - CPVMCL * (T[0] - 273.15); HM[0] = LV[0] * Q[0]; TV[0] = T[0] * (1. + Q[0] * EPSI - Q[0])
    AHMIN = 1.0E12; IHMIN = NL
    for i in range(1, NL + 1):
        TVX = T[i] * (1. + Q[i] * EPSI - Q[i]); TVY = T[i-1] * (1. + Q[i-1] * EPSI - Q[i-1])
        GZ[i] = GZ[i-1] + 0.5 * params.RD * (TVX + TVY) * (P[i-1] - P[i]) / PH[i]
        CPN[i] = params.CPD * (1. - Q[i]) + params.CPV * Q[i]; H[i] = T[i] * CPN[i] + GZ[i]
        LV[i] = params.LV0 - CPVMCL * (T[i] - 273.15)
        HM[i] = (params.CPD * (1. - Q[i]) + params.CL * Q[i]) * (T[i] - T[0]) + LV[i] * Q[i] + GZ[i]
        TV[i] = T[i] * (1. + Q[i] * EPSI - Q[i])
        if (i+1) >= params.MINORIG and HM[i] < AHMIN and HM[i] < HM[i-1]: AHMIN = HM[i]; IHMIN = i
    IHMIN = min(IHMIN, NL - 1); AHMAX = 0.0; NK = 0
    for i in range(params.MINORIG - 1, IHMIN + 1):
        if HM[i] > AHMAX: NK = i; AHMAX = HM[i]
    if T[NK] < 250.0 or Q[NK] <= 0.0 or IHMIN == (NL - 1):
        CBMF = 0.0; return FT, FQ, FU, FV, PRECIP, WD, TPRIME, QPRIME, CBMF, OUTCAPE, IFLAG
    RH = Q[NK] / QS[NK]; CHI = T[NK] / (1669.0 - 122.0 * RH - T[NK]); PLCL = P[NK] * (RH**CHI)
    if PLCL < 200.0 or PLCL >= 2000.0:
        IFLAG = 2; CBMF = 0.0; return FT, FQ, FU, FV, PRECIP, WD, TPRIME, QPRIME, CBMF, OUTCAPE, IFLAG
    ICB = NL - 1
    for i in range(NK + 1, NL + 1):
        if P[i] < PLCL: ICB = min(ICB, i)
    if ICB >= (NL - 1):
        IFLAG = 3; CBMF = 0.0; return FT, FQ, FU, FV, PRECIP, WD, TPRIME, QPRIME, CBMF, OUTCAPE, IFLAG
    TVP = np.zeros(ND); TP = np.zeros(ND); CLW = np.zeros(ND)
    TVP, TP, CLW = _tlift_functional_np(P, T, Q, QS, GZ, ICB, NK, ND, NL, 1, TVP, TP, CLW, params)
    ...
    if IFLAG != 4: IFLAG = 1
    TVP, TP, CLW = _tlift_functional_np(P, T, Q, QS, GZ, ICB, NK, ND, NL, 2, TVP, TP, CLW, params)
    EP = np.zeros(ND); SIGP = np.zeros(ND)
    for i in range(NK + 1):
        EP[i] = 0.0; SIGP[i] = params.SIGS
    for i in range(NK + 1, NL + 1):
        TCA = TP[i] - 273.15
        ELACRIT = params.ELCRIT if TCA >= 0.0 else params.ELCRIT * (1.0 - TCA / params.TLCRIT)
        ELACRIT = max(ELACRIT, 0.0); EPMAX = 0.999
        EP[i] = EPMAX * (1.0 - ELACRIT / max(CLW[i], 1.0E-8))
        EP[i] = max(min(EP[i], EPMAX), 0.0); SIGP[i] = params.SIGS
    for i in range(ICB + 1, NL + 1): TVP[i] -= TP[i] * Q[NK]
    HP = H.copy()
    NENT = np.zeros(ND + 1, dtype=np.int32)
    WATER = np.zeros(ND + 1); EVAP = np.zeros(ND + 1); WT = np.full(ND + 1, params.OMTSNOW)
    MP = np.zeros(ND + 1); M = np.zeros(ND + 1); LVCP = np.zeros(ND + 1)
    for i in range(NL + 1): LVCP[i] = LV[i] / CPN[i]
    QENT = np.zeros((ND + 1, ND + 1)); ELIJ = np.zeros((ND + 1, ND + 1))
    MENT = np.zeros((ND + 1, ND + 1)); SIJ = np.zeros((ND + 1, ND + 1))
    UENT = np.zeros((ND + 1, ND + 1)); VENT = np.zeros((ND + 1, ND + 1))
    TRAENT = np.zeros((ND + 1, ND + 1, max(1, NTRA)))
    for i in range(NL + 1):
        for j in range(NL + 1):
            QENT[i, j] = Q[j]; UENT[i, j] = U[j]; VENT[i, j] = V[j]
            for k in range(NTRA): TRAENT[i, j, k] = TRA[j, k]
    QP = np.zeros(ND + 1); UP = np.zeros(ND + 1); VP = np.zeros(ND + 1)
    ...
    INB = max(INB, INB1); CAPE = CAPEM + BYP
    DEFRAC = max(CAPEM - CAPE, 0.001); FRAC = min(max(-CAPE / DEFRAC, 0.0), 1.0); OUTCAPE = CAPE
    ...
    DTPBL /= (PH[NK] - PH[ICB]); DTMA = TVPPLCL - TVAPLCL + params.DTMAX + DTPBL
    CBMFOLD = CBMF; DAMPS = params.DAMP * DELT / params.DELT0
    CBMF = (1. - DAMPS) * CBMF + 0.1 * params.ALPHA * DTMA; CBMF = max(CBMF, 0.0)
    ...
    for i in range(ICB + 1, INB + 1):
        ...
        for j in range(ICB, INB + 1):
            ...
            if 0.0 < SIJ[i, j] < 0.9:
                ...
                ELIJ[i, j] = max(0.0, ALTEM); MENT[i, j] = M[i] / (1. - SIJ[i, j]); NENT[i] += 1
    ...
    for i in range(ICB + 1, INB + 1):
        if NENT[i] != 0:
            ...
            SCRIT = max(SCRIT, 0.0); ASIJ = 0.0; SMIN = 1.0
            for j in range(ICB, INB + 1):
                if 0.0 < SIJ[i, j] < 0.9:
                    if j > i:
                        SMID = min(SIJ[i, j], SCRIT); SJMAX = SMID; SJMIN = SMID
            ...
            s = 0.0
            for j in range(ICB, INB + 1): s += MENT[i, j]
            if s < 1.0E-18:
                NENT[i] = 0; MENT[i, i] = M[i]; QENT[i, i] = Q[NK] - EP[i] * CLW[i]
                UENT[i, i] = U[NK]; VENT[i, i] = V[NK]
                for k in range(NTRA): TRAENT[i, i, k] = TRA[NK, k]
                ELIJ[i, i] = CLW[i]; SIJ[i, i] = 1.0
    if EP[INB] >= 0.0001:
        JTT = 0
        for i in range(INB, -1, -1):
            ...
            if i > 0:
                for j in range(i):
                    WDTRAIN += params.G * max(0.0, ELIJ[j, i] - (1. - EP[i]) * CLW[i]) * MENT[j, i]
            ...
            EVAP[i] = SIGT * AFAC * REVAP; WATER[i] = REVAP * REVAP
            if i > 0:
                DHDP = max((H[i] - H[i-1]) / (P[i-1] - P[i]), 10.0)
                MP[i] = 100. * GINV * LV[i] * params.SIGD * EVAP[i] / DHDP
            ...
            if i != INB:
                ...
                if MP[i] > MP[i+1]:
                    RAT = MP[i+1] / MP[i]
                    QP[i] = QP[i+1] * RAT + Q[i] * (1.0 - RAT) + 100. * GINV * params.SIGD * (PH[i] - PH[i+1]) * (EVAP[i] / MP[i])
                    UP[i] = UP[i+1] * RAT + U[i] * (1. - RAT); VP[i] = VP[i+1] * RAT + V[i] * (1. - RAT)
                    for k in range(NTRA): TRAP[i, k] = TRAP[i+1, k] * RAT + TRAP[i, k] * (1. - RAT)
                elif MP[i+1] > 0.0:
                    QP[i] = (GZ[i+1] - GZ[i] + QP[i+1] * (LV[i+1] + T[i+1] * (params.CL - params.CPD)) + params.CPD * (T[i+1] - T[i])) / (LV[i] + T[i] * (params.CL - params.CPD))
    ...
    if NK == 0:
        for ii in range(1, INB + 1): AM += M[ii]
    ...
    for j in range(NTRA): FTRA[0, j] += params.G * DPINV * (MP[1] * (TRAP[1, j] - TRA[0, j]) + AM * (TRA[1, j] - TRA[0, j]))
    for j in range(1, INB + 1):
        ...
        for k in range(NTRA): FTRA[0, k] += params.G * DPINV * MENT[j, 0] * (TRAENT[j, 0, k] - TRA[0, k])
    for i in range(1, INB + 1):
        DPINV = 0.01 / (PH[i] - PH[i+1]); CPINV = 1.0 / CPN[i]
        AMP1 = 0.0
        if i >= NK:
            for ii in range(i + 1, INB + 2): AMP1 += M[ii]
        for k in range(i + 1):
            for jj in range(i + 1, INB + 2): AMP1 += MENT[k, jj]
        if (2. * params.G * DPINV * AMP1) >= DELTI: IFLAG = 4
        AD = 0.0
        for k in range(i):
            for jj in range(i, INB + 1): AD += MENT[jj, k]
        FT[i] += params.G * DPINV * (AMP1 * (T[i+1] - T[i] + (GZ[i+1] - GZ[i]) * CPINV) - AD * (T[i] - T[i-1] + (GZ[i] - GZ[i-1]) * CPINV)) - params.SIGD * LVCP[i] * EVAP[i]
        FT[i] += params.G * DPINV * MENT[i, i] * (HP[i] - H[i] + T[i] * (params.CPV - params.CPD) * (Q[i] - QENT[i, i])) * CPINV
        FT[i] += params.SIGD * WT[i+1] * (params.CL - params.CPD) * WATER[i+1] * (T[i+1] - T[i]) * DPINV * CPINV
        ...
        for k in range(NTRA): FTRA[i, k] += params.G * DPINV * (AMP1 * (TRA[i+1, k] - TRA[i, k]) - AD * (TRA[i, k] - TRA[i-1, k]))
        for k in range(i):
            ...
            for j in range(NTRA): FTRA[i, j] += params.G * DPINV * MENT[k, i] * (TRAENT[k, i, j] - TRA[i, j])
        ...
        FU[i] += params.G * (MP[i+1] * (UP[i+1] - U[i]) - MP[i] * (UP[i] - U[i-1])) * DPINV
        FV[i] += params.G * (MP[i+1] * (VP[i+1] - V[i]) - MP[i] * (VP[i] - V[i-1])) * DPINV
        for j in range(NTRA): FTRA[i, j] += params.G * DPINV * (MP[i+1] * (TRAP[i+1, j] - TRA[i, j]) - MP[i] * (TRAP[i, j] - TRA[i-1, j]))
    ...
    ENTS = 0.0; UAV = 0.0; VAV = 0.0; DENOM2 = PH[0] - PH[INB+1]
    for i in range(INB + 1):
        dp = PH[i] - PH[i+1]
        ENTS += (CPN[i] * FT[i] + LV[i] * FQ[i]) * dp
        UAV += FU[i] * dp; VAV += FV[i] * dp
    ENTS /= DENOM2; UAV /= DENOM2; VAV /= DENOM2
    for i in range(INB + 1):
        FT[i] -= ENTS / CPN[i]
        FU[i] = (1. - params.CU) * (FU[i] - UAV)
        FV[i] = (1. - params.CU) * (FV[i] - VAV)
    for k in range(NTRA):
        TRAAV = 0.0
        for i in range(INB + 1): TRAAV += FTRA[i, k] * (PH[i] - PH[i+1])
        TRAAV /= DENOM2
        for i in range(INB + 1): FTRA[i, k] -= TRAAV
    return FT, FQ, FU, FV, PRECIP, WD, TPRIME, QPRIME, CBMF, OUTCAPE, IFLAG
