...
def lw_single_column(tau, T_layer, T_surface, emissivity=1.0,
                     diagnostics_level=0):
    """Run the LW transport solver on a single column.

    Convenience wrapper for interactive exploration.

    Args:
        tau: (nlev,) optical depth per layer
        T_layer: (nlev,) air temperature per layer, K
        T_surface: surface temperature, K (scalar)
        emissivity: surface emissivity (scalar, default 1.0)
    ...
    T_arr = np.asarray(T_layer)
    nlev = tau_arr.shape[0]

    tau_4d = tau_arr.reshape(1, 1, nlev, 1)
    planck_src = sigma * T_arr.reshape(1, 1, nlev, 1) ** 4
    T_s = float(T_surface)
    surf_src = np.array([[sigma * T_s ** 4]]).reshape(1, 1, 1)
    emis = np.array([[float(emissivity)]])
    weights = np.ones((1, 1))

    T_2d = T_arr.reshape(nlev, 1)
    T_surf_1d = np.array([T_s])

    raw = lw_transport(T_2d, T_surf_1d, tau_4d, planck_src, surf_src, emis,
                       weights, sigma, diagnostics_level=diagnostics_level)
