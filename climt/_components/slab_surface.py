...
class SlabSurface(TendencyComponent):
    ...
    def array_call(self, raw_state):
        ...
        diagnostics["depth_of_slab_surface"][sea_mask] = raw_state[
            "ocean_mixed_layer_thickness"
        ][sea_mask]
        diagnostics["depth_of_slab_surface"][land_mask] = raw_state[
            "soil_layer_thickness"
        ][land_mask]

        mass_surface_slab = (
            raw_state["surface_material_density"] * diagnostics["depth_of_slab_surface"]
        )
        heat_capacity_surface = (
            mass_surface_slab * raw_state["surface_thermal_capacity"]
        )
