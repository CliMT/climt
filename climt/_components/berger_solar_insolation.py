...
try:
    ...
    logging.warning(
        "Import failed. Insolation is likely not compiled and will not be " "available."
    )
...
def _get_solar_parameters_np(
    ...
    lmbda = lambda_m + eccentricity*(
        2.*sin_temp + eccentricity*(
            1.25*np.sin(2*temp) + eccentricity * (
                (13./12.)*np.sin(3*temp) - 0.25*sin_temp)))
    inverse_rho = (
        (1 + eccentricity*np.cos(lmbda - (omega_tilde + np.pi))) / (
