...
class BergerSolarInsolation(DiagnosticComponent):
    ...
    def array_call(self, state):
        ...
        solar_insolation, solar_zenith_angle, obliquity, eccentricity, rho = (
            self._driver(state["time"], lat_flat, lon_flat, solar_constant)
        )
        return {
    ...
    def _driver(self, time, lat, lon, solar_constant):
        ...
        if year not in self._orbital_parameters:
            self._orbital_parameters[year] = _get_orbital_parameters_functional(
                float(year - 1950)
            )
...
def _get_orbital_parameters_functional(years_since_jan_1_1950):
    ...
    obliquity += np.sum(
        A
        * arcsec_to_degree
        * np.cos((f * arcsec_to_degree * t + delta) * np.pi / 180.0)
    )
    obliquity = obliquity * np.pi / 180.0
    ...
    # Equation 6
    omega_tilde = pi_val * 180.0 / np.pi + 50.439273 * arcsec_to_degree * t + 3.392506
    omega_tilde += np.sum(
        F * np.sin((f_prime * arcsec_to_degree * t + delta_prime) * np.pi / 180.0)
    )

    omega_tilde = omega_tilde % 360.0
    omega_tilde = omega_tilde * np.pi / 180.0

    beta = np.sqrt(1.0 - eccentricity_squared)

    # Bullet points 1-4 on page 2365
    lambda_m0 = 2.0 * (
        (0.5 * eccentricity + 0.125 * eccentricity_cubed)
        * (1.0 + beta)
        * np.sin(omega_tilde + np.pi)
        - 0.25 * eccentricity_squared * (0.5 + beta) * np.sin(2 * (omega_tilde + np.pi))
        + 0.125
        * eccentricity_cubed
        * (1.0 / 3.0 + beta)
        * np.sin(3 * (omega_tilde + np.pi))
    )
...
def _get_solar_parameters_np(
    ...
    ncol = lat.size
    solar_zenith_angle = np.zeros(ncol)
    solar_insolation = np.zeros(ncol)

    eccentricity_squared = eccentricity * eccentricity
    lambda_m = lambda_m0 + years_since_vernal_equinox * 2.0 * np.pi
    ...
    lmbda = lambda_m + eccentricity * (
        ...
        * (
            1.25 * np.sin(2 * temp)
            + eccentricity * ((13.0 / 12.0) * np.sin(3 * temp) - 0.25 * sin_temp)
        )
    )
    inverse_rho = (1 + eccentricity * np.cos(lmbda - (omega_tilde + np.pi))) / (
        1 - eccentricity_squared
    )
    rho = 1.0 / inverse_rho
    inverse_rho_squared = inverse_rho * inverse_rho
    solar_declination_angle = np.arcsin(np.sin(obliquity) * np.sin(lmbda))

    sin_delta = np.sin(solar_declination_angle)
    cos_delta = np.cos(solar_declination_angle)

    for i in prange(ncol):
        H = 2 * np.pi * (fractional_day + lon[i] / 360.0)
        cos_mu = np.sin(lat[i]) * sin_delta - np.cos(lat[i]) * cos_delta * np.cos(H)
        solar_zenith_angle[i] = np.arccos(cos_mu)
