...
C 20031112  Added SAVE statements for data-loaded constants. (ACH)
C 20240828  Eliminated Computed GOTOs. (ACH)
C     
...
C  Declare all other variables.
     ...
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      INTEGER I, I1, I2, IFLAG, IMXER, LF0,
...
C-----------------------------------------------------------------------
 200  NSLAST = NST
      ...
      IF ((TN - TOUT)*H .LT. 0.0D0) GO TO 250
      GO TO 400
      ELSEIF (ITASK .EQ. 4 .OR. ITASK .EQ. 5) THEN
      TCRIT = RWORK(1)
...
C Branch on KFLAG.  Here KFLAG = 0 or -1 or -2. ------------------------
      IF (KFLAG .EQ. -1) GO TO 530
      IF (KFLAG .EQ. -2) GO TO 540
C-----------------------------------------------------------------------
C Block F.
...
*DECK DLSODES
     ...
     1            ISTATE, IOPT, RWORK, LRW, IWORK, LIW, JAC, MF)
      ...
      DIMENSION NEQ(*), Y(*), RTOL(*), ATOL(*), RWORK(LRW), IWORK(LIW)
C-----------------------------------------------------------------------
C This is the 28 August 2024 version of
...
C-----------------------------------------------------------------------
     ...
     3   NSLJ, NGP, NLU, NNZ, NSP, NZL, NZU
      INTEGER I, I1, I2, IFLAG, IMAX, IMUL, IMXER, IPFLAG, IREM,
     1   J, LENRAT, LENYHT, LENIW, LENRW, LF0, LIA, LJA,
...
C DIPREP and DPREP do sparse matrix preprocessing if MITER = 1 or 2. ---
      ...
      IF (IPFLAG .NE. -1) THEN
        ...
        IWORK(24) = IPJAN
      ENDIF
      IF (IPFLAG .EQ. -1) GO TO 628
      IF (IPFLAG .EQ. -2) GO TO 629
      IF (IPFLAG .EQ. -3) GO TO 630
      IF (IPFLAG .EQ. -4) GO TO 631
      IF (IPFLAG .EQ. -5) GO TO 632
      IF (IPFLAG .EQ. -6) GO TO 633
      IWORK(22) = LYH
      IF (LENRW .GT. LRW) GO TO 617
...
C Branch on KFLAG.  Here KFLAG = 0 or -1, -2, or -3. -------------------
      IF (KFLAG .EQ. -1) GO TO 530
      IF (KFLAG .EQ. -2) GO TO 540
      IF (KFLAG .EQ. -3) GO TO 550
C-----------------------------------------------------------------------
C Block F.
C The following block handles the case of a successful return from the
C core integrator (KFLAG = 0).  Test for stop conditions.
...
*DECK DLSODA
     ...
     1            ISTATE, IOPT, RWORK, LRW, IWORK, LIW, JAC, JT)
      ...
      DIMENSION NEQ(*), Y(*), RTOL(*), ATOL(*), RWORK(LRW), IWORK(LIW)
C-----------------------------------------------------------------------
C This is the 4 September 2024 version of
...
C     dy(i)/dt = f(i) = f(i,t,y(1),y(2),...,y(NEQ)) (i = 1,...,NEQ).
C
C This is a variant version of the DLSODE package.
...
C           interrupt/restart feature.
C 20031112  Added SAVE statements for data-loaded constants.
C 20240904  Eliminated Computed GOTOs (in Blocks D and F). (ACH)
C
C-----------------------------------------------------------------------
...
*DECK DLSODAR
     ...
     1   JROOT(NG)
C-----------------------------------------------------------------------
C This is the 18 September 2024 version of
...
C           interrupt/restart feature.
C 20031112  Added SAVE statements for data-loaded constants.
C 20240918  Eliminated Computed GOTOs (in Blocks D and F). (ACH)
C
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      ...
      INTEGER I, I1, I2, IFLAG, IMXER, LENIW,
...
*DECK DLSODPK
     ...
     1            ISTATE, IOPT, RWORK, LRW, IWORK, LIW, JAC, PSOL, MF)
      ...
      DIMENSION NEQ(*), Y(*), RTOL(*), ATOL(*), RWORK(LRW), IWORK(LIW)
C-----------------------------------------------------------------------
C This is the 25 September 2024 version of
...
C 20240925  Eliminated Computed GOTOs (in Blocks D and F). (ACH)
C     
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     1   NNI, NLI, NPS, NCFN, NCFL
      INTEGER I, I1, I2, IFLAG, IMXER, LF0, LENIW,
...
C Branch on KFLAG.  Here KFLAG = 0 or -1 or -2 or -3. ------------------
      IF (KFLAG .EQ. -1) GO TO 530
      IF (KFLAG .EQ. -2) GO TO 540
      IF (KFLAG .EQ. -3) GO TO 550
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      ...
      INTEGER JPRE, JACFLG, LOCWP, LOCIWP, LSAVX, KMP, MAXL, MNEWT,
     1   NNI, NLI, NPS, NCFN, NCFL
      INTEGER I, I1, I2, IER, IFLAG, IMXER, LF0,
...
C
      ...
      IHIT = ABS(TN - TCRIT) .LE. 100.0D0*UROUND*HMX
      IF (IHIT) T = TCRIT
      IF (IRFP .EQ. 1 .AND. TLAST .NE. TN) GO TO 400
      IF (IHIT) GO TO 400
      TNEXT = TN + H*(1.0D0 + 4.0D0*UROUND)
      IF ((TNEXT - TCRIT)*H .GT. 0.0D0) THEN
        H = (TCRIT - TN)*(1.0D0 - 4.0D0*UROUND)
...
*DECK DLSODI
     ...
     1          IWORK(LIW)
C-----------------------------------------------------------------------
C This is the 13 November 2024 version of
...
C 20241113  Eliminated Computed GOTOs (in Blocks D and F). (ACH)
C
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     2   LYH, LEWT, LACOR, LSAVF, LWM, LIWM, METH, MITER,
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      INTEGER I, I1, I2, IER, IFLAG, IMXER, IRES,
...
C Branch on KFLAG. -----------------------------------------------------
      IF (KFLAG .EQ. -1) GO TO 530
      IF (KFLAG .EQ. -2) GO TO 540
      IF (KFLAG .EQ. -3) GO TO 400
      IF (KFLAG .EQ. -4) GO TO 550
C-----------------------------------------------------------------------
C Block F.
C The following block handles the case of a successful return from the
C core integrator (KFLAG = 0).  Test for stop conditions.
...
C 20031112  Added SAVE statements for data-loaded constants.
C 20031117  Changed internal names NRE, LSAVR to NFE, LSAVF resp.
C 20241113  Eliminated Computed GOTOs. (ACH)
C
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     2   LENYH, LENYHM, LENWK, LREQ, LRAT, LREST, LWMIN, MOSS, MSBJ,
     3   NSLJ, NGP, NLU, NNZ, NSP, NZL, NZU
      INTEGER I, I1, I2, IER, IFLAG, IMAX, IMUL, IMXER, IPFLAG,
     1    IREM, IRES, J, LENRAT, LENYHT, LENIW, LENRW,
...
C Compute initial dy/dt, if necessary, and load it into YH.-------------
     ...
     1              RWORK(LYD0), IER, RES, ADDA)
      NFE = NFE + 1
      IF (IER .EQ. 1) GO TO 565
      IF (IER .GE. 2) GO TO 560
C Check TCRIT for legality (ITASK = 4 or 5). ---------------------------
 120  CONTINUE
      IF (ITASK .NE. 4 .AND. ITASK .NE. 5) GO TO 125
      TCRIT = RWORK(1)
