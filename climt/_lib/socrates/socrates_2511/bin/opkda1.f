...
C   19890501  Modified prologue to SLATEC/LDOC format.  (FNF)
C   19890503  Minor cosmetic changes.  (FNF)
C   19930809  Renamed to allow single/double precision versions. (ACH)
C   20240619  Replaced Computed GO TO with IF blocks. (ACH)
...
C**End
     ...
     5   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      ...
      DOUBLE PRECISION C, R, S, TP, TN1
      CHARACTER*80 MSG
C
C***FIRST EXECUTABLE STATEMENT  DINTDY
      IFLAG = 0
      IF (K .LT. 0 .OR. K .GT. NQ) GO TO 80
      TFUZZ = 100.0D0*UROUND*SIGN(ABS(TN) + ABS(HU), HU)
      TP = TN - HU - TFUZZ
      TN1 = TN + TFUZZ
      IF ((T-TP)*(T-TN1) .GT. 0.0D0) GO TO 90
C
      S = (T - TN)/H
      IC = 1
      IF (K .EQ. 0) GO TO 15
...
C   19930809  Renamed to allow single/double precision versions. (ACH)
C   20010418  Reduced size of Common block /DLS001/. (ACH)
C   20031105  Restored 'own' variables to Common block /DLS001/, to
C             enable interrupt/restart feature. (ACH)
C   20240626  Replaced Computed GO TO with IF blocks.
...
C If MITER = 5, make MBAND calls to F to approximate J. ----------------
 ...
 560    CONTINUE
      NFE = NFE + MBA
      ENDIF
C MITER = 4 or 5. Add identity matrix. ---------------------------------
      II = MBAND + 2
      DO 580 I = 1,N
        WM(II) = WM(II) + 1.0D0
 580    II = II + MEBAND
...
C   20031105  Restored 'own' variables to Common block /DLS001/, to
C             enable interrupt/restart feature. (ACH)
C   20240624  Replaced Computed GO TO with IF ... THEN blocks.
...
C   20031105  Restored 'own' variables to Common block /DLS001/, to
C             enable interrupt/restart feature. (ACH)
C   20240626  Replaced computed GO TO with IF statements.
...
C   19890503  Minor cosmetic changes.  (FNF)
C   19930809  Renamed to allow single/double precision versions. (ACH)
C   20240417  Replaced Computed GO TO with IF ... THEN blocks. (ACH)
...
C See whether J should be reevaluated (JOK = 0) or not (JOK = 1). ------
      ...
      IF (ICF .EQ. 2) JOK = 0
C See if there was loss of significant figures in P(i,i). --------------
      IF (JOK .EQ. 1) THEN
        RCONT = ABS(CON)/CONMIN
        IF (RCONT .GT. RBIG .AND. IPLOST .EQ. 1) JOK = 0
      ENDIF
C
      IF (JOK .EQ. 0) THEN
C MITER = 1 or 2, and the Jacobian is to be reevaluated. ---------------
      ...
      ENDIF
C
      IF (JOK .EQ. 0 .AND. MITER .EQ. 1) THEN
C If MITER = 1, call JAC, multiply by scalar, and add identity. --------
 ...
 130    CONTINUE
      ENDIF
C
      IF (JOK .EQ. 0 .AND. MITER .EQ. 2) THEN
...
C If JOK = 1, reconstruct new P from old P. ----------------------------
 ...
 275    CONTINUE
      ENDIF
C
C MITER = 1 OR 2.  Do numerical factorization of P matrix. -------------
      NLU = NLU + 1
      CON0 = CON
      IERPJ = 0
      DO 295 I = 1,N
...
C If MITER = 5, make MBAND calls to F to approximate J. ----------------
 ...
 560    CONTINUE
      ...
      ENDIF
C
C MITER = 4 or 5.  Compute norm of Jacobian. ---------------------------
...
C
      IF (JOB .EQ. 2) GO TO 200
      IF (JOB .EQ. 3) GO TO 300
C
...
C Get residual at predicted values, if not already done in PJAC. -------
 240  IRES = 1
      ...
      NFE = NFE + 1
      KGO = ABS(IRES)
      IF (KGO .EQ. 2) GO TO 435
      IF (KGO .EQ. 3) GO TO 430
 250  DO 260 I = 1,N
 260    ACOR(I) = 0.0D0
C-----------------------------------------------------------------------
...
C ISTATE = 3 and MOSS .ne. 0. Load Y from YH(*,1) and S from YH(*,2). --
 ...
 25       S(I) = YH(N+I)
        ENDIF
      IF (MOSS .EQ. 1) GO TO 70
      IF (MOSS .EQ. 2) GO TO 100
      IF (MOSS .EQ. 3) GO TO 150
      GO TO 200
C
