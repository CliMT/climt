...
! *****************************COPYRIGHT*******************************
!
!+ Program to convert Serdyuchenko ozone data (830-1100nm) to uvxsc format.
PROGRAM ser2uvxsc
...
!   File unit numbers
  INTEGER :: i, j, d1, d2
  INTEGER, PARAMETER :: header_length = 61734 ! Start at 829.94nm
  INTEGER, PARAMETER :: record_length = 12
  INTEGER, PARAMETER :: data_length =27007
  TYPE(StrXscHead) :: xsc
...
! Open the file 'SerdyuchenkoGorshelevVersionJuly2013.dat'
  CALL get_free_unit(ierr, iu_dat)
  OPEN(UNIT=iu_dat, FILE='SerdyuchenkoGorshelevVersionJuly2013.dat', &
...
! Read the cross-section data
  READ(iu_dat, *, decimal='comma') ser_data
  CLOSE(iu_dat)
...
!  xsc%wavenumber_min = REAL(FLOOR(ser_wn(data_length)), RealK)
  ...
  xsc%wavenumber_max = 12049.0_RealK ! 829.94 nm
...
! Open a file for the output HITRAN uvxsc database
  CALL get_free_unit(ierr, iu_xsc)
  OPEN(UNIT=iu_xsc, FILE='O3_Serdyuchenko13_830nm-1100nm.uvxsc', &
...
!   Write out the data in HITRAN uvxsc format
