...
SUBROUTINE set_extern_ckd_frn_data(ierr)
...
!   Output status of netcdf call
  ...
  IF (ios /= 0) THEN
    WRITE(iu_err, "(/a)") '*** Error reading foreign continuum file name'
    ierr = i_err_fatal
    RETURN
  END IF
  INQUIRE(FILE=TRIM(file_in), EXIST=l_exist)
  IF (.NOT.l_exist) THEN
    WRITE(iu_err, '(a)') '*** The foreign continuum file does not exist'
    ierr=i_err_exist
    RETURN
  END IF
  ...
  IF (INDEX(file_in, '.nc') > 0) THEN
    ...
    CALL nf(nf90_get_var(ncid, varid, c_foreign_h2o_296%c))
    c_foreign_h2o_296%table_start = c_foreign_h2o_296%c(1)
    c_foreign_h2o_296%table_end = c_foreign_h2o_296%c(c_foreign_h2o_296%n_freq)
    c_foreign_h2o_296%table_inc = ( c_foreign_h2o_296%table_end &
                                  - c_foreign_h2o_296%table_start ) &
                                / REAL( c_foreign_h2o_296%n_freq - 1, RealK )

    ! Get foreign continuum coefficients
    CALL nf(nf90_inq_varid(ncid, 'for_absco_ref', varid))
    CALL nf(nf90_get_var(ncid, varid, c_foreign_h2o_296%c))

    ! Scale coefficients by 1e20 as used in table format
    c_foreign_h2o_296%c = c_foreign_h2o_296%c * 1.0e20_RealK

    ! Uncomment to write out data in old file format
    ! WRITE(200, "(a, a)") 'Foreign continuum data from ', TRIM(file_in)
    ! WRITE(200, "(a/)") '*BEGIN_DATA'
    ! WRITE(200, "(a, 1pe12.5, a)") '     Start of table                = ', &
    !   c_foreign_h2o_296%table_start, ' cm-1'
    ! WRITE(200, "(a, 1pe12.5, a)") '     End of table                  = ', &
    !   c_foreign_h2o_296%table_end, ' cm-1'
    ! WRITE(200, "(a, 1pe12.5, a)") '     Increment in table            = ', &
    !   c_foreign_h2o_296%table_inc, ' cm-1'
    ! WRITE(200, "(a, i5)") '     Number of points in table     = ', &
    !   c_foreign_h2o_296%n_freq
    ! WRITE(200, "(/a)") '     Continuum coefficients (cm^3.molec-1 * 1.0E-20)'
    ! WRITE(200, "(4(1pe15.5))") c_foreign_h2o_296%c
    ! WRITE(200, "(a/)") '*END'
