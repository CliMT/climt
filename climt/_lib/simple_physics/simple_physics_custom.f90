...
!--------------------------------------------------------------
     if (use_ts_ext == 1) then
         do i=1,pcols
            Tsurf(i) = ts(i)
         end do
     else if (test .eq. 1) then     ! moist baroclinic wave with simple-physics
        do i=1,pcols
...
!===============================================================================
  !  call physics_dme_adjust(state, tend, qini, dtime)   ! This is for CESM/CAM
   ...
   return
end subroutine simple_physics_func

END MODULE Simple_Physics_Fort
