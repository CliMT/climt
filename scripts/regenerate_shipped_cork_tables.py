"""Regenerate every shipped cork correlated-k table from the Chaverot
...
data via ``scripts/generate_cork_tables_linepyline.py``; it is NOT regenerated
by this script.

This script is a thin wrapper around ``generate_cork_tables.py`` — it
...
---------------------------------------
* ``bands`` — cork resolution bands in wavenumber (cm^-1).
...
* ``ngpt`` — number of correlated-k g-points per band after re-binning. Two
    g-points is the low-resolution target for cork use; it preserves
...
OUTPUT_DIR = os.path.join(
    CLIMT_ROOT, "climt", "_data", "cork", "correlated_k"
)
GENERATOR = os.path.join(CLIMT_ROOT, "scripts", "generate_cork_tables.py")
...
#     output:  filename under climt/_data/cork/correlated_k/
