"""Heatmaps of CORK/LBL band-mean k ratio over (band, pressure) for multiple
profiles and CORK tables.
...
Reads:
  ...
  - all candidate CORK tables under climt/_data/cork/correlated_k/

Output:
  - examples/lbl_cork_heatmap.png

Run in the climt env:
    conda run -n climt python examples/compare_lbl_cork_heatmap.py
"""
import glob
import os
...
def cork_k_at(table, T, p_pa, h2o):
...
def main():
    ...
    if not tables:
        raise SystemExit("no CORK tables matched")
    ...
    cb.set_label("CORK / LBL  (1 = perfect, <1 CORK under, >1 CORK over)")
    fig.suptitle("Planck-weighted band-mean k: CORK vs LBL  (376 ppm CO2, dry)",
                 fontsize=12, y=0.995)
    out = "examples/lbl_cork_heatmap.png"
