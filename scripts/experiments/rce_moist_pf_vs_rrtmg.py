"""Moist radiative-convective equilibrium: RRTMG-LW vs Picket-Fence-LW tropopause.

Third rung of the model hierarchy (above pure radiative equilibrium and the dry
RCE in `rce_dry_pf_vs_rrtmg.py`). Each single column is driven by a prescribed
absorbed SW flux and relaxed to equilibrium under:

    LW radiation (RRTMG  or  PicketFenceLongwave)
  + SlabSurface
...
and radiative cooling balances. Diagnostic of interest: does PF-LW place the
...
Apples-to-apples LW test (per user choices):
  ...
  * Ozone is zeroed on RRTMG (PF has none) -> both see CO2(+H2O) only.

The DRY counterpart (SimplePhysics no-BL + DryConvectiveAdjustment) is the
separate `rce_dry_pf_vs_rrtmg.py`; the two give different tropopauses and are
meant to be compared.

Run:  conda run --no-capture-output -n climt python scripts/experiments/rce_moist_pf_vs_rrtmg.py [--days N]
"""
import argparse
import os
...
COLUMNS = {
    "RRTMG-LW":               None,
    "PF-LW shipped default":  "earth_low_res_lw_8gpt",
    "PF-LW candD decoupcont": "earth_lw_candD_decoupcont",
}
COLORS = {"RRTMG-LW": "black", "PF-LW shipped default": "#999999",
          "PF-LW candD decoupcont": "#d62728"}
if args.table is not None:
    COLUMNS = {"RRTMG-LW": None, f"PF-LW {args.table}": args.table}
...
def make_lw(table):
    return RRTMGLongwave() if table is None else \
        PicketFenceLongwave(optics="correlated_k", table=table)


def build_column(table):
    """One moist-RCE column. LW comp is built here so the PF band-count global is
...
def tropopause(p_hpa, T, dtheta=10.0):
    ...
    troposphere (unlike the dry, well-mixed +2 K case in rce_dry_pf_vs_rrtmg.py);
...
fig.suptitle(f"Moist RCE: RRTMG-LW vs PF-LW — SimplePhysics(BL) + EmanuelConvection\n"
             f"CO2={int(CO2_MOLE_FRAC*1e6)} ppm, O3=0, SW={SW_FLUX:.0f} W/m^2, {args.days:.0f} days")
plt.tight_layout(rect=[0, 0, 1, 0.93])
out = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "debug_data", "rce_moist_pf_vs_rrtmg.png"))
os.makedirs(os.path.dirname(out), exist_ok=True)
fig.savefig(out, dpi=150, bbox_inches="tight")
print(f"\nSaved {out}")

npz = os.path.join(os.path.dirname(out), "rce_moist_pf_vs_rrtmg.npz")
