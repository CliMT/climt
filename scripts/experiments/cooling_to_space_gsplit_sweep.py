...
TABLES = {
    ...
    "CORK 6-band GL (refined)":        "earth_low_res_lw_co2refined_gl",
}
HR_KEY_RRTMG = "air_temperature_tendency_from_longwave"
HR_KEY_CORK = "air_temperature_tendency_from_longwave"
...
for label, tname in TABLES.items():
    c = CorkLongwaveRadiation(optics="correlated_k", table=tname)
    cork_components[label] = c
    cork_states[label] = get_default_state([c], grid_state=grid)

all_states = {LABELS[0]: rrtmg_state, **cork_states}
all_components = {LABELS[0]: rad_lw, **cork_components}
...
for label in LABELS:
    ...
    key = HR_KEY_RRTMG if label == LABELS[0] else HR_KEY_CORK
