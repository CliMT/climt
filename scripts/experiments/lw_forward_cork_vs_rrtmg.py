"""Experiment #16 — fixed-profile LW forward comparison: RRTMG-LW vs CORK-LW.

Both dry (exp #14) and moist (exp #15) RCE show CORK over-traps relative to RRTMG
...
plus OLR and surface back-radiation. CORK per-band TOA flux localizes spectrally
which CORK band lets least escape (e.g. is the 800-1800 window leaking enough?).

Run: conda run --no-capture-output -n climt python scripts/experiments/lw_forward_cork_vs_rrtmg.py
"""
import argparse
import os
import numpy as np
...
_ap.add_argument("--table", default="earth_low_res_lw_co2refined_linepyline",
                 help="CORK correlated-k table name (without .nc)")
...
def run_scheme(make_comp, T_ref, q_ref, T_surf):
    ...
    if "mole_fraction_of_ozone_in_air" in state:
        state["mole_fraction_of_ozone_in_air"].values[:] = 0.0   # CORK has no O3
...
_, dp = run_scheme(lambda: CorkLongwaveRadiation(optics="correlated_k", table=CORK_TABLE),
                   T_ref, q_ref, T_SURF)
...
print(f"{'quantity':32s} {'RRTMG':>9s} {'CORK':>9s} {'CORK-RRTMG':>9s}")
...
print("  (CORK OLR < RRTMG  AND/OR  CORK back-radiation > RRTMG  => CORK traps more)\n")

# ── Per-level up-flux divergence (where does CORK emit less to space?) ──────────
print(f"{'p_hPa':>8s} {'up_RR':>8s} {'up_CORK':>8s} {'d_up':>7s} | "
      f"{'dn_RR':>8s} {'dn_CORK':>8s} {'d_dn':>7s}")
...
# ── CORK per-band TOA flux: which CORK band lets least escape? ────────────────────
if "upwelling_longwave_flux_in_air_per_band" in dp:
    ckdir = os.path.join("climt", "_data", "cork", "correlated_k", CORK_TABLE + ".nc")
...
axUp.plot(up_r, p_int, "k-", label="RRTMG"); axUp.plot(up_p, p_int, "r-", label="CORK")
axUp.set_xlabel("Upwelling LW (W/m^2)"); axUp.set_title("Upward flux (lower CORK = more trapping)")
axDn.plot(dn_r, p_int, "k-", label="RRTMG"); axDn.plot(dn_p, p_int, "r-", label="CORK")
axDn.set_xlabel("Downwelling LW (W/m^2)"); axDn.set_title("Downward flux")
axHR.plot(hr_r, p_hpa, "k-", label="RRTMG"); axHR.plot(hr_p, p_hpa, "r-", label="CORK")
...
for ax in (axT, axUp, axDn, axHR):
    ...
    if ax is not axT:
        ax.legend()
fig.suptitle("Fixed-profile LW forward: RRTMG vs CORK (identical T,p,q) — isolates the radiative scheme")
plt.tight_layout(rect=[0, 0, 1, 0.95])
out = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..",
                                   "debug_data", f"lw_forward_cork_vs_rrtmg_{TAG}_{CORK_TABLE}.png"))
