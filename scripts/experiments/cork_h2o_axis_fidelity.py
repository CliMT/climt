"""Experiment #13 — CORK table H2O-VMR-axis fidelity vs LBL (lines + MT_CKD).
...
on: it compares the CORK table band-mean k against the linepyline line-by-line
band-mean kappa across the full H2O VMR axis, at tropospheric (T, p) nodes.

LBL reference = `cork_table_builder.kappa_sampling.sample_kappa_grid` with the
...
MT_CKD continuum ON and OFF, so we can:
  * confirm the table tracks the continuum-ON reference (continuum is in the
    table, as the code path in generate_cork_tables_linepyline implies), and
...
Run (linepyline env, has linepyline + MT_CKD + netCDF4):
  /Users/joymonteiro/miniconda3/envs/linepyline/bin/python \
      scripts/experiments/cork_h2o_axis_fidelity.py [--dnu 0.1] [--T 230 290] [--p 1e4 1e5]
"""
...
from cork_table_builder.kappa_sampling import sample_kappa_grid  # noqa: E402

_CK = os.path.join(_REPO, "climt", "_data", "cork", "correlated_k")

# earth scenario composition (matches generate_cork_tables_linepyline.py SCENARIOS["earth"])
BACKGROUND = "air"
...
def cork_band_means(table_path, T_tgt, p_tgt_pa, X_targets):
    """CORK table band-mean k (m^2/kg) at exact (T, p, X) nodes -> dict[band_edge] = array over X."""
    ...
    for tname, base in TABLES.items():
        ...
        for iT, Tv in enumerate(T_grid):
            for iP, pv in enumerate(p_grid):
                pfm, edges = cork_band_means(path, Tv, pv, H2O_VMR_GRID)
                print(f"\n-- T={Tv:.0f} K, p={pv/100:.0f} hPa --")
                print(f"{'band':12s} {'X_H2O':>8s} {'CORK k':>10s} {'LBL_on':>10s} "
                      f"{'CORK/LBL':>7s} {'cont x':>7s}")
                for edge in edges:
