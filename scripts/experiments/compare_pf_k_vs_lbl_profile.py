"""Compare PF k(p, T) to LBL kappa(p, T) across a real T(p) profile.

For each PF k-table of interest, walks the LBL profile (60 levels of dry
adiabat from `lbl_k_adiabat.nc`) and compares:
  - PF: g-integrated band-mean k at each layer's (p, T), bilinearly
    interpolated from the table's (log p, T) grid at X_H2O = 1e-6.
  - LBL: uniform-dnu mean of kappa(nu) inside the PF band's wavenumber
...
p=100 hPa, three bands) holds across all PF bands and all stratospheric
...
the PF integrator. If no, we've localized which bands/layers diverge.
...
PF_DIR = os.path.join(os.path.dirname(__file__), "..", "..",
...
def load_pf_band_mean_k(table_path):
...
def pf_k_at_layer(band_edges, T_grid, log_p_grid, x_grid, k_band,
                  p_pa, T_K, x_h2o):
...
def main():
    ...
    for table in PF_TABLES:
        path = os.path.join(PF_DIR, table + ".nc")
        if not os.path.exists(path):
            print(f"missing {path}, skipping")
            continue
        print()
        print(f"=== {table} ===")
        be, Tg, lpg, xg, k_band = load_pf_band_mean_k(path)
        nband = be.shape[0]
        ...
        # Per-layer k for PF and LBL
        k_pf = np.zeros((len(p_lbl), nband))
        k_lbl = np.zeros_like(k_pf)
        for k, (p, T) in enumerate(zip(p_lbl, T_lbl)):
            k_pf[k, :] = pf_k_at_layer(be, Tg, lpg, xg, k_band,
                                       p, T, X_H2O_TARGET)
            for b in range(nband):
                k_lbl[k, b] = lbl_band_mean(nu_lbl, kappa_lbl[k, :],
                                            be[b, 0], be[b, 1])
        ratio = np.where(k_lbl > 0, k_pf / k_lbl, np.nan)

        # CO2-dominated bands only — LBL is CO2-only, so other bands are
        # apples-to-oranges (PF includes H2O baseline, LBL doesn't).
        ...
        # Restrict comparison to layers where T is in PF table's T_grid
        ...
        print()
        print(f"  Per-layer PF vs LBL band-mean k (CO2-dominated bands), m^2/kg-air:")
        ...
        for target_p in spot_p_hpa:
            ...
            row_pf = "  ".join(f"{k_pf[k_idx, b]:.2e}" for b in range(nband)
                                if co2_mask[b])
            ...
            label_w = max(len(f"{int(be[b,0])}-{int(be[b,1])}") for b in range(nband)
                          if co2_mask[b])
            print(f"  {p_lbl[k_idx]/100:9.2f}  {T_lbl[k_idx]:6.1f}  PF:  {row_pf}")
        ...
        for b in range(nband):
            ...
            ax.semilogx(k_pf[:, b], p_hpa, "C0-", label="PF")
            ...
            ax2.set_xscale("log")
            ax2.set_xlabel("PF/LBL ratio", color="C3", fontsize=8)
        ...
        fig.suptitle(
            f"PF k vs LBL kappa on dry adiabat (T_sfc=288 K, CO2=376 ppm)\n"
            f"table: {table}",
            fontsize=10,
        )
        fig.tight_layout()
        ...
        out = os.path.join(out_dir, f"pf_vs_lbl_k_profile__{table}.png")
