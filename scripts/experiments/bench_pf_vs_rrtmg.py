"""Benchmark PicketFence-LW (consolidated njit(parallel) transport) vs RRTMG-LW.

Wall-clock per call over NCOL columns, warm JIT. Also profiles the PF call to
...
def main():
    ...
    # --- PicketFence-LW (correlated-k, 14-band, prange transport) ---
    ...
    st_p = _make_state(pf)
    mean_p, std_p, min_p = _time_component(pf, st_p, N_TIME)
    print(f"PF-LW (14b x 8g)  : {mean_p:7.2f} ± {std_p:5.2f} ms/call  "
