"""Experiment #21 step 2 (linepyline env) — LBL OLR tiebreak: CORK vs RRTMG vs LBL.

The CORK-vs-RRTMG OLR gap (exp #16) could mean CORK over-traps OR RRTMG under-traps.
linepyline (line-by-line, the same reference that built the CORK k-table) settles it:
run the LBL on the IDENTICAL fixed profile (from dump_forward_profile.py) and see
whether the true OLR sits with CORK or with RRTMG.

To make this apples-to-apples we set the linepyline two-stream diffusivity D=1.66
(CORK's value; rte-rrtmgp uses 1.64) — so the ONLY difference between LBL and CORK is
full spectral resolution vs CORK's 7-band/8-g-point correlated-k. The OLR is also
binned onto CORK's band edges to localize any per-band over-trapping.
...
# CORK winsplit band edges (cm^-1); LBL OLR is binned onto these.
BAND_EDGES = [10.0, 500.0, 630.0, 700.0, 800.0, 1250.0, 1800.0, 3250.0]
NU_MIN, NU_MAX, DNU = 10.0, 3250.0, 0.1
D_CORK = 1.66  # match CORK's diffusivity so only spectral resolution differs
...
def lbl_olr(rtm, p_pa, T, Ts, q, co2):
    ...
    ds = rtm.radiative_transfer(
        ...
        D=D_CORK, line_shape="pseudovoigt", binning=False,
...
def main():
    ...
    print(f"LBL OLR tiebreak — fixed profile, D={D_CORK} (CORK-matched), "
          f"CO2={int(co2*1e6)} ppm, O3=0, Ts={Ts} K\n")

    for tag, q in (("MOIST", q_ms), ("DRY", np.zeros_like(q_ms))):
        # Reference CORK/RRTMG OLR are baked at the profile CO2 (376 ppm); they are
        # meaningless under --co2 override, so fall back to NaN if absent/stale.
        _k_cork, _k_rr = f"olr_cork_{tag.lower()}", f"olr_rrtmg_{tag.lower()}"
        olr_cork = float(d[_k_cork]) if (a.co2 is None and _k_cork in d.files) else float("nan")
        ...
        print(f"  {'TOTAL OLR (10-3250 cm^-1)':32s}  RRTMG={olr_rr:7.2f}   "
              f"CORK={olr_cork:7.2f}   LBL={total:7.2f}")
        print(f"  {'LBL-CORK':32s}  {total-olr_cork:+7.2f}     "
              f"LBL-RRTMG {total-olr_rr:+7.2f}")
        nearer = "CORK" if abs(total - olr_cork) < abs(total - olr_rr) else "RRTMG"
        print(f"  -> LBL is closer to {nearer}\n")
        print(f"  {'band [cm^-1]':18s} {'CORK OLR':>9s} {'LBL OLR':>9s} {'LBL-CORK':>9s}")
        ...
        np.savez(out, nu=nu, olr_spec=olr_spec, total=total,
                 bands=np.array(bands), olr_cork=olr_cork, olr_rrtmg=olr_rr)
