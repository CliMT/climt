"""Experiment T2 — isolate CORK table node-fidelity from interpolation error.

Experiment #4 found the shipped CORK wing band (500-630 cm^-1) is 4.15x more
...
factor by comparing the CORK table k DIRECTLY at its T-nodes (110, 170 K) at
the p=1e4 Pa node (no p-interp, no T-interp) against LBL band-mean k at the
same (T, p).

  ratio_node = CORK_node(T) / LBL(T)        -> pure table/source fidelity
  ratio_147  = CORK_linterp(147) / LBL(147) -> node fidelity + interp curvature
...
CORK_TABLE = _arg if _arg.endswith(".nc") else os.path.join(_CK_DIR, _arg + ".nc")
...
def main():
    with nc.Dataset(CORK_TABLE) as ds:
    ...
    print(f"CORK table: {os.path.basename(CORK_TABLE)}")
    print(f"p node = {np.exp(lpg)[iP]:.0f} Pa, T nodes = {Tg[iT_lo]:.0f}/"
          f"{Tg[iT_hi]:.0f} K, X = {xg[iX]:.1e}\n")
    print(f"{'band':<14} {'CORK@110':>9} {'LBL@110':>9} {'node110':>8}  "
          f"{'CORK@170':>9} {'LBL@170':>9} {'node170':>8}  "
