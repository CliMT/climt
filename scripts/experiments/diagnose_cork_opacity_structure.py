"""Diagnose the vertical opacity structure of the CORK k-tables, using the
...
Outputs: examples/diagnose_cork_opacity_structure.png + console table.
"""
import os
...
NZ = 30
TABLES = {
    "CORK 6-band GL":       "earth_low_res_lw_co2refined_gl",
    "CORK 10-band GL":      "earth_low_res_lw_10band_gl",
    "CORK 4-band 16gpt 2s": "earth_low_res_lw_4band_16gpt",
    "CORK 4-band 32gpt 2s": "earth_low_res_lw_4band_32gpt",
    "CORK 6-band 16gpt GL": "earth_low_res_lw_6band_16gpt",
}
...
def equilibrate_and_diagnose(table_name):
    ...
    lw = CorkLongwaveRadiation(optics="correlated_k", table=table_name)
    sf = SlabSurface()
...
fig.suptitle("CORK opacity vertical structure (from component diagnostic) — "
             "does the wing collapse aloft?", fontsize=12)
plt.tight_layout(rect=[0, 0, 1, 0.96])
out = os.path.join(os.path.dirname(__file__), "diagnose_cork_opacity_structure.png")
