"""Experiment #11 — CORK table band-mean k vs LBL at WARM nodes (230/290 K).

The RCE test (exp #10) left a tropospheric warm bias: CORK is +5 to +16 K warmer
than RRTMG from the surface to ~50 hPa (peak ~+16 K at 600-700 hPa), shared by
all CORK tables. exp #4 attributed this to a warm-T CO2 over-opacity (CORK/LBL ~1.8x
at 288 K), but exps #7/#8 only validated k-fidelity at the COLD nodes (110/170
K). This script closes that gap: compare the CORK table band-mean k DIRECTLY at the
...
we read CORK k at exact (T, p) nodes and g-integrate with the table's own weights.

Run: python scripts/experiments/cork_warm_node_fidelity.py   (climt env is fine)
"""
...
_CK = os.path.join(_HERE, "..", "..", "climt", "_data", "cork", "correlated_k")
...
def band_mean_cork(table_path, T_tgt, p_tgt_pa):
    """CORK table band-mean k (m^2/kg) at the exact (T, p) nodes, dry, per band."""
...
def main():
    ...
    for name, base in TABLES.items():
        ...
        print(f"\n=== {name} ===")
        print(f"{'band':14s} {'T':>5s} {'p_hPa':>7s} {'CORK k':>11s} {'LBL k':>11s} {'CORK/LBL':>8s}")
        for T_tgt in T_TARGETS:
            for p_tgt in P_TARGETS:
                cork, Tnode, pnode = band_mean_cork(path, T_tgt, p_tgt)
                ...
                for edge, key in BAND_KEY.items():
                    ...
                    r = cork[edge] / lbl if lbl > 0 else np.inf
                    tag = "core" if edge == (630.0, 700.0) else "wing"
                    print(f"{edge[0]:.0f}-{edge[1]:.0f} ({tag:4s}) {Tnode:5.0f} "
                          f"{pnode/100:7.1f} {cork[edge]:11.3e} {lbl:11.3e} {r:8.2f}")
