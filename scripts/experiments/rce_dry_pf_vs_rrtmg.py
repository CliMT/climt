"""Dry radiative-convective equilibrium: RRTMG-LW vs Picket-Fence-LW tropopause.

Second rung of the model hierarchy (above pure radiative equilibrium in
`radiative_equilibrium_pf_convergence.py`). Each single column is driven by a
...
over. The diagnostic of interest is the tropopause: does PF-LW place it at a
...
Apples-to-apples LW test (per user choices):
  * Shortwave is a fixed prescribed flux, identical on every column -> only the
    LW scheme differs.
  * Ozone is zeroed on RRTMG (PF tables have none) -> both see CO2(+H2O) only.

Run:  conda run --no-capture-output -n climt python scripts/experiments/rce_dry_pf_vs_rrtmg.py [--days N]
"""
import argparse
import os
...
# label -> PF table name (None = RRTMG)
COLUMNS = {
    "RRTMG-LW":            None,
    "PF-LW candB 12band":  "earth_lw_candB",
}
COLORS = {"RRTMG-LW": "black", "PF-LW candB 12band": "#d62728"}
...
def build_column(table):
    """One dry-RCE column. LW comp is built here so the PF band-count global is
...
# Build RRTMG first (sets 16 LW bands for its own state), then PF columns.
models = {}
...
fig.suptitle(f"Dry RCE: RRTMG-LW vs PF-LW — SimplePhysics(no BL) + DryConvectiveAdjustment\n"
             f"dry, CO2={int(CO2_MOLE_FRAC*1e6)} ppm, O3=0, SW={SW_FLUX:.0f} W/m^2, {args.days:.0f} days")
plt.tight_layout(rect=[0, 0, 1, 0.94])
out = os.path.join(os.path.dirname(__file__), "..", "..", "debug_data", "rce_dry_pf_vs_rrtmg.png")
out = os.path.abspath(out)
os.makedirs(os.path.dirname(out), exist_ok=True)
fig.savefig(out, dpi=150, bbox_inches="tight")
print(f"\nSaved {out}")

npz = os.path.join(os.path.dirname(out), "rce_dry_pf_vs_rrtmg.npz")
