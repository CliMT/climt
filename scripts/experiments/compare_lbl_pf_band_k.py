"""Compare Planck-weighted band-mean k from LBL vs PF correlated-k tables.
...
For each table:
  k_PF(band)  = Σ_gpt w_gpt × k_coefficients(band, gpt; T=250 K, p=P_REF, q=0)
  k_LBL(band) = ∫_band B(ν, 250 K) k_LBL(ν) dν  /  ∫_band B(ν, 250 K) dν
              (k_LBL(ν) at the same p as PF, interpolated onto the LBL p-grid)

Prints a table per .nc with: band edges, k_PF, k_LBL, ratio.

Run in the climt env:
    conda run -n climt python examples/compare_lbl_pf_band_k.py
"""
import glob
import os
...
def pf_band_mean(nc_path, T_ref, p_ref_pa, h2o_ref):
    """Return (band_edges (nband,2), k_pf (nband,), p_grid_pa, T_grid, h2o_grid)."""
    ...
    k_pf = (k_interp * w).sum(axis=1)  # (band,)
    return band_lims, k_pf
...
def main():
    ...
    print(f"\nLBL vs PF band-mean k  @ T={T_REF} K, p={P_REF_HPA} hPa, dry, 376 ppm CO2")
    print("=" * 78)
    for tpath in tables:
        name = os.path.basename(tpath)
        band_lims, k_pf = pf_band_mean(tpath, T_REF, P_REF_HPA * 100.0, H2O_VMR_REF)
        print(f"\n{name}")
        print(f"  {'band (cm⁻¹)':>18s}  {'k_PF (m²/kg)':>14s}  {'k_LBL (m²/kg)':>14s}  {'PF/LBL':>8s}")
        for (lo, hi), kp in zip(band_lims, k_pf):
