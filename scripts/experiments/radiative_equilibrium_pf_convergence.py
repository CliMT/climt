"""Radiative equilibrium: RRTMG LW vs. Picket Fence LW (2 / 4 / 8 g-points).
...
Shows g-point convergence: as g-points increase from 2→4→8, the Picket Fence
...
def load_band_edges(table_name):
    """Return (nband, 2) array of [ν_lower, ν_upper] in cm^-1 for a PF k-table."""
    path = os.path.join(
        os.path.dirname(__file__), "..", "..", "climt", "_data",
        "picket_fence", "correlated_k", f"{table_name}.nc",
    )
...
LABELS = [
    ...
    "PF 6-band Chaverot (defective)",
    "PF 6-band linepyline (fix)",
]
# Table name for each PF label (None for RRTMG — no per-band diagnostic).
PF_TABLE = {
    "RRTMG (16 bands)":              None,
    "PF 4-band GL (shipped)":        "earth_low_res_lw_8gpt",
    "PF 6-band Chaverot (defective)": "earth_low_res_lw_co2refined_gl",
    "PF 6-band linepyline (fix)":    "earth_low_res_lw_co2refined_linepyline",
}
COLORS = ["black", "#999999", "#ff7f0e", "#d62728"]
STYLES = ["-", "--", "-", "-"]
# Diagnostic key for LW heating rate (K day⁻¹) differs between RRTMG and PF
...
# ── RRTMG LW ──────────────────────────────────────────────────────────────────
# Must be created before any PicketFenceLongwave so that get_default_state uses
# 16 LW bands for this state; PF constructors reset the global to 4 bands.
...
# ppm in the stratosphere). The Picket Fence CO2(+H2O) tables have NO ozone, so
# leaving RRTMG's O3 on makes the "head-to-head" compare RRTMG-with-O3 vs
# PF-without — O3's 9.6 um LW band is a major stratospheric absorber. Zero it so
# both sides see the same absorbers (CO2 only, dry).
rrtmg_state["mole_fraction_of_ozone_in_air"].values[:] = 0.0

# ── Picket Fence LW — three table variants ───────────────────────────────────
# Each PF constructor resets the global num_longwave_bands, which get_default_state
# reads when sizing the emissivity field. Build each state immediately after its
# constructor so the band count matches.
pf_shipped = PicketFenceLongwave(optics="correlated_k", table="earth_low_res_lw_8gpt")
surface_pf_shipped = SlabSurface()
pf_state_shipped = get_default_state([pf_shipped, surface_pf_shipped], grid_state=grid)

pf_6b_chav = PicketFenceLongwave(optics="correlated_k", table="earth_low_res_lw_co2refined_gl")
surface_pf_6b_chav = SlabSurface()
pf_state_6b_chav = get_default_state([pf_6b_chav, surface_pf_6b_chav], grid_state=grid)

pf_6b_lpl  = PicketFenceLongwave(optics="correlated_k", table="earth_low_res_lw_co2refined_linepyline")
surface_pf_6b_lpl  = SlabSurface()
pf_state_6b_lpl  = get_default_state([pf_6b_lpl,  surface_pf_6b_lpl],  grid_state=grid)

all_pf_states = [pf_state_shipped, pf_state_6b_chav, pf_state_6b_lpl]
for st in all_pf_states:
...
for st in [rrtmg_state] + all_pf_states:
...
steppers = {
    ...
    "PF 4-band GL (shipped)":         AdamsBashforth([pf_shipped, surface_pf_shipped]),
    "PF 6-band Chaverot (defective)": AdamsBashforth([pf_6b_chav, surface_pf_6b_chav]),
    "PF 6-band linepyline (fix)":     AdamsBashforth([pf_6b_lpl,  surface_pf_6b_lpl]),
}
states = {
    "RRTMG (16 bands)":               rrtmg_state,
    "PF 4-band GL (shipped)":         pf_state_shipped,
    "PF 6-band Chaverot (defective)": pf_state_6b_chav,
    "PF 6-band linepyline (fix)":     pf_state_6b_lpl,
}
...
for step in trange(N_STEPS, desc="Integrating"):
    for label in LABELS:
        ...
        if step % STORE_EVERY == 0:
            ...
            # PF variants expose per-band upwelling fluxes; capture them so we
...
# ── Per-level RRTMG vs PF diagnostic ──────────────────────────────────────────
# What we want: for each PF variant, the per-level temperature and LW heating
...
print()
print(f"Per-level RRTMG vs PF (final snapshot, sim day "
...
for label in LABELS:
    if label == RRTMG_LABEL:
        continue
    pf_snap = history[label][-1]
    T_pf = np.asarray(pf_snap["T_profile"])
    HR_pf = np.asarray(pf_snap["hr_day"])
    dT = T_pf - T_rr
    dHR = HR_pf - HR_rr  # PF − RRTMG; negative = PF cools more
    print()
    print(f"--- {label} ---")
    print(f"  OLR = {pf_snap['olr']:.2f} W m^-2, "
          f"T_sfc = {pf_snap['T_surface']:.2f} K, "
          f"T_strat_min = {T_pf.min():.2f} K @ p = {p_hpa[np.argmin(T_pf)]:.2f} hPa")
    ...
    print(f"  max |ΔHR| = {np.abs(dHR).max()*1000:.3f} mK/day @ p = "
          ...
          f"(PF − RRTMG; negative = PF cools more)")
    print(f"  {'p [hPa]':>9s}  {'T_RR':>7s}  {'T_PF':>7s}  {'ΔT':>7s}  "
          f"{'HR_RR':>9s}  {'HR_PF':>9s}  {'ΔHR':>9s}    units: K, K/day")
    for k in range(len(p_hpa)):
        print(f"  {p_hpa[k]:9.3f}  "
              f"{T_rr[k]:7.2f}  {T_pf[k]:7.2f}  {dT[k]:+7.2f}  "
              f"{HR_rr[k]:9.4f}  {HR_pf[k]:9.4f}  {dHR[k]:+9.4f}")

# Save the history for later analysis without re-running.
_npz_path = os.path.join(os.path.dirname(__file__),
                         "radiative_equilibrium_pf_convergence_history.npz")
_to_save = {}
for label in LABELS:
    snap = history[label][-1]
...
out_path = os.path.join(os.path.dirname(__file__), "radiative_equilibrium_pf_convergence.png")
...
# of the upwelling field. For each PF variant, convert the per-band upwelling
...
pf_labels = [lbl for lbl in LABELS if PF_TABLE[lbl] is not None]
n_pf = len(pf_labels)

fig2, axes2 = plt.subplots(2, n_pf, figsize=(4 * n_pf, 9))
if n_pf == 1:
    axes2 = axes2[:, None]
...
for j, label in enumerate(pf_labels):
    snap = history[label][-1]
    band_edges = load_band_edges(PF_TABLE[label])  # (nband, 2) cm^-1
...
out_path2 = os.path.join(os.path.dirname(__file__), "radiative_equilibrium_pf_per_band.png")
