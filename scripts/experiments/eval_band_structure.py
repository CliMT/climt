...
Inner-loop tool for designing a CORK LW band structure that captures single-column
OLR. Computes CORK per-band TOA OLR on the FIXED exp #16 profile (from
...
GOAL METRIC: |LBL - CORK| total OLR, moist & dry. Target < 5 W/m^2 (= RRTMG's own
agreement with LBL: 4.1 moist / 7.3 dry). Per-band LBL-CORK localizes residual
over-trapping.
...
CKDIR = os.path.join("climt", "_data", "cork", "correlated_k")
...
def cork_olr(table, T, q, Ts, co2, grid):
    cork = CorkLongwaveRadiation(optics="correlated_k", table=table)
...
def main():
    ...
    print(f"{'case':6s} {'RRTMG':>8s} {'CORK':>8s} {'LBL':>8s} "
          f"{'LBL-CORK':>8s} {'LBL-RRTMG':>10s}")
    for tag, q in (("moist", q_moist), ("dry", np.zeros_like(q_moist))):
        total_cork, per_band_cork = cork_olr(table, T, q, Ts, co2, grid)
        ...
        print(f"{tag:6s} {rr:8.2f} {total_cork:8.2f} "
              f"{lbl_total:8.2f} {lbl_total-total_cork:+8.2f} "
              f"{lbl_total-rr:+10.2f}")
        # per-band LBL-CORK
        print(f"   {'band [cm^-1]':16s} {'CORK':>8s} {'LBL':>8s} {'LBL-CORK':>8s}")
        for (lo, hi), cork_b in zip(edges, per_band_cork):
            sel = (nu >= lo) & (nu < hi)
            lbl_b = float(np.sum(olr_spec[sel]) * DNU)
            flag = "  <<" if abs(lbl_b - cork_b) > 4.0 else ""
            print(f"   {f'{lo:.0f}-{hi:.0f}':16s} {cork_b:8.2f} {lbl_b:8.2f} "
                  f"{lbl_b - cork_b:+8.2f}{flag}")
        print()
